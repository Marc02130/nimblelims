...
- `GET/PUT /v1/eln-process-definitions/{id}/steps/{step_id}/accepted-sample-types`. SoT for inbound types on experiment **and** LimsRun steps (E-7). `POST /v1/experiments/{id}/start` and `PATCH /v1/lims-runs/{id}/start` type-gate against this list when the instance is a process step (**422** `route_sample_type`). Template `accepted_sample_types` is refused (**422** `accepted_sample_types_not_on_template`). Derive the first process and its first ordered Experiment/LimsRun allowed types on read. Any stored display copy must refresh when process order or first-step acceptance changes.
