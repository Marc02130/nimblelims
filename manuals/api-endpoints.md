...
`POST /v1/asked-for` creates no Test or work order. Route later evaluates analysis + TAT candidates and first-process type acceptance. **Tobias-signed Pass on `8cfa2a9`:** zero → **422** “No routing-map row accepts this analysis, TAT, and sample type”; exactly one → ordered route snapshot. Two saved rows that both accept current type → **409** is **unsigned** this SHA. Never silently choose `first()`. WO-7 creates/attaches Tests at LimsRun start and writes `asked_for_params` on a **new** Test (Pass on `99b692d3`); classic `/tests` skip remains OPEN.
...
Promotes instrument JSONB → Tests/Results in one transaction (**run always has `analysis_id`**). Each active Test must already exist from the first LimsRun start (WO-7). If any cohort sample lacks one, publish returns **422**, refuses the whole run, writes no Results, invents no Test, and leaves the run `complete`. This refuse is **Tobias-signed Pass** on `8cfa2a9` (carol **422** `test_missing`) and remains history on `b005cfe`. New-Test freeze write is Pass on `99b692d3` only; classic skip **OPEN**. Overall P2 Pass remains unsigned. Historical `9c4f9da` remains signed not Pass. **409** with `code: promotion_conflict` if another run/manual result owns the same test/analyte/replicate.

### PATCH /v1/lims-runs/{id}/start
**Requires `analysis_id`** on the run and at least one cohort sample — **400** if either is missing. With `sample_ids` in the body, first start of the **asked-for** analysis locks the cohort, creates or attaches one active Test per `(sample, analysis)`, and **writes** `asked_for_params`. Classic `/tests` must leave `asked_for_params` **NULL**; NULL or default `{}` is not a freeze, and skip-on-`{}` is not one either — classic default `{}` and frozen `{}` are the same JSON. Extract LimsRun must not share this `analysis_id`.

**WO-7 first-start freeze is not closed.** New-Test write is **Tobias-signed Pass** on `8cfa2a9` (Test `99b692d3`, `asked_for_params` `{}` written, not SQL NULL, not a skipped classic row; later start no overwrite on that path). Do **not** fold classic `/tests` skip as Pass. Do **not** claim `{}` vs frozen `{}` is distinguishable. `if test: continue` is **not** a freeze. A classic `/tests` row with NULL or default `{}` is **not** frozen — first LimsRun start must **write**, and classic `/tests` must leave `asked_for_params` NULL. **Skip-on-`{}` is not a freeze**: a classic default `{}` and a frozen `{}` are the same JSON. Skip is honest only once classic `/tests` leaves NULL or a **freeze marker** lands. Extract LimsRun must not share the asked-for `analysis_id` or it attaches/freezes the panel Test at extract start.
