...
These are explicit later planning actions, not calls that Receive or asked-for create should chain automatically. Body for batch: `{ "asked_for_ids": ["uuid"] }`. **200** `{ items: [{ asked_for_id, work_order, no_route }], count }`. Empty map → `no_route: true`, status remains `requested`, and nothing is minted. Maps match analysis + TAT; there is no admin-authored sample type. Before mint, Route compares the sample’s current type with the first ordered Experiment/LimsRun step’s derived allow-list. A mismatch returns **422** `detail.code = "route_sample_type"` and mints nothing. A match creates a queued `work_order`, changes asked-for to `routed`, and creates no Test; minting that queue record does not mean work has started. Write requires `test:assign` plus project access, not `experiment:manage`; Client and inaccessible-project writes return **403**, not 404.

### Routing map / work orders
- `GET/POST /v1/routing-map` · `PATCH/DELETE /v1/routing-map/{id}` (write: `config:edit`). Rows author analysis × inclusive TAT range → one process definition; TAT overlap → **409**. Retract the sample-type create field.
- `GET/PUT /v1/eln-process-definitions/{id}/steps/{step_id}/accepted-sample-types`. The map UI derives and displays allowed types for the selected first/only process and its first ordered Experiment or LimsRun step. The display is informational and may refresh or be stored when process/step selection changes; it is not admin-authored and map save performs no chain-wide type check.
- `GET /v1/work-orders` (requires `sample:read`; filters: `status`, `sample_id`) returns `{ items, count }`.
- `POST /v1/work-orders/{id}/start` (requires `experiment:manage`) instantiates the first snapshot process definition, assigns the work-order sample, sets the work order `in_progress`, and returns its `process_id`. The linked ELN process owns the ordered Experiment/LimsRun step execution; Route must expose that order rather than an unordered step bag. Route has already gated the first step. Each later step start compares the sample’s current type with that step’s accepted types; empty or incompatible returns **422** `detail.code = "route_sample_type"` and does not mean the sample is broken. P2 does not create a second workflow home.
