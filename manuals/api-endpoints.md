...
- `GET/PUT /v1/eln-process-definitions/{id}/steps/{step_id}/accepted-sample-types`. Map save performs no chain-wide type check. The map UI displays the first ordered Experiment or LimsRun step’s allowed types for information only.
- `GET /v1/work-orders` (requires `sample:read`; filters: `status`, `sample_id`) returns `{ items, count }`.
- `POST /v1/work-orders/{id}/start` (requires `experiment:manage`) instantiates the first snapshot process definition, assigns the work-order sample, sets the work order `in_progress`, and returns its `process_id`. The linked ELN process owns the ordered Experiment/LimsRun step execution; Route must expose that order rather than an unordered step bag. Starting each step compares the sample’s current type with that step’s accepted types. A mismatch returns **422** `detail.code = "route_sample_type"` and does not mean the sample is broken. P2 does not create a second workflow home.
