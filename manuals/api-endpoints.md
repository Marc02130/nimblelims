...
**WO-7 first-start freeze is unsigned on `8cfa2a9` — not closed, not verified.** `if test: continue` is **not** a freeze. A classic `/tests` row with NULL or default `{}` is **not** frozen — first LimsRun start must **write**, and classic `/tests` must leave `asked_for_params` NULL. **Skip-on-`{}` is not a freeze**: a classic default `{}` and a frozen `{}` are the same JSON, so `{}` cannot prove a first-start snapshot. Skip is honest only once classic `/tests` leaves NULL or a **freeze marker** lands. Extract LimsRun must not share the asked-for `analysis_id` or it attaches/freezes the panel Test at extract start.
