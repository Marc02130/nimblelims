...
| **First-start freeze** | **Not closed.** New-Test write is **Tobias-signed Pass** on `8cfa2a9` (Test `99b692d3`; `asked_for_params` `{}` written, not SQL NULL, not a skipped classic row; later start no overwrite on that path). Classic `/tests` skip **OPEN** — do not fold as Pass. Do **not** claim `{}` vs frozen `{}` is distinguishable. `if test: continue` is **not** a freeze, and neither is skip-on-`{}`. Classic `/tests` must leave `asked_for_params` **NULL**. Skip is honest only once classic `/tests` leaves NULL or a **freeze marker** lands. Extract LimsRun must not share the asked-for `analysis_id` |
