...
| **First-start freeze** | **Unsigned on `8cfa2a9` — not closed, not verified.** `if test: continue` is **not** a freeze, and neither is skip-on-`{}` (classic default `{}` and frozen `{}` are the same JSON). Classic `/tests` must leave `asked_for_params` **NULL**; NULL or default `{}` is **not** frozen — first start must **write**. Skip is honest only once classic `/tests` leaves NULL or a **freeze marker** lands. Extract LimsRun must not share the asked-for `analysis_id` or it attaches/freezes the panel Test at extract start |
...
| **Permissions** | **Publish** may create/update Results on this path, but may not create the WO-7 Test |
...
- **Analyses / Analytes**: Catalog assays. Aliases on analytes support multi-CRO column names. Every run **must** have an analysis. The WO-7 lock is that the first start of the **asked-for** analysis creates or attaches the Test and writes `asked_for_params`. Classic `/tests` must leave it **NULL**; NULL or default `{}` is not a freeze, and skip-on-`{}` is not one either — classic default `{}` and frozen `{}` are the same JSON. Extract LimsRun must not share that `analysis_id`. Freeze is **unsigned until QA**.
...
- Main pages: RunsManagement, LimsRunDetail (Overview / Data / Dose Response tabs)
  - Overview: **Analysis** and cohort required to start; first start locks the cohort, creates or attaches each Test, and freezes asked-for params (lock; **in code on `8cfa2a9`, unsigned until QA**). **Publish** opens promotion preview. If any cohort Test is missing, WO-7 refuses the whole publish with **422**, writes no Results, creates no Test, and leaves the run `complete`. Publish-refuse is **Tobias-signed Pass** on `b005cfe`. Overall P2 Pass remains unsigned. `9c4f9da` / `b005cfe` remain signed history.
