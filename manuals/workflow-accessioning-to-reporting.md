...
**Separate later look-up:** **Asked-for** records requested analysis + TAT and creates zero Tests/work orders. Later Route evaluates analysis + TAT candidates against current type: zero acceptable → 422; multiple → 409; exactly one snapshots ordered `process_definition[]`. Work Orders starts the first process only; later starts advance in order. None of this is part of Receive.
