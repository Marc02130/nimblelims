...
Separately, **Asked-for** records requested analysis + TAT without minting work. Later Route yields 422 for zero acceptable rows, 409 for multiple, and snapshots an ordered process route only when exactly one accepts. Start instantiates its first process only. See [asked-for.md](asked-for.md). All remain **out of Receive**.
