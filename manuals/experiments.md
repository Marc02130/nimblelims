...
One **Add aliquot/pool** (template **+ Aliquot/pool** or ad hoc **Add aliquot/pool**) creates the `aliquot_pool_plan` (`experiment_data`) and empty `aliquots_pools` (`experiment_sample_data`) entries together. The UI must not offer plan-only or destination-only authoring. **Either half is the pair:** deleting the dest entry removes both, same as deleting the plan. API **POST** of either side (`aliquot_pool_plan` or `aliquots_pools`) creates the mate (**201**; GET includes both). **DELETE** of either inactivates both. After the pair is gone, Add re-enables — one pair at a time per template or experiment, not a lifetime lock. E-10 UAT §7 is **Fail** (Tobias QA, 2026-09-14 21:31 ET, `e5a8fdd`; packet 7.1–7.8 Pass; overall Fail = Deiter double-Add — API second plan POST while a pair or half exists). Do **not** invent overall §7 Pass. **Rolf Confirm: Hold merge** — `feat/e10-aliquot-atomic-pair` does **not** go to `main` until server-side single-pair uniqueness lands and Tobias restamps; that fix is not in yet. **Tobias dogfood Ready=Yes** on `feat/e10-aliquot-atomic-pair` (2026-09-14 21:23 ET; `/workspace/dogfood-e10-e5a8fdd/READY.md`; SHA **`e5a8fdd`**; paths **1–5 Pass**; clean FE Docker/CRA green) is dogfood history, **not** a UAT Pass; the earlier **No Hold** covered starting §7. Prior Ready=No on **`9312c54`** (TS2345) stays history. **Deiter Lab Ops Confirm** of Marc’s fold is not a Pass. §§1–6 stay `008baf2`. Selecting a concrete method immediately attaches both `METHOD_CATALOG` maps: plan columns and destination FieldDefinitions. The destination entry is populated only after execute.
