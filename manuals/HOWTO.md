...
**Aliquot/pool atomic pair (E-10):** One **+ Aliquot/pool** / **Add aliquot/pool** creates plan (`aliquot_pool_plan`) and dest (`aliquots_pools`) together. POST either side creates the pair. Delete or DELETE either half removes or inactivates both. After the pair is gone, Add re-enables (one pair at a time — not a lifetime lock). Dest stays empty until execute. UAT §7 **Unsigned**. **Tobias dogfood Ready=Yes** on `feat/e10-aliquot-atomic-pair` (2026-09-14 21:23 ET; `/workspace/dogfood-e10-e5a8fdd/READY.md`; SHA **`e5a8fdd`**). Paths **1–5 Pass**. Clean FE Docker/CRA green; no local patch. Ready=Yes is **not** §7 Pass. Do **not** invent §7 Pass. **Rolf Confirm: No Hold.** Ready=Yes stands on **`e5a8fdd`**. Tobias proceeds with formal §7. Prior Ready=No on **`9312c54`** (TS2345) stays history. **Deiter Lab Ops Confirm** of Marc’s fold (not a Pass). Do **not** restamp dest-container-type §§1–6 (`008baf2`). See [experiments.md](experiments.md).
...
- Do **not** teach plan-only or dest-only aliquot/pool add from presets. Do **not** teach deleting one half and leaving the other. Do **not** treat Add staying disabled after the pair is gone as the lock (one pair at a time). E-10 UAT §7 is **unsigned** until Tobias stamps Pass/Fail (**Rolf Confirm: No Hold**; Ready=Yes on **`e5a8fdd`** is not a Pass; **Deiter Lab Ops Confirm** of Marc’s fold is not a Pass). Do **not** restamp dest-container-type §§1–6 (`008baf2`).
