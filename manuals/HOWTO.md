...
4. **Route:** snapshots the ordered list, **zero Tests**, **zero** dest samples/containers. Dest type on a later aliquot/pool plan is catalog intent until execute.
...
- Do **not** teach dest existing at Route / Start / map-save / receive / asked-for. Dest type on the plan is catalog intent until execute. Dest exists only after aliquot/pool execute.
- Do **not** write C2 or C3 Pass. Deiter C2 **Fail** on `02fe95f` stands as signed history. `570bbc0` does **not** inherit C2 Pass or Fail. C2 execute on `1572071` is extra container, same sample (`_follow_destination_in_process`; emptied-source assign **422**) and C3 is the different-dest-type click — numbered on `570bbc0`, neither is QA-clicked, neither is a shipped Pass. **Fail C3 if dest tube is on the blood sample.** Do **not** teach `570bbc0` as a product execute SHA. Type-changing execute must not retarget the parent’s `container_id`. **PATCH is not a path.**
