...
**Current P2 status (2026-09-04):** Named-slot product is on `feat/p2-named-slot`; the product SHA to land is **`6a67667`** (`0079`) after the rebase onto `8887e36`. **AC-P2-OQ-WO-8 Pass** (Tobias, 2026-09-03, `6244bf6`) plus a **merge-gate restamp Pass** (Tobias QA, 2026-09-04, click SHA **`effd242`**, product `6a67667`). **Tobias overall P2 Pass (QA)** 2026-09-03 21:26 ET on `6244bf6`, folding `bf51b19` + `80f054b` + `6244bf6`. **Full Leadership Confirm overall P2 Pass** (Rolf / Deiter / Hans / Heidi / Günter) on that same SHA; the restamp does not rewrite it. Older “overall P2 unsigned” text in historical table rows records those earlier SHAs. This is not a claim that the named-slot product has merged to `main`; merging on the `6244bf6` stamp alone is not the gate.
...
**Named-slot product status (2026-09-04): landed on `feat/p2-named-slot`; the product SHA to land is `6a67667` (Alembic `0079`) after the rebase onto `8887e36`.** Containment eligibility is not the current behavior on this branch; it is only what `main` still carries, because named-slot code has not merged. The merge gate is the **restamp**: **AC-P2-OQ-WO-8 Pass** (Tobias QA merge-gate restamp, 2026-09-04 20:18 ET, click SHA **`effd242`**, product `6a67667`) — not the `6244bf6` stamp alone. **AC-P2-OQ-WO-8 Pass** (Tobias): `routing_map.asked_for_step_id` is the author-named LimsRun slot; Route compares `asked.analysis_id` with that slot, not chain containment. One named Qubit match mints; zero returns **422**; 2+ returns **409** `route_pick_required`, shows the picker, and mints only after selection. WGS+Qubit-as-process-QC does not steal Quantified DNA. **Tobias overall P2 Pass (QA)** 2026-09-03 21:26 ET on `6244bf6`, folding `bf51b19` + `80f054b` + `6244bf6`. **Full Leadership Confirm overall P2 Pass** (Rolf / Deiter / Hans / Heidi / Günter, 2026-09-03) on the same SHA, folding that Tobias overall. Distinct stamps. This branch status is not a claim that the named-slot product has merged to `main`.
