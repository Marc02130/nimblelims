...
3. P2 finds rows whose TAT matches, then keeps the route whose **first process’s first ordered Experiment/LimsRun** accepts the sample’s current type **and**, when the ask is an **assay**, whose **named asked-for LimsRun slot** has `asked.analysis_id` (exactly one assay LimsRun; supporting LimsRuns — Qubit / Nanodrop — are not a second asked-for). Eligibility is **not** “any chain that contains Qubit.” A WGS map with Qubit as process QC must **not** steal a Quantified DNA ask (409 / wrong join). **Quantified DNA** does **not** match extract-only zero-LimsRun routes (old 1.4 **struck**; **422** on 0 LimsRuns). **Tobias-signed Pass on `8cfa2a9`:** zero acceptable rows returns **422** and mints nothing. Route two-accept **409** is **unsigned** that SHA. Never silently call `first()`.
