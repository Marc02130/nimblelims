...
| Later Route / work order (planner, not after receive) | Surfaces on this P2 branch. Live SHA `9342439` — **Tobias-signed AC-P2-9..11 Pass**. Round 2 **CEO Confirm** (R2-1…R2-4) pending Deiter / Hans / Heidi / Günter. A route **may have multiple LimsRun analyses**; asked-for matches any chain that **contains** that analysis. Freeze skip **OPEN**. **OQ-WO-6 stays OPEN** (CEO Confirm: earlier LimsRun must not share asked-for `analysis_id`; do not teach extract-as-special). Dest mint **Hold**. Parser at import. **No overall P2 Pass**. Hold product merge. |
...
**Leadership honesty locks (Deiter / Hans / Heidi / Günter) — restamp honesty on `9342439`, not a merge vote.** Tobias-signed AC-P2-9..11 Pass. Later-step type-gate **Met**. AC-P2-11 map-save only. Dest-type mint Hold. Freeze skip OPEN. **OQ-WO-6 stays OPEN** (CEO Confirm R2-3; pending Deiter / Hans / Heidi / Günter). Route stays `test:assign`. Overall P2 unsigned. Not IC50.
