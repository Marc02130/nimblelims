...
2. **OQ-WO-6 (extract is a process):** Extract is experiment / aliquot-pool **execute**, not a LimsRun, and has **no** asked-for `analysis_id`. Type gates catch blood-on-Qubit. Blood→DNA is a new derivative sample in a new container (`parent_sample_id`). The asked-for assay (ELISA or **sequencing**) appears on **exactly one** LimsRun. Qubit / Nanodrop are **supporting LimsRuns in the same route** (other `analysis_id`s, own Tests, own params freeze — not a second asked-for, not on extract). Map-save / Route **422** if asked-for analysis appears 0 or 2+ times among LimsRuns.
