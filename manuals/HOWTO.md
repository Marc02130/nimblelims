...
| Later Route / work order (planner, not after receive) | Surfaces on this P2 branch (`8cfa2a9`). Route snapshots the ordered list, **zero Tests**. First Start = `chain[0]` only — if it mints later processes/Tests, that is a punch, not shipped. Live AC-P2 **unsigned**. Hold product merge |
| Process / Experiment / LimsRun | Execute substrate shipped. Later Start = next pending process, on the sample that exists then. WO-7 freeze is **unsigned on `8cfa2a9`** — `if test: continue` is **not** a freeze, and skip-on-`{}` is not one either; not closed, not verified |
| Results | Classic type-a-number on a Test; persist lock is a later packet. WO-7 whole-run publish-refuse is **Tobias-signed Pass** on `b005cfe` (history). Overall P2 Pass remains unsigned |
...
**The lake accepts nonsense on purpose.** Qubit-on-blood may sit in it. The live lock (in code on `8cfa2a9`, **unsigned until QA**) is that Route compares the sample’s current type with the mapped process’s first ordered Experiment/LimsRun allow-list; a mismatch is `route_sample_type` **422**. Later starts repeat that current-type check. The lake and routing-map save do not run a chain-wide check.
...
**Punch:** Route does **not** start the chain. Start does **not** start the chain. Route snapshots the ordered list and mints **zero Tests**. **First Start instantiates `chain[0]` only.** Later Start = the next pending process, on the sample that exists then. This is not a process-of-processes at Route.

**If first Start also mints later processes (Qubit/reporting) or their Tests, that is a punch — do not teach it as shipped.**

Route 0→**422** / two-accept→**409**, first-start freeze, and later-start type gates are **unsigned on `8cfa2a9`**. Do not teach them as UAT Pass or freeze-closed.

**Leadership honesty locks (Hans / Heidi / Günter) — live AC-P2 unsigned on `8cfa2a9`. Hold product merge. Dest-type Hold out. Not IC50.**

1. **Freeze skip:** `if test: continue` is **not** a freeze. Classic `/tests` must leave `asked_for_params` **NULL**; a row with NULL or default `{}` is **not** frozen. First LimsRun start must **write** the snapshot onto that Test. **Skip-on-`{}` is not a freeze either** — a classic default `{}` and a frozen `{}` are the same JSON, so `{}` proves nothing. Skip becomes honest only once classic `/tests` leaves `asked_for_params` NULL or a **freeze marker** lands; until then, skip only on a provable LimsRun-start snapshot. Do not claim freeze closed/verified on `8cfa2a9`.
...
3. **Start:** First Start instantiates `chain[0]` only. Later Start = next pending process, on the sample that exists then.
...
3. P2 finds rows whose analysis + TAT match, then keeps only rows whose **first process’s first ordered Experiment/LimsRun** accepts the sample’s current type. **In code on `8cfa2a9` (unsigned until QA):** zero acceptable rows returns **422** and mints nothing; a type refusal uses `route_sample_type`. Two or more saved rows that both accept this current type return **409**. Never silently call `first()`.
...
5. Open Experiments → **Work Orders** (`/work-orders`) and choose **Start process**. **First Start instantiates `chain[0]` only**, links it through `eln_processes.work_order_id`, and opens it at `/experiments/processes/{id}`. It does not mint a process-of-processes. If this click also mints later processes (Qubit/reporting) or their Tests, that is a punch — do not teach it as shipped. Start process / LimsRun start remain `experiment:manage`; publish is `experiment:publish`.
6. Complete that process, then use **Later Start** for the next pending process, on the sample that exists then. Each later process/step start compares the sample’s **current** type with the allow-list at that start; empty or incompatible returns **422** `route_sample_type` **unsigned on `8cfa2a9`**. The sample is not broken. Dest-type Hold is out: do not claim extract already minted a daughter unless the product did.

**WO-7 first-start freeze is unsigned on `8cfa2a9`. Do not claim it closed or verified.** `if test: continue` is **not** a freeze. Classic `/tests` must leave `asked_for_params` **NULL**; a row with NULL or default `{}` is **not** frozen — the first LimsRun start for that asked-for analysis must **write** the snapshot onto that Test. **Skip-on-`{}` is not a freeze**: classic default `{}` and frozen `{}` are the same JSON, so an empty object cannot prove a first-start snapshot. Skip is honest only once classic `/tests` leaves `asked_for_params` NULL or a **freeze marker** lands; until then, skip only on a provable LimsRun-start snapshot. Extract LimsRun must **not** share the asked-for `analysis_id` or it attaches/freezes the panel Test at extract start. Do not claim overall P2 Pass.
...
The WO-7 lock puts Test selection/creation and the params freeze on the **first LimsRun start** for the asked-for analysis — **not** on receive, Route, or work-order Start. `if test: continue` is **not** a freeze, and neither is skip-on-`{}` — classic default `{}` and frozen `{}` are the same JSON. Classic `/tests` must leave `asked_for_params` **NULL**; NULL or default `{}` is not frozen, so the first start must **write**. Skip is honest only once classic `/tests` leaves NULL or a freeze marker lands. Extract LimsRun must not share the asked-for `analysis_id` or it attaches/freezes the panel Test at extract start. **Unsigned on `8cfa2a9` — not closed, not verified.**
...
- Do **not** teach skip-on-`{}` as a freeze. A classic default `{}` and a frozen `{}` are the same JSON, so `{}` cannot prove a first-start snapshot. Skip is honest only once classic `/tests` leaves `asked_for_params` NULL or a freeze marker lands.
