...
3. P2 finds rows whose TAT matches, then keeps **any** route whose **first process’s first ordered Experiment/LimsRun** accepts the sample’s current type **and**, when the ask is an **assay**, whose chain has the asked-for analysis on **exactly one** LimsRun (the route may have supporting LimsRuns too — Qubit / Nanodrop — not a second asked-for). **Quantified DNA** does **not** match extract-only zero-LimsRun routes (old 1.4 **struck**; **422** on 0 LimsRuns). Named-slot pending. **Tobias-signed Pass on `8cfa2a9`:** zero acceptable rows returns **422** and mints nothing. Route two-accept **409** is **unsigned** that SHA. Never silently call `first()`.
...
- Do **not** hang the asked-for panel `analysis_id` on extract or library prep — extract is typically an **experiment** (equipment). Do **not** forever-ban extract-as-LimsRun. **After extract** on a **WGS / WES** ask, Qubit / Nanodrop / other QC is a **LimsRun on the DNA** (instrument, own Test). **Process QC is not an asked-for.** Do not put QC on extract. Type gates are a different control (blood-on-Qubit). **Do not hang ELISA on the DNA dest after C3.** **Quantified DNA ask (1.4 / OQ-WO-8 OPEN; CEO Confirm 1–6 Rolf):** wear existing Qubit as the asked-for LimsRun; do **not** mint a second catalog analysis named Quantified DNA; other process QC supporting. Old 1.4 struck; **422** on 0 LimsRuns. Named-slot pending. Do **not** code extract-only zero LimsRuns.
