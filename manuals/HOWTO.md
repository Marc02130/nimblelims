...
2. **OQ-WO-6 (earlier LimsRun `analysis_id`):** Extract is **not** a special assay. Type gates catch blood-on-Qubit. Blood→DNA is a new derivative sample in a new container (`parent_sample_id`); the parent stays. Any **earlier** LimsRun that reuses the asked-for panel analysis mints/freezes that Test on the parent sample. Earlier LimsRun = own analysis, or experiment-only.
...
- Do **not** conflate same-type follow with type-changing derivative mint. Same dest type = same sample, additional container. Different dest type = new derivative sample in a new container (`parent_sample_id`); parent stays.
