...
2. Scan or type the **primary barcode**. Optionally **Add** more barcodes for the **same** sample (more tubes, not aliquots). **Marc lock:** that second blood tube **may** carry its **own asked-for and own route** (equivalent aliquot / extra container on the same Sample). Do not teach both tubes as one asked-for.
...
2. **OQ-WO-6 (extract does not wear the panel `analysis_id`):** Extract uses **equipment**, not instruments, on the common path. It is an **experiment** (aliquot/pool execute) that may sit on a process. Do **not** forever-ban extract-as-LimsRun if that equipment later is an instrument. On the common path extract has **no** asked-for panel `analysis_id`. LimsRun is instruments / analysis / Tests. **After extract** on a **WGS / WES** ask, a **QC LimsRun** (Qubit, Nanodrop, or other QC instrument) may quantify the DNA — own Test, own params freeze. **Process QC is not an asked-for.** Do not invent a second asked-for for Qubit/Nanodrop. Do not put QC on extract. Type gates catch blood-on-Qubit. Blood→DNA is a new derivative sample in a new container (`parent_sample_id`). When the ask **is** an assay (ELISA, NGS, sequencing, …) it appears on **exactly one** LimsRun. Map-save / Route **422** if that **assay** analysis appears 0 or 2+ times among LimsRuns. **If the asked-for is Extracted DNA:** they get a **tube of DNA**. No sequencing. **No other LimsRuns.** Zero assay LimsRuns is legal (**1.4**). **If the asked-for is WGS / WES:** sequencing is the ask; extract is **route machinery** to the right matrix, not a second ask. Extract experiment first; QC LimsRun (Qubit/Nanodrop) may quantify DNA — **process QC is not an asked-for**; then the one assay LimsRun. **Do not put ELISA in that extract-to-DNA path.** A blood ELISA ask runs ELISA on the **blood** (or plasma/serum) tube, **not** on a DNA derivative. **No route branching this phase:** Route **blood** for WGS (extraction → sequencing); that asked-for **owns WGS params**. C3 mints DNA; a **C2 aliquot** of that DNA continues WGS. **WES is a new asked-for on the DNA tube** with **its own params**; that tube is then **aliquoted or used up**. Do not teach dest auto-joining a second WO. **Merge hold is 1.2 dest-cohort lookup** after C3 (not freeze skip).
...
The WO-7 lock puts Test selection/creation and the params freeze on the **first LimsRun start** for the asked-for analysis — **not** on receive, Route, or work-order Start. Freeze skip NULL is **Tobias Pass** on `bf51b19`. `{}` on `99b692d3` stays `8cfa2a9` history. `if test: continue` is **not** a freeze. Do **not** teach skip-on-`{}` on the old default. **OQ-WO-6 extract CLOSED** for the common path. Extract is typically a process. **Merge hold is 1.2 dest-cohort lookup**, not freeze skip. **Do not claim overall P2 Pass.**
...
- Do **not** treat first-start freeze skip as still OPEN. Freeze skip NULL is **Tobias Pass** on `bf51b19`. A write of `{}` onto a new Test on `8cfa2a9` (`99b692d3`) is history, not that Pass. Do **not** fold freeze skip as the merge hold — merge hold is **1.2 dest-cohort lookup**.
...
- Do **not** hang the asked-for panel `analysis_id` on extract or library prep — extract is typically an **experiment** (equipment). Do **not** forever-ban extract-as-LimsRun. **After extract** on an **assay** ask, Qubit / Nanodrop / other QC is a **LimsRun on the DNA** (instrument, own Test). **Process QC is not an asked-for.** Do not put QC on extract. Type gates are a different control (blood-on-Qubit). Extracted DNA ask: DNA tube only; zero assay LimsRuns legal.
