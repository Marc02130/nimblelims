...
**Named-slot product status (2026-09-03): not shipped yet.** OQ-WO-8 remains Closed history from PR 119, but the product still needs the persisted author-named asked-for LimsRun slot and 2+ route picker. Until that product slice lands and Tobias executes the unsigned `AC-P2-OQ-WO-8`, do not teach containment matching or the picker as verified behavior. The intended behavior is: match `asked.analysis_id` to the named slot, not any occurrence in the chain; 0 maps → 422; 1 → mint; 2+ → tech picks a route and mint occurs only after selection. Overall P2 remains unsigned.
...
3. Named-slot target: P2 keeps rows whose TAT matches, whose **first process’s first ordered Experiment/LimsRun** accepts the sample’s current type, and whose persisted **named asked-for LimsRun slot** has `analysis_id == asked.analysis_id`. Eligibility is not containment. A WGS map with Qubit only as process QC must not steal a Quantified DNA ask. Quantified DNA wears existing Qubit; the zero-LimsRun Extracted DNA clause is struck. This target is pending product code and unsigned Tobias UAT.
4. Target outcome after named-slot filtering: zero acceptable rows returns **422** and mints nothing; exactly one creates one queued `work_order`, snapshots the ordered list, changes asked-for to `routed`, and creates zero Tests; two or more show a route picker and mint nothing until the tech posts the selected `routing_map_id`. Never silently call `first()`. The 2+ picker supersedes older “409 only / no picker” teaching and is not yet shipped.
