...
6. Complete that process, then use **Later Start** for the next pending process. That start uses samples still assigned to the previous process. If aliquot/pool already minted a daughter or plated aliquot, **that product continues**; the inbound sample is no longer assigned. Each later process/step start compares the continuing sample’s **current** type with the allow-list at that start; empty or incompatible returns **422** `route_sample_type`. Later-step type-gate is **Met** on `9342439` (qPCR on still-Blood **422** when no mint had run). Mint itself is aliquot/pool OOB submit, not Route or Start.
