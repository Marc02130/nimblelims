...
| Later Route / work order (planner, not after receive) | Surfaces on this P2 branch. Live SHA `9342439` — **Tobias-signed AC-P2-9..11 Pass**. A route **may have multiple LimsRun analyses**; asked-for matches any chain that **contains** that analysis. Freeze skip **OPEN**. **OQ-WO-6 OPEN** (earlier LimsRun must not share asked-for `analysis_id`; extract is not a special assay — type gates catch blood-on-Qubit). Dest mint **Hold**. **No overall P2 Pass**. Hold product merge. |
...
Empty Route 0→**422** is **Tobias-signed Pass** on `8cfa2a9`. Live SHA is `9342439`: **Tobias-signed AC-P2-9..11 Pass**. A route **may have multiple LimsRun analyses**; asked-for ELISA matches extract→Qubit→ELISA because ELISA is **in** the chain. Freeze skip **OPEN**. **OQ-WO-6 OPEN** (earlier LimsRun must not reuse the asked-for `analysis_id`; extract is not special). Dest mint **Hold**. Do not write overall P2 Pass. Hold product merge.

**Leadership honesty locks (Deiter / Hans / Heidi / Günter) — restamp honesty on `9342439`, not a merge vote.** Tobias-signed AC-P2-9..11 Pass. Later-step type-gate **Met**. AC-P2-11 map-save only. Dest-type mint Hold. Freeze skip OPEN. **OQ-WO-6 OPEN** (earlier LimsRun `analysis_id`). Route stays `test:assign`. Overall P2 unsigned. Not IC50.
