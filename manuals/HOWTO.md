...
**WO-7 first-start freeze skip NULL is Tobias Pass on `bf51b19`.** Classic `/tests` leaves `asked_for_params` **NULL**. First LimsRun start **writes** the snapshot. Later start does **not** overwrite. Frozen `{}` is locked empty. `{}` on `99b692d3` stays `8cfa2a9` history of a new-Test write — do **not** re-score that SHA as skip Pass. Do **not** teach skip-on-`{}` on the old NOT NULL default. `if test: continue` is **not** a freeze. Older “freeze skip stays unsigned / OPEN” copy is **superseded** by `bf51b19`. **OQ-WO-7 OPEN** after C3 (WGS params on the DNA Test from the WO). **That click never landed.** **OQ-WO-6 extract CLOSED** for the common path: extract is an experiment (equipment) and does not wear the panel `analysis_id`. When the ask is an assay, asked-for analysis appears once on the assay LimsRun. Extracted DNA: zero assay LimsRuns legal. Do not claim overall P2 Pass.
...
The WO-7 lock puts Test selection/creation and the params freeze on the **first LimsRun start** for the asked-for analysis — **not** on receive, Route, or work-order Start. Freeze skip NULL is **Tobias Pass** on `bf51b19`. `{}` on `99b692d3` stays `8cfa2a9` history. `if test: continue` is **not** a freeze. Do **not** teach skip-on-`{}` on the old default. **OQ-WO-6 extract CLOSED** for the common path. Extract is typically a process. **OQ-WO-7 OPEN** (WGS params on the DNA Test from the WO after C3), not freeze skip. **That click never landed.** **Do not claim overall P2 Pass.**
...
- Do **not** treat first-start freeze skip as still OPEN. Freeze skip NULL is **Tobias Pass** on `bf51b19`. A write of `{}` onto a new Test on `8cfa2a9` (`99b692d3`) is history, not that Pass. Do **not** fold freeze skip as the leftover — **OQ-WO-7 OPEN** (WGS params on the DNA Test from the WO after C3). **That click never landed.**
