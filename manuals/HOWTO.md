...
| Later Route / work order (planner, not after receive) | Surfaces on this P2 branch. Signed AC-P2-9..11 history: `9342439`. Product SHA for `0077` assignment slices: `4671ba8` (0077 in that commit). AC-P2-C1/C2 **unsigned**. Round 2 **Leadership Confirm** (Rolf/Deiter/Hans/Heidi/Günter; R2-1…R2-4). A route **may have multiple LimsRun analyses**; asked-for matches any chain that **contains** that analysis. Freeze skip **OPEN**. **OQ-WO-6 stays OPEN** (Leadership Confirm: earlier LimsRun must not share asked-for `analysis_id`; do not teach extract-as-special). Dest mint **Hold**. Parser at import. **No overall P2 Pass**. Hold product merge. |
| Process / Experiment / LimsRun | Execute substrate shipped. Process assignment = the **tube in hand** (container-with-sample), not a naked sample; no vessel or two vessels with no pick → **422**, no silent pick. After an **equivalent aliquot** (same sample, new container), dest continues and the emptied inbound source is removed; **Later Start follows the dest container**, not the parent vessel and not a dest type. **Dest mint stays Hold** — `_execute_transfer` still inserts a new Sample with `dest_sample_type`; that is not the equivalent aliquot. Contents grain **Leadership Confirmed** (Rolf/Deiter/Hans/Heidi/Günter). Product `4671ba8` / `02fe95f` / `0077`; AC-P2-C1/C2 **unsigned**. Dest mint **Hold**: Start extract does **not** create a DNA daughter (tube still Blood). Freeze skip is **OPEN**: `{}` is ambiguous until classic `/tests` leaves NULL or a freeze marker exists. Later-step type-gate **Met** on `9342439` (qPCR-on-blood / still-Blood **422** `route_sample_type`; not dest-type E2E). Do not claim freeze closed |
...
**Punch:** Route does **not** start the chain. Start does **not** start the chain. Route snapshots the ordered list and mints **zero Tests**. **First Start instantiates `chain[0]` only.** Later Start = the next pending process on the continuing **dest** container-with-sample (after aliquot/pool execute), not the parent vessel.

Empty Route 0→**422** is **Tobias-signed Pass** on `8cfa2a9`. Signed AC-P2-9..11 history is `9342439`. Product SHA for `0077` assignment slices is `4671ba8` (0077 in that commit). AC-P2-C1/C2 **unsigned**. Round 2 **Leadership Confirm** (Rolf/Deiter/Hans/Heidi/Günter; R2-1…R2-4). A route **may have multiple LimsRun analyses**; asked-for ELISA matches extract→Qubit→ELISA because ELISA is **in** the chain. Freeze skip **OPEN**. **OQ-WO-6 stays OPEN** (earlier LimsRun must not reuse the asked-for `analysis_id`; do not teach extract-as-special). Dest mint **Hold**. Parser at import, not process authoring. Do not write overall P2 Pass. Hold product merge.

**Leadership honesty locks (Deiter / Hans / Heidi / Günter) — restamp honesty on `9342439`, not a merge vote.** Tobias-signed AC-P2-9..11 Pass. Later-step type-gate **Met**. AC-P2-11 map-save only. Dest-type mint Hold. Freeze skip OPEN. **OQ-WO-6 stays OPEN** (Leadership Confirm R2-3). Route stays `test:assign`. Overall P2 unsigned. Not IC50.
...
2. **OQ-WO-6 (earlier LimsRun `analysis_id`):** Extract is **not** a special assay. Type gates catch blood-on-Qubit. Blood→DNA is a derivative (dest mint Hold). Any **earlier** LimsRun that reuses the asked-for panel analysis mints/freezes that Test on the parent sample. Earlier LimsRun = own analysis, or experiment-only.
3. **Start:** First Start instantiates `chain[0]` only and assigns the receive **container-with-sample**. Later Start = next pending process on continuing dest assignments (source removed after execute). Do **not** teach later Start as “uses samples still assigned” as if source stayed.
...
6. Complete that process, then use **Later Start** for the next pending process. Process assignment is the **tube in hand** — a **container-with-sample** (Contents), not a naked sample. Assign with **no** vessel → **422** `process_container_required`; assign a sample sitting in **two** vessels with no `container_id` pick → **422** as well. The app never picks a vessel for you (no `first()`). After aliquot/pool **execute** (OOB, not Route/Start), **dest continues** the process and inbound **source is removed**. Later Start follows the **dest container**, not the parent vessel and **not** a dest **type**. Each later process/step start compares the continuing assignment’s **current** type with the allow-list at that start; empty or incompatible returns **422** `route_sample_type`. Later-step type-gate is **Met** on `9342439` (qPCR on still-Blood **422** when no mint had run). Mint itself is aliquot/pool OOB execute, not Route or Start.

**Equivalent aliquot vs dest mint.** An **equivalent aliquot** is the **same sample in a new container** — no new sample identity, no `sample_type` rewrite. That is what “dest continues the process” means here. **Dest mint** is a different motion and stays **Hold**: `_execute_transfer` still inserts a **new Sample** carrying `dest_sample_type` (which can be DNA) and joins that row to the process. Do **not** teach that mint as the equivalent aliquot, and do not claim equivalent-aliquot code is closed while `_execute_transfer` still mints a Sample. Extract-hold UAT step **1.7 is OOB execute**, not the P2 Contents click, and must not teach a DNA daughter. Contents grain is **Leadership Confirmed** (Rolf / Deiter / Hans / Heidi / Günter) at `4671ba8` / `02fe95f`; AC-P2-C1/C2 stay **unsigned** until Tobias clicks.
...
Qubit-first on blood is refused by Route before a work order is minted. If Qubit is later, its step start refuses while the sample’s current type is still blood. A **422** `route_sample_type` means current type is wrong for the assigned first step or the later step being started; it does **not** mean the sample is broken. Dest-type mint **Hold** stays: P2 extract / work-order Start still does **not** mint a DNA daughter. After Start first extract, tube is still Blood, 0 DNA daughters. Aliquot/pool execute mint (dest continues / source removed) is a different motion from dest-type Hold on the extract spine. Do **not** claim Start extract creates a DNA daughter. Do **not** claim dest-type Hold is closed.
...
- Do **not** teach routing-map save or Route as ANDing one type across later processes or steps. Route gates only the first process’s first ordered Experiment/LimsRun; later starts gate current type on the continuing **dest** container-with-sample.
...
- Do **not** teach first Start minting later processes (Qubit/reporting) or their Tests as shipped. First Start instantiates `chain[0]` only. Later Start = next pending process on the dest container-with-sample after execute, not the parent tube.
- Do **not** teach Route as starting work. Route snapshots the ordered list, **zero Tests**.
- Do **not** claim freeze closed/verified on `8cfa2a9`. Signed AC-P2-9..11 history stays `9342439`; Leadership restamp notes are honesty, **not** a merge vote. `0077` assignment slices live at `4671ba8`; AC-P2-C1/C2 **unsigned**. Overall P2 stays unsigned.
...
- Do **not** claim equivalent-aliquot code is closed while `_execute_transfer` still inserts a Sample. Coding stays **Grok Build**.
- Do **not** silently pick a vessel when a sample sits in two containers. That is **422**, not `first()`.
- Do **not** teach extract-hold UAT **1.7** as the P2 Contents click, and do not teach a DNA daughter there. 1.7 is **OOB execute**.
- Do **not** teach Start extract or dest-type Hold as closed blood→DNA daughter E2E.
