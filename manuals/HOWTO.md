...
**Punch:** Route does **not** start the chain. Start does **not** start the chain. Route snapshots the ordered list and mints **zero Tests**. **First Start instantiates `chain[0]` only.** Later Start following the dest container is the **lock on `1572071`**, **unsigned** until Tobias (C2/C3 numbered on `570bbc0`) — not a shipped Pass.
...
- Do **not** write C2 or C3 Pass. Deiter C2 **Fail** on `02fe95f` stands as signed history. C2 execute on `1572071` is **same-type dest-follow only** (`_follow_destination_in_process`; emptied-source assign **422**) and C3 is the different-dest-type click — numbered on `570bbc0`, neither is QA-clicked, neither is a shipped Pass. Do **not** teach `570bbc0` as a product execute SHA. Type-changing execute must not retarget the parent’s `container_id`. **PATCH is not a path.**
