...
**Aliquot/pool atomic pair (E-10):** One **+ Aliquot/pool** / **Add aliquot/pool** creates plan (`aliquot_pool_plan`) and dest (`aliquots_pools`) together. POST either side creates the pair. Delete or DELETE either half removes or inactivates both. After the pair is gone, Add re-enables (one pair at a time — not a lifetime lock). Dest stays empty until execute. UAT §7 **Fail** (Tobias QA, 2026-09-14 21:31 ET, `e5a8fdd`; packet 7.1–7.8 Pass; overall Fail = Deiter double-Add API uniqueness). Do **not** invent overall §7 Pass. **Tobias dogfood Ready=Yes** on `feat/e10-aliquot-atomic-pair` (2026-09-14 21:23 ET; `/workspace/dogfood-e10-e5a8fdd/READY.md`; SHA **`e5a8fdd`**). Paths **1–5 Pass**. Clean FE Docker/CRA green; no local patch. Ready=Yes is **not** a UAT Pass. **Rolf Confirm: No Hold.** Ready=Yes stands on **`e5a8fdd`**. Product owns API uniqueness before restamp. Prior Ready=No on **`9312c54`** (TS2345) stays history. **Deiter Lab Ops Confirm** of Marc’s fold (not a Pass). Do **not** restamp dest-container-type §§1–6 (`008baf2`). See [experiments.md](experiments.md).
