...
2. **OQ-WO-6 (extract is a process):** Extract is experiment / aliquot-pool **execute**, not a LimsRun, and has **no** asked-for `analysis_id`. Type gates catch blood-on-Qubit. Blood→DNA is a new derivative sample in a new container (`parent_sample_id`). The asked-for assay (ELISA, NGS, Qubit-as-asked-for, sequencing, …) appears on **exactly one** LimsRun. Qubit / Nanodrop are **supporting LimsRuns in the same route as whatever the asked-for assay is** (other `analysis_id`s, own Tests, own params freeze — not a second asked-for, not on extract). Map-save / Route **422** if asked-for analysis appears 0 or 2+ times among LimsRuns. **No route branching this phase:** Route **blood** for WGS (extraction → sequencing); that asked-for **owns WGS params**. C3 mints DNA; a **C2 aliquot** of that DNA continues WGS. **WES is a new asked-for on the DNA tube** with **its own params**; that tube is then **aliquoted or used up**. Do not teach dest auto-joining a second WO.
