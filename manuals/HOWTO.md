...
2. **OQ-WO-6 (extract is never a LimsRun):** Extract uses **equipment**, not instruments. It is an **experiment** (aliquot/pool execute) that may sit on a process. It is **never** a LimsRun and has **no** asked-for `analysis_id`. LimsRun is instruments / analysis / Tests. **After extract**, a **QC LimsRun** (Qubit, Nanodrop, or other QC instrument) quantifies the DNA — own Test, own params freeze. **Process QC is not an asked-for.** Do not invent a second asked-for for Qubit/Nanodrop. Do not put QC on extract. Type gates catch blood-on-Qubit. Blood→DNA is a new derivative sample in a new container (`parent_sample_id`). The asked-for assay (ELISA, NGS, sequencing, …) appears on **exactly one** LimsRun. Map-save / Route **422** if asked-for analysis appears 0 or 2+ times among LimsRuns. **No route branching this phase:** Route **blood** for WGS (extraction → sequencing); that asked-for **owns WGS params**. C3 mints DNA; a **C2 aliquot** of that DNA continues WGS. **WES is a new asked-for on the DNA tube** with **its own params**; that tube is then **aliquoted or used up**. Do not teach dest auto-joining a second WO.
