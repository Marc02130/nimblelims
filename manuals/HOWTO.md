...
3. **Start:** First Start instantiates `chain[0]` only and assigns the receive **container-with-sample**. The intended Later Start follows a continuing dest assignment, but Deiter’s C2 click failed: execute minted dest and did not join dest or remove source. Do **not** teach dest-follows as shipped.
...
6. Complete that process, then use **Later Start** only where the product has a valid continuing assignment. Process assignment is the **tube in hand** — a **container-with-sample** (Contents), not a naked sample. Assign with **no** vessel → **422** `process_container_required`; assign a sample sitting in **two** vessels with no `container_id` pick → **422** as well; receive-tube assign → **201**. Deiter’s C1 click **Pass** verifies those outcomes. Deiter’s C2 click **Fail** means the equivalent-aliquot follow is not a shipped path: execute **mints dest**, does **not** join dest or remove source; emptied-source assign is a **201 mix-up**; **PATCH is not a path**. Do not teach PATCH or dest-follows as the remedy. Later-step type-gate remains **Met** on `9342439` (qPCR on still-Blood **422** when no mint had run).

**Equivalent aliquot vs dest mint.** The target equivalent aliquot is the **same sample in a new container** — no new sample identity, no `sample_type` rewrite — but Deiter’s C2 click on `4671ba8` / `02fe95f` **failed**. Execute minted dest and did not join dest or remove source. Do **not** teach dest-follows as shipped, and do **not** teach PATCH as its path. The separate dest mint Hold **passed**: Start extract leaves the tube **Blood**, with **0 DNA** daughters. That Hold Pass does not close `_execute_transfer`; C2 records that execute still mints dest. Extract-hold UAT step **1.7 is OOB execute** and must not teach a DNA daughter.
...
- Do **not** claim freeze closed/verified on `8cfa2a9`. Signed AC-P2-9..11 history stays `9342439`; Leadership restamp notes are honesty, **not** a merge vote. Deiter clicked `0077` on `4671ba8` / `02fe95f`: C1 **Pass**, C2 **Fail**, dest mint Hold **Pass**. Overall P2 stays unsigned.
...
- Do **not** claim equivalent-aliquot code is closed. Deiter C2 **Fail** records that `_execute_transfer` mints dest, does not join dest or remove source, and emptied-source assign is a 201 mix-up.
