...
**Current P2 QA status (2026-09-03):** The older “overall P2 unsigned” text in historical rows records the state of those earlier SHAs. Tobias has now stamped **overall P2 Pass (QA)** on `6244bf6`, folding `bf51b19` + `80f054b` + `6244bf6`. **Leadership overall Pass remains unsigned / not Pass.**
...
**Named-slot product status (2026-09-03): landed and QA-stamped on `feat/p2-named-slot` @ `6244bf6` (Alembic `0079`).** **AC-P2-OQ-WO-8 Pass** (Tobias): `routing_map.asked_for_step_id` is the author-named LimsRun slot; Route compares `asked.analysis_id` with that slot, not chain containment. One named Qubit match mints; zero returns **422**; 2+ returns **409** `route_pick_required`, shows the picker, and mints only after selection. WGS+Qubit-as-process-QC does not steal Quantified DNA. **Tobias overall P2 Pass (QA)** is also stamped on `6244bf6`, folding `bf51b19` + `80f054b` + `6244bf6`. **Leadership overall Pass remains unsigned / not Pass.** This branch status is not a claim that the named-slot product has merged to `main`.
