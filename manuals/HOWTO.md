...
6. Complete that process, then use **Later Start** for the next pending process. Process assignment is a **container-with-sample** (Contents), not a naked sample. After aliquot/pool **execute** (OOB, not Route/Start), **dest continues** the process and inbound **source is removed**. Later Start follows the **dest** container-with-sample, not the parent vessel. Assign without a container → **422** `process_container_required`. Each later process/step start compares the continuing assignment’s **current** type with the allow-list at that start; empty or incompatible returns **422** `route_sample_type`. Later-step type-gate is **Met** on `9342439` (qPCR on still-Blood **422** when no mint had run). Mint itself is aliquot/pool OOB execute, not Route or Start.
...
- Do **not** teach later Start as following the parent/source tube after dest mint, or as legal assign of a sample with no container.
