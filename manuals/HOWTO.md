...
| Later Route / work order (planner, not after receive) | Surfaces on this P2 branch. Live product SHA **`570bbc0`**. Signed AC-P2-9..11 history: `9342439`. Deiter Contents click on product `4671ba8` / assignment commit `02fe95f` (`0077`) is signed history: C1 **Pass**, C2 **Fail**, dest mint Hold **Pass**. Leadership Confirmed that click; Wilhelmina folded C2 Fail into sketch `a3741d1` and requirements `60a9447`. C1/C2 are **not** unsigned. Docs Confirm `84d2810` is not a new execute or the click SHA. Live AC-P2-C2 (same dest type) and AC-P2-C3 (different dest type) on `570bbc0` are **unsigned** until Tobias — two clicks (dest-follow execute txn, emptied-source **422**, same-sample dest container; in code since `1572071`, not QA-clicked; do not teach dest-follow as shipped). Freeze skip **OPEN**. **OQ-WO-6 stays OPEN**. **No overall P2 Pass**. Hold product merge. |
| Process / Experiment / LimsRun | Execute substrate shipped. Process assignment = the **tube in hand**; Deiter C1 **Pass** on `02fe95f` stays history. Deiter C2 **Fail** on `02fe95f` is signed history (Leadership restamp): dest not on the process; emptied-source **201**; later Start rode the emptied parent; **PATCH is not a path**. Live C2 lock on `570bbc0` is same-type dest-follow only: same dest type = **same sample, additional container**; `_follow_destination_in_process` follows that path; emptied-source assign **422**; later Start follows that dest container. **Unsigned** until Tobias. Live **C3** on `570bbc0` is the different-dest-type click, also **unsigned**: **new derivative sample** in a new container (`parent_sample_id`). The parent **Sample row** stays as history with its original type and parent-type work; its process assignment does not stay. For both grains, execute writes the destination sample + destination container pair to `eln_process_samples` and marks the inbound source assignment `removed` in the same transaction. Dest mint Hold is lifted only for type-changing execute. Deiter Hold **Pass** on `02fe95f` remains Start-extract Blood / **0 DNA** history. Freeze skip is **OPEN**. Later-step type-gate **Met** on `9342439`. Do not claim freeze closed |
...
3. **Start:** First Start instantiates `chain[0]` only and assigns the receive **container-with-sample**. Later Start following dest is the lock on **`570bbc0`** (in code since `1572071`; not QA-clicked). Deiter’s C2 **Fail** on `02fe95f` is signed history. Do **not** teach dest-follow as shipped. Do **not** write C2 or C3 Pass.
...
- Do **not** teach routing-map save or Route as ANDing one type across later processes or steps. Route gates only the first process’s first ordered Experiment/LimsRun; later starts gate current type. Later Start following dest container is the lock on **`570bbc0`**, **unsigned** until Tobias.
...
- Do **not** write C2 or C3 Pass. Deiter C2 **Fail** on `02fe95f` stands as signed history. C2 on `570bbc0` is **same-type dest-follow only** (`_follow_destination_in_process`; emptied-source assign **422**) and C3 is the different-dest-type click — neither is QA-clicked, neither is a shipped Pass. Type-changing execute must not retarget the parent’s `container_id`. **PATCH is not a path.**
