...
2. **OQ-WO-6 (extract does not wear the panel `analysis_id`):** Extract uses **equipment**, not instruments, on the common path. It is an **experiment** (aliquot/pool execute) that may sit on a process. Do **not** forever-ban extract-as-LimsRun if that equipment later is an instrument. On the common path extract has **no** asked-for panel `analysis_id`. LimsRun is instruments / analysis / Tests. **After extract** on an **assay** ask, a **QC LimsRun** (Qubit, Nanodrop, or other QC instrument) may quantify the DNA — own Test, own params freeze. **Process QC is not an asked-for.** Do not invent a second asked-for for Qubit/Nanodrop. Do not put QC on extract. Type gates catch blood-on-Qubit. Blood→DNA is a new derivative sample in a new container (`parent_sample_id`). When the ask **is** an assay (ELISA, NGS, sequencing, …) it appears on **exactly one** LimsRun. Map-save / Route **422** if that **assay** analysis appears 0 or 2+ times among LimsRuns. **If the asked-for is Extracted DNA:** they get a **tube of DNA**. No sequencing. **No other LimsRuns.** Zero assay LimsRuns is legal (**1.4**). **If the asked-for is WGS / WES / ELISA:** sequencing/ELISA is the ask; extract is **route machinery** to the right matrix, not a second ask. Extract experiment first; QC LimsRun (Qubit/Nanodrop) may quantify DNA — **process QC is not an asked-for**; then the one assay LimsRun. **No route branching this phase:** Route **blood** for WGS (extraction → sequencing); that asked-for **owns WGS params**. C3 mints DNA; a **C2 aliquot** of that DNA continues WGS. **WES is a new asked-for on the DNA tube** with **its own params**; that tube is then **aliquoted or used up**. Do not teach dest auto-joining a second WO. **Merge hold is 1.2 dest-cohort lookup** after C3 (not freeze skip).
