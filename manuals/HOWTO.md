...
| Later Route / work order (planner, not after receive) | Surfaces on this P2 branch (`8cfa2a9`). Route snapshots the ordered list, **zero Tests**. First Start = `chain[0]` only — Tobias-signed **Pass** (ELISA first LimsRun, 1 step, no Qubit/qPCR Tests). Empty Route **422** Pass. Live AC-P2 **per-AC signed; overall P2 unsigned**. Hold product merge |
| Process / Experiment / LimsRun | Execute substrate shipped. Later Start = next pending process, on the sample that exists then. New-Test freeze write is **Pass** on Test `99b692d3`; classic `/tests` skip-on-`{}` is **OPEN**. Later-step type-gate **unsigned**. Do not claim freeze closed |
...
Empty Route 0→**422** is **Tobias-signed Pass** on `8cfa2a9` (“No routing-map row accepts this analysis, TAT, and sample type”). Map overlap **409** and blood+qPCR map-save **201** (not AND) are **Pass**. Route two-accept **409** is **unsigned**. First-start freeze is **Pass** only for the **new-Test** write on `99b692d3`; classic `/tests` skip-on-`{}` is **OPEN**. Later-start type gates are **unsigned**. Do not write overall P2 Pass or freeze-closed.

**Leadership honesty locks (Hans / Heidi / Günter) — live AC-P2 per-AC signed on `8cfa2a9`; overall P2 unsigned. Hold product merge. Dest-type Hold out. Not IC50.**
...
3. P2 finds rows whose analysis + TAT match, then keeps only rows whose **first process’s first ordered Experiment/LimsRun** accepts the sample’s current type. **Tobias-signed Pass on `8cfa2a9`:** zero acceptable rows returns **422** “No routing-map row accepts this analysis, TAT, and sample type” and mints nothing. Route two-accept **409** is **unsigned** this SHA. Never silently call `first()`.
...
6. Complete that process, then use **Later Start** for the next pending process, on the sample that exists then. Each later process/step start compares the sample’s **current** type with the allow-list at that start; empty or incompatible returns **422** `route_sample_type`. **Later-step type-gate at start (current tube) was not click-run on `8cfa2a9` — leave unsigned, not Pass.** The sample is not broken. Dest-type Hold is out: do not claim extract already minted a daughter unless the product did.

**WO-7 first-start freeze is not closed.** Tobias-signed **Pass** on `8cfa2a9` is the **new-Test write** only: first start **wrote** `asked_for_params` `{}` onto **new** Test `99b692d3` (not SQL NULL, not a skipped classic row); later start did not overwrite that path. Do **not** fold classic `/tests` skip as Pass. Do **not** claim `{}` vs frozen `{}` is distinguishable. `if test: continue` is **not** a freeze. Classic `/tests` must leave `asked_for_params` **NULL**; a row with NULL or default `{}` is **not** frozen. **Skip-on-`{}` is not a freeze**: classic default `{}` and frozen `{}` are the same JSON. Skip is honest only once classic `/tests` leaves `asked_for_params` NULL or a **freeze marker** lands. Extract LimsRun must **not** share the asked-for `analysis_id` or it attaches/freezes the panel Test at extract start. Do not claim overall P2 Pass.
...
The WO-7 lock puts Test selection/creation and the params freeze on the **first LimsRun start** for the asked-for analysis — **not** on receive, Route, or work-order Start. New-Test write is **Tobias-signed Pass** on `8cfa2a9` (Test `99b692d3`); classic `/tests` skip-on-`{}` is **OPEN**. `if test: continue` is **not** a freeze, and neither is skip-on-`{}` — classic default `{}` and frozen `{}` are the same JSON. Classic `/tests` must leave `asked_for_params` **NULL**. Skip is honest only once classic `/tests` leaves NULL or a freeze marker lands. Extract LimsRun must not share the asked-for `analysis_id` or it attaches/freezes the panel Test at extract start. **Do not claim freeze closed.**
...
- Do **not** treat first-start freeze as closed. New-Test write on `99b692d3` is Pass; classic `/tests` skip-on-`{}` is OPEN. Empty Route 422 and map overlap 409 are Pass; later-step type-gate and Route two-accept 409 are unsigned until clicked.
