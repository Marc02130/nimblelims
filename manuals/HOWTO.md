...
Empty Route 0→**422** is **Tobias-signed Pass** on `8cfa2a9`. Routing-map Pass on that SHA is **click-save in the UI**: no sample-type picker; ELISA TAT 1–7 saved; Blood extract + later DNA qPCR chain saved (no AND 422); second ELISA overlap **409**. Live SHA is `9342439`: **Tobias-signed AC-P2-9..11 Pass**. **Leadership Confirm** of analysis-in-chain / overlap 409 / handoff 422 (Rolf/Deiter/Hans/Heidi/Günter). Deiter / Hans / Heidi / Günter restamp notes (honesty, **not** a merge vote): later-step type-gate **Met** (qPCR-on-blood / still-Blood **422** `route_sample_type`; not dest-type E2E); AC-P2-11 / handoff Pass is **map-save only**; dest mint **Hold** (no execute rewrite of `sample_type`; tube still Blood; 0 DNA daughters); freeze skip **OPEN**; extract `analysis_id` **OPEN**; Route stays `test:assign`. Do not write overall P2 Pass. Hold product merge.

**Leadership honesty locks (Deiter / Hans / Heidi / Günter) — restamp honesty on `9342439`, not a merge vote.** Tobias-signed AC-P2-9..11 Pass. Later-step type-gate **Met**. AC-P2-11 map-save only. Dest-type mint Hold. Freeze skip OPEN. Extract `analysis_id` OPEN. Route stays `test:assign`. Overall P2 unsigned. Not IC50.
...
6. Complete that process, then use **Later Start** for the next pending process, on the sample that exists then. Each later process/step start compares the sample’s **current** type with the allow-list at that start; empty or incompatible returns **422** `route_sample_type`. Later-step type-gate is **Met** on `9342439` (Tobias Pass; Deiter / Hans / Heidi / Günter): later qPCR start on still-Blood **422** `route_sample_type` (current type vs that step; sample not dead). That click is **not** dest-type E2E. Dest-type Hold is out.
...
- Do **not** claim freeze closed/verified on `8cfa2a9`. Live SHA `9342439` has Tobias-signed AC-P2-9..11 Pass; Leadership restamp notes are honesty, **not** a merge vote. Overall P2 stays unsigned.
