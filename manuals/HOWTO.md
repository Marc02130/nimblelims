...
**Aliquot/pool atomic pair (E-10):** One **+ Aliquot/pool** / **Add aliquot/pool** creates plan (`aliquot_pool_plan`) and dest (`aliquots_pools`) together. POST either side creates the pair. Delete or DELETE either half removes or inactivates both. After the pair is gone, Add re-enables (one pair at a time — not a lifetime lock). Dest stays empty until execute. UAT §7 **Unsigned**. **Tobias dogfood Ready=No** (2026-09-14 20:26 ET; `/workspace/dogfood-e10-9312c54/READY.md` — paths 1–5 Pass; clean-image TS2345 `hasAliquotPair`). **Rolf Hold §7** until that fix is on the tip. **Deiter Lab Ops Confirm** of Marc’s fold (not a Pass). Do **not** invent Ready=Yes or §7 Pass. Do **not** restamp dest-container-type §§1–6 (`008baf2`). See [experiments.md](experiments.md).
