...
| Later Route / work order (planner, not after receive) | Surfaces on this P2 branch. Signed AC-P2-9..11 history: `9342439`. Deiter Contents click on product `4671ba8` / assignment commit `02fe95f` (`0077`): C1 **Pass**, C2 **Fail**, dest mint Hold **Pass**. Leadership Confirmed that click; C1/C2 are **not** unsigned. Docs Confirm `84d2810` is not a new execute or the click SHA. **Grok Build** owns dest-join / source-remove. Freeze skip **OPEN**. **OQ-WO-6 stays OPEN**. **No overall P2 Pass**. Hold product merge. |
| Process / Experiment / LimsRun | Execute substrate shipped. Process assignment = the **tube in hand**; Deiter C1 **Pass**. C2 **Fail** (Leadership restamp on `02fe95f`): dest not on the process / never lands on `eln_process_samples`; emptied source still assignable (**201**) is leftover amount-0 Contents / leftover process-join; later Start via `_continuing_assignments` rides the emptied parent; results would not be attributable to the dest vessel; **PATCH is not a path**. Dest mint Hold **Pass** is a different punch (still Blood, **0 DNA**). Coding stays **Grok Build**. Freeze skip is **OPEN**. Later-step type-gate **Met** on `9342439`. Do not claim freeze closed |
...
**Equivalent aliquot vs dest mint.** The target equivalent aliquot is the **same sample in a new container** — no new sample identity, no `sample_type` rewrite — but Deiter’s C2 click on `4671ba8` / `02fe95f` **failed**, and Leadership Confirmed that Fail. Execute never writes the same-sample dest container onto `eln_process_samples`; join/release no-op without `entry.process_step_id`, so later Start via `_continuing_assignments` rides the emptied parent. Emptied-source **201** is leftover amount-0 Contents, not dest-follow — the wrong tube stays on the instance. Dest never lands on `eln_process_samples`. Results would not be attributable to the dest vessel. **PATCH is not a path.** Dest mint Hold is a different punch (new Sample with `dest_sample_type` is not this fix; Start extract still Blood, **0 DNA**). Coding stays **Grok Build** — dest-join / source-remove. Extract-hold UAT step **1.7 is OOB execute** and must not teach a DNA daughter.
...
- Do **not** claim freeze closed/verified on `8cfa2a9`. Signed AC-P2-9..11 history stays `9342439`; Leadership restamp notes are honesty, **not** a merge vote. Deiter clicked `0077` on `4671ba8` / `02fe95f`: C1 **Pass**, C2 **Fail**, dest mint Hold **Pass**. Leadership Confirmed that click; C1/C2 are **not** unsigned. Overall P2 stays unsigned.
