...
**Current P2 status (2026-09-04 UI dogfood):** Named-slot product is on `main` (merge **`6625490`**, PR **122**, product **`6a67667`**, Alembic **`0079`**). **Tobias UI dogfood Pass** 2026-09-04 21:25 ET on `main` tip **`be65d1e`** (alice-tech picker; not the API-only merge-gate restamp). **AC-P2-OQ-WO-8 Pass** (Tobias, 2026-09-03, `6244bf6`) stays signed history. **Merge-gate restamp Pass** (Tobias QA, 2026-09-04, click SHA **`effd242`**, product `6a67667`) stays history — API-only is not this UI Pass. **Tobias overall P2 Pass (QA)** 2026-09-03 21:26 ET on `6244bf6`, folding `bf51b19` + `80f054b` + `6244bf6`. **Full Leadership Confirm overall P2 Pass** (Rolf / Deiter / Hans / Heidi / Günter) on that same SHA; this UI dogfood does not rewrite it. Older “overall P2 unsigned” text in historical table rows records those earlier SHAs. Older “named-slot has not merged” copy is superseded for living product state.

Handbooks in this folder: [atomic-receive.md](atomic-receive.md), [asked-for.md](asked-for.md), [navigation.md](navigation.md), [api-endpoints.md](api-endpoints.md), [dev-setup.md](dev-setup.md), [admin-setup.md](admin-setup.md), [processes.md](processes.md), [experiments.md](experiments.md), [lims-runs.md](lims-runs.md), [ui-schema.md](ui-schema.md). Index: [README.md](README.md).  
...
**Named-slot product status (2026-09-04 UI dogfood): on `main` at merge `6625490` (PR 122), product `6a67667` (Alembic `0079`).** Containment eligibility is not current `main` behavior. **Tobias UI dogfood Pass** 2026-09-04 21:25 ET on `main` tip **`be65d1e`** (alice-tech row Route). UI dogfood **Met**. The merge-gate **restamp** **AC-P2-OQ-WO-8 Pass** (Tobias QA, 2026-09-04 20:18 ET, click SHA **`effd242`**, product `6a67667`) stays history — API-only is not this UI Pass — and is not a rewrite of the `6244bf6` stamp. **AC-P2-OQ-WO-8 Pass** (Tobias): `routing_map.asked_for_step_id` is the author-named LimsRun slot; Route compares `asked.analysis_id` with that slot, not chain containment. One named Qubit match mints; zero returns **422** (UI refuse toast on this click); 2+ returns **409** `route_pick_required`, shows the picker, and mints only after selection. WGS+Qubit-as-process-QC does not steal Quantified DNA. **Tobias overall P2 Pass (QA)** 2026-09-03 21:26 ET on `6244bf6`, folding `bf51b19` + `80f054b` + `6244bf6`. **Full Leadership Confirm overall P2 Pass** (Rolf / Deiter / Hans / Heidi / Günter, 2026-09-03) on the same SHA, folding that Tobias overall. Distinct stamps. Ops: admin map grid has no Active toggle (Delete only) — known limitation, not a Fail of named-slot.
...
2. **OQ-WO-6 (extract does not wear the panel `analysis_id`):** Extract uses **equipment**, not instruments, on the common path. It is an **experiment** (aliquot/pool execute) that may sit on a process. Do **not** forever-ban extract-as-LimsRun if that equipment later is an instrument. On the common path extract has **no** asked-for panel `analysis_id`. LimsRun is instruments / analysis / Tests. **After extract** on a **WGS / WES** ask, a **QC LimsRun** (Qubit, Nanodrop, or other QC instrument) may quantify the DNA — own Test, own params freeze. **Process QC is not an asked-for.** Do not invent a second asked-for for Qubit/Nanodrop. Do not put QC on extract. Type gates catch blood-on-Qubit. Blood→DNA is a new derivative sample in a new container (`parent_sample_id`). When the ask **is** an assay (ELISA, NGS, sequencing, …) it appears on **exactly one** LimsRun. Map-save / Route **422** if that **assay** analysis appears 0 or 2+ times among LimsRuns. **If the asked-for is Quantified DNA (CEO Confirm 1–6 Rolf):** they get **concentration data**. Test `(DNA, Qubit)` is the ask. Wear existing Qubit. Old 1.4 (zero LimsRuns) is **struck**; map-save / Route **422** on 0 LimsRuns is right. **Nanodrop may sit** as supporting QC (own analysis_id; not a second asked-for). Tube-only DNA is a later SKU. Extract-as-instrument LimsRun is **later**. Do **not** teach a forever ban. Named-slot is Closed (eligibility vs named asked-for LimsRun slot, not contains-Qubit). **OQ-WO-8 Closed.** **If the asked-for is WGS / WES:** sequencing is the ask; extract is **route machinery** to the right matrix, not a second ask. Extract experiment first; QC LimsRun (Qubit/Nanodrop) may quantify DNA — **process QC is not an asked-for**; then the one assay LimsRun. **Do not put ELISA in that extract-to-DNA path.** A blood ELISA ask runs ELISA on the **blood** (or plasma/serum) tube, **not** on a DNA derivative. **No route branching this phase:** Route **blood** for WGS (extraction → sequencing); that asked-for **owns WGS params**. C3 mints DNA; a **C2 aliquot** of that DNA continues WGS. **WES is a new asked-for on the DNA tube** with **its own params**; that tube is then **aliquoted or used up**. Do not teach dest auto-joining a second WO. **OQ-WO-7 Closed** after C3 (WGS params on the DNA Test from the WO; not freeze skip). **AC-P2-OQ-WO-7 Pass** (Tobias, 2026-09-01, `80f054b`). After C3, Test `(DNA, WGS)` freezes `{library_kit: TruSeq}` from the blood WO, **not** `{}`. Leftover **`9f86d14`** not recoded. Closeout **1.4 / OQ-WO-8 Closed** (**Full Leadership Confirm**; **CEO Confirm 1–6 stands**; named asked-for LimsRun slot Closed — not contains-Qubit).
...
**Setup:** an administrator configures **Routing map** at `/admin/routing-map`. Each row is TAT plus an ordered `process_definition[]`. There is **no admin-picked analysis or sample type**. A route **may list several LimsRun analyses**. Assignment when the ask is an **assay:** first process’s first Experiment/LimsRun allow-list **and** the asked-for analysis on **exactly one** LimsRun in that chain (assay step). **Quantified DNA ask (CEO Confirm 1–6 Rolf):** Qubit once; wear existing Qubit; **422** on 0 LimsRuns. Extract is typically an experiment (equipment) and is not counted; it **may** be a LimsRun later. Named asked-for LimsRun slot Closed (**OQ-WO-8 Closed**): eligibility is `asked.analysis_id` vs that slot, not contains-Qubit. Process QC (Qubit/Nanodrop) is a supporting LimsRun, **not** an asked-for. Parser is **not** on the map or the process step — chosen at import (instrument XOR CRO). Map save does **not** AND inbound type across later processes. It **does** 422 when the type emerging from process *x* is not accepted by process *x+1* (authoring, not a runnable extract→Qubit), **and** 422 if an **assay** asked-for analysis appears 0 or 2+ times among LimsRuns.
...
3. Named-slot behavior on `main` (product `6a67667` / merge `6625490`; stamped on `6244bf6`, restamped on `effd242`, UI dogfood Pass on `be65d1e`): P2 keeps rows whose TAT matches, whose **first process’s first ordered Experiment/LimsRun** accepts the sample’s current type, and whose persisted `routing_map.asked_for_step_id` slot has `analysis_id == asked.analysis_id`. Eligibility is not containment. A WGS map with Qubit only as process QC does not steal a Quantified DNA ask. Quantified DNA wears existing Qubit; the zero-LimsRun Extracted DNA clause is struck.
4. After named-slot filtering: zero acceptable rows returns **422** and mints nothing (UI toast refuse on the `be65d1e` click); exactly one creates one queued `work_order`, snapshots the ordered list, changes asked-for to `routed`, and creates zero Tests; two or more return **409** `route_pick_required`, show a route picker, and mint nothing until the tech posts the selected `routing_map_id`. Never silently call `first()`. Tobias stamped these outcomes **Pass** on `6244bf6` and restamped them **Pass** on `effd242` (product `6a67667`) for the merge gate (API history). **Tobias UI dogfood Pass** on `be65d1e` is the alice-tech picker click.
...
**WO-7 first-start freeze skip NULL is Tobias Pass on `bf51b19`.** Classic `/tests` leaves `asked_for_params` **NULL**. First LimsRun start **writes** the snapshot. Later start does **not** overwrite. Frozen `{}` is locked empty. `{}` on `99b692d3` stays `8cfa2a9` history of a new-Test write — do **not** re-score that SHA as skip Pass. Do **not** teach skip-on-`{}` on the old NOT NULL default. `if test: continue` is **not** a freeze. Older “freeze skip stays unsigned / OPEN” copy is **superseded** by `bf51b19`. **OQ-WO-7 Closed** after C3 (WGS params on the DNA Test from the WO). **AC-P2-OQ-WO-7 Pass** (Tobias, 2026-09-01, `80f054b`): Test **`55f9cad9`** `(DNA, WGS)` `{library_kit: TruSeq}` from WO **`4ea9de0c`**. Leftover **`9f86d14`** not recoded. After C3, do **not** teach `{}` on DNA WGS. **OQ-WO-6 extract CLOSED** for the common path: extract is an experiment (equipment) and does not wear the panel `analysis_id`. When the ask is an assay, asked-for analysis appears once on the assay LimsRun. Quantified DNA: Qubit once (**CEO Confirm 1–6 stands**); old 1.4 struck. **OQ-WO-8 Closed.** Named asked-for LimsRun slot Closed (not contains-Qubit). Do not claim overall P2 Pass.
...
**Aliquot/pool wrapper (E-10):** `WRAPPER_CATALOG` `aliquot_pool` (cardinality **1**). One **+ Aliquot/pool** / **Add aliquot/pool** creates plan (`aliquot_pool_plan`) and dest (`aliquots_pools`) together. POST either side creates the pair. Second instance → **409** `wrapper_at_capacity`. Delete or DELETE either half removes or inactivates both. After the pair is gone, Add re-enables (one wrapper instance — not a lifetime lock). Dest stays empty until execute. UAT §7 **Pass** (Tobias QA, 2026-09-14 22:28 ET, `dc7ee92`; **7.1–7.8 Pass** on `e5a8fdd` not rescored + Deiter **double-Add Pass** on `dc7ee92`). Prior overall **Fail** on `e5a8fdd` is **history**. **Lab Ops: Deiter Met** on double-Add 2026-09-14. **Rolf Confirm: Hold merge lifted**. **E-10 Met**. Product on `main` @ `e56a89f` (PR **129**). **Tobias dogfood Ready=Yes** on **`dc7ee92`** (`/workspace/dogfood-e10-dc7ee92/READY.md`). Ready=Yes on **`e5a8fdd`** and Ready=No on **`9312c54`** stay history. Do **not** restamp dest-container-type §§1–6 (`008baf2`). See [experiments.md](experiments.md).
...
The WO-7 lock puts Test selection/creation and the params freeze on the **first LimsRun start** for the asked-for analysis — **not** on receive, Route, or work-order Start. Freeze skip NULL is **Tobias Pass** on `bf51b19`. `{}` on `99b692d3` stays `8cfa2a9` history. `if test: continue` is **not** a freeze. Do **not** teach skip-on-`{}` on the old default. **OQ-WO-6 extract CLOSED** for the common path. Extract is typically a process. **OQ-WO-7 Closed** (WGS params on the DNA Test from the WO after C3), not freeze skip. **AC-P2-OQ-WO-7 Pass** (Tobias, 2026-09-01, `80f054b`). After C3, DNA WGS freezes `{library_kit: TruSeq}` from the blood WO, **not** `{}`. Leftover **`9f86d14`** not recoded. Closeout **1.4 / OQ-WO-8 Closed** (**Full Leadership Confirm**; **CEO Confirm 1–6 stands**; named asked-for LimsRun slot Closed — not contains-Qubit). **Do not claim overall P2 Pass.**
...
- Do **not** treat first-start freeze skip as still OPEN. Freeze skip NULL is **Tobias Pass** on `bf51b19`. A write of `{}` onto a new Test on `8cfa2a9` (`99b692d3`) is history, not that Pass. Do **not** fold freeze skip as the leftover — **OQ-WO-7 Closed** (WGS params on the DNA Test from the WO after C3). **AC-P2-OQ-WO-7 Pass** (Tobias, 2026-09-01, `80f054b`). After C3, do **not** teach `{}` on DNA WGS. Leftover **`9f86d14`** not recoded. Closeout **1.4 / OQ-WO-8 Closed** (**Full Leadership Confirm**; **CEO Confirm 1–6 stands**; named asked-for LimsRun slot Closed — not contains-Qubit).
...
- Do **not** hang the asked-for panel `analysis_id` on extract or library prep — extract is typically an **experiment** (equipment). Do **not** forever-ban extract-as-LimsRun. **After extract** on a **WGS / WES** ask, Qubit / Nanodrop / other QC is a **LimsRun on the DNA** (instrument, own Test). **Process QC is not an asked-for.** Do not put QC on extract. Type gates are a different control (blood-on-Qubit). **Do not hang ELISA on the DNA dest after C3.** **Quantified DNA ask (1.4 / OQ-WO-8 Closed; CEO Confirm 1–6 stands):** wear existing Qubit as the asked-for LimsRun; do **not** mint a second catalog analysis named Quantified DNA; other process QC supporting. Old 1.4 struck; **422** on 0 LimsRuns. Named asked-for LimsRun slot Closed (not contains-Qubit). Do **not** code extract-only zero LimsRuns.
...
- Do **not** teach plan-only or dest-only aliquot/pool add from presets. Do **not** teach deleting one half and leaving the other. Do **not** treat Add staying disabled after the pair is gone as the lock (one wrapper instance). Do **not** teach a second aliquot/pool pair while one exists (**409** `wrapper_at_capacity`). E-10 UAT §7 is **Pass** (Tobias QA, 2026-09-14 22:28 ET, `dc7ee92`; **7.1–7.8 Pass** on `e5a8fdd` + Deiter **double-Add Pass** on `dc7ee92`). Prior overall **Fail** on `e5a8fdd` is **history**. Ready=Yes on **`dc7ee92`**. **Rolf Confirm: Hold merge lifted**. **E-10 Met**. Product on `main` @ `e56a89f` (PR **129**). Do **not** restamp dest-container-type §§1–6 (`008baf2`).
