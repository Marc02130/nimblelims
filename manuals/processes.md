...
**Equivalent aliquot vs dest mint.** The target equivalent aliquot is the **same sample in a new container**: no new sample identity, no `sample_type` rewrite. It is **not shipped as a verified follow path**: Deiter C2 **Fail** records that execute minted dest and did not join dest or remove source. Do not teach PATCH as the path. The distinct dest mint Hold **Pass** records Start extract still **Blood**, with **0 DNA** daughters. It does not close `_execute_transfer` dest mint. Extract-hold UAT step **1.7** is **OOB entry execute**, not the P2 Contents click, and must not teach a DNA daughter.
