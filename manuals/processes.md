...
**Asked-for does not start a process.** Recording **requested analysis** does not execute or mint a Test. A routing-map row authors TAT + ordered `process_definition[]`, with no analysis or sample-type picker. A route **may have multiple LimsRun analyses**. Map save **409**s when overlapping TAT, overlapping first-step allow-lists, **and** overlapping LIMS Run analysis **sets** all hold; extract-first and Qubit-first for the same TAT are legal. Route assigns when current type is on the first process’s first Experiment/LimsRun list **and** the asked-for analysis is **contained** in the route: zero acceptable → **422**, two saved rows that both accept this type and this analysis → **409**, exactly one → queued work order. Never silently use `first()`. Map save **422**s when the type emerging from process *x* is not accepted by process *x+1*. **Start process** requires `experiment:manage` and instantiates only the first process. A process assignment is a **container that holds a sample** (a sample may have many containers; only one container-with-sample is in the process; assign without a container → **422** `process_container_required`). After aliquot/pool execute, **every dest continues** and **every inbound source is removed**. Later Start follows dest, not the parent vessel. Later process/step starts gate current type then (`422 route_sample_type`). Dest mint **Hold**: Start extract does not mint a DNA daughter. **OQ-WO-6:** an earlier LimsRun must not share the asked-for `analysis_id`.
