...
**Equivalent aliquot vs dest mint.** Equivalent aliquot is the **same sample in a new container**: no new sample identity, no `sample_type` rewrite. Deiter C2 **Fail** on `02fe95f` stands as signed history. Live lock on **`1572071`** (in code; not QA-clicked): execute retargets the process assignment onto the dest container in the execute txn (`_follow_destination_in_process`; does not require `entry.process_step_id`); emptied-source assign **422**. Do **not** write Pass. **PATCH is not a path.** Dest mint Hold **Pass** on `02fe95f` is a different punch — still **Blood**, **0 DNA**; a new Sample with `dest_sample_type` is not this fix. Extract-hold UAT step **1.7** is **OOB entry execute** and must not teach a DNA daughter.
