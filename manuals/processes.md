...
**Equivalent aliquot vs dest mint.** Equivalent aliquot is the **same sample in a new container**: no new sample identity, no `sample_type` rewrite. Deiter C2 **Fail** on `02fe95f` stands until a restamp. **Coded, not restamped:** execute retargets the process assignment onto the dest container in the execute txn (`_follow_destination_in_process`; does not require `entry.process_step_id`); emptied-source assign **422**. **PATCH is not a path.** Dest mint Hold **Pass** is a different punch — still **Blood**, **0 DNA**; a new Sample with `dest_sample_type` is not this fix. Extract-hold UAT step **1.7** is **OOB entry execute** and must not teach a DNA daughter.
