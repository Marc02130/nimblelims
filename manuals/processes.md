...
**Asked-for does not start a process.** Recording **requested analysis** does not execute or mint a Test. A routing-map row authors TAT + ordered `process_definition[]`, with no analysis or sample-type picker. Map save **409**s when overlapping TAT, overlapping first-step allow-lists, **and** overlapping LIMS Run analyses all hold; extract-first and Qubit-first for the same TAT are legal. Route assigns when current type is on the first process’s first Experiment/LimsRun list **and** the asked-for analysis matches a LIMS Run in the route: zero acceptable → **422**, two saved rows that both accept this type and this analysis → **409**, exactly one → queued work order. Never silently use `first()`. Map save **422**s when the type emerging from process *x* is not accepted by process *x+1* (aliquot/pool dest on the last experiment/LIMS Run of *x* if set; else that last step’s accepted types). **Start process** requires `experiment:manage` and instantiates only the first process. A later start advances to the next definition in snapshot order. Later process/step starts gate current type then; empty or incompatible returns **422** `route_sample_type`. Dest-type mint remains Hold.
