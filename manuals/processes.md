...
**Asked-for does not start a process.** Recording **requested analysis** on `/asked-for` does not assign samples to a process, does not execute, and does not mint a Test. The unnumbered later **Route** planner requires `test:assign` plus project access, not `experiment:manage`; it mints a queued work order, and that mint does not mean work has started. **Start process** on `/work-orders` requires `experiment:manage`, instantiates the first snapshot definition, and sets `eln_processes.work_order_id`. Route must make the process and step order apparent; it must not present steps as an unordered bag. LimsRun steps pick an **analysis** (no fake ExperimentTemplate required). Routing-map create authors analysis + TAT + one process definition, with no sample-type picker. It derives allowed types for the selected first/only process and its first ordered Experiment or LimsRun step. Route compares the sample’s current type with that first-step list. Later steps compare current type only when each starts; an empty or incompatible list returns **422** `route_sample_type`, and the sample is not broken.
