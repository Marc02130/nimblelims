...
| Params | Lock: freeze onto `tests.asked_for_params` at the **first LimsRun start** of the asked-for analysis (WO-7). `if test: continue` is **not** a freeze. Freeze skip NULL is **Tobias Pass** on `bf51b19` (classic NULL; first start writes; later start does not overwrite). `{}` on `99b692d3` stays `8cfa2a9` history. **OQ-WO-6 extract CLOSED** for the common path: extract is an experiment (equipment) and does not wear the panel `analysis_id`. When the ask is an assay, asked-for analysis appears **once** on the assay LimsRun. Extracted DNA: zero assay LimsRuns legal. **OQ-WO-7 OPEN:** WGS params on the DNA Test from the WO after C3. P2 on `main` (`5040f2d`) with this OPEN. **That click never landed.** |
