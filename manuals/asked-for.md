...
**Status:** P1 lake shipped. P2 Route / work_orders / WO-7 surfaces are on `feat/work-order-p2`. Signed AC-P2-9..11 history: `9342439`. Deiter Contents click on product `4671ba8` / assignment commit `02fe95f` (`0077`): C1 **Pass**, C2 **Fail**, dest mint Hold **Pass**. Leadership Confirmed that click; C1/C2 are **not** unsigned. C2 Fail: dest not on the process; emptied source still assignable (**201**) is leftover process-join; later Start rides the emptied parent; **PATCH is not a path**. Docs Confirm `84d2810` is not a new execute and not the click SHA. Dest-join / source-remove is **coded** (`_follow_destination_in_process` in the execute txn). Deiter C2 Fail stands until a restamp. Freeze skip **OPEN**; **OQ-WO-6 stays OPEN**. **Overall P2 unsigned / not Pass.** Hold product merge.
...
5. Experiments → **Work Orders** is the backlog. **First Start instantiates `chain[0]` only** and assigns the receive **container-with-sample**. Deiter C1 **Pass** verifies no-vessel/two-vessel **422** and receive-tube **201**. Do not teach equivalent-aliquot dest-follows as shipped: Deiter C2 **Fail** records execute minted dest, did not join dest or remove source, and emptied-source assign was a **201 mix-up**. **PATCH is not a path**.
