...
| Wrong pairings | Map create has no sample-type or analysis picker. A row holds TAT + ordered `process_definition[]`. A route **may have multiple LimsRun analyses**. Map save 409s when TAT, first-step allow-lists, **and** LIMS Run analysis **sets** overlap. When the ask is an **assay**, Route assigns when current type is on the first process’s first ordered step **and** the asked-for analysis appears on **exactly one** LimsRun (assay step). **Extracted DNA ask:** DNA tube; **zero** assay LimsRuns is legal (**1.4**). Extract is an **experiment** (equipment execute) on a process. It **may** be a LimsRun later if the equipment is an instrument. After extract on a **sequencing** asked-for (WGS / WES), Qubit / Nanodrop / other QC is a **LimsRun on the DNA**. **Process QC is not an asked-for.** **ELISA not on DNA:** a panel/asked-for ELISA is **not** run on the DNA derivative as the intended matrix. Do **not** teach ELISA-on-DNA as the happy path for a blood ELISA ask. Same Sample **may** have a second tube with its **own asked-for + route**. Separate containers = **separate process assignments** — do not collapse two tubes into one process-sample row. |
...
| Params | Lock: freeze onto `tests.asked_for_params` at the **first LimsRun start** of the asked-for analysis (WO-7). `if test: continue` is **not** a freeze. Freeze skip NULL is **Tobias Pass** on `bf51b19` (classic NULL; first start writes; later start does not overwrite). `{}` on `99b692d3` stays `8cfa2a9` history. **OQ-WO-6 extract CLOSED** for the common path: extract is an experiment (equipment) and does not wear the panel `analysis_id`. When the ask is an assay, asked-for analysis appears **once** on the assay LimsRun. Extracted DNA: zero assay LimsRuns legal. **Merge hold is 1.2 dest-cohort lookup** after C3, not freeze skip. |
...
1. For one `requested` row, choose **Route**. For several requested rows, select them and choose **Route selected**. Each asked-for Routes onto **its** map. Two tubes of the same Sample that are both in play keep **separate** process assignments after Start.
2. P2 matches TAT, then keeps **any** route whose first process / first ordered Experiment/LimsRun accepts current type **and**, when the ask is an **assay**, whose chain has the asked-for analysis on **exactly one** LimsRun (the route may have supporting QC LimsRuns too — Qubit / Nanodrop after extract, **not** an asked-for). **Extracted DNA** may match an extract-only route with **zero** assay LimsRuns (**1.4**).
