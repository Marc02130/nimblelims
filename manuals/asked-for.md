...
5. Experiments → **Work Orders** is the backlog. **First Start instantiates `chain[0]` only** and assigns the receive **container-with-sample**. Later Start uses assignments still on the previous process (aliquot/pool dest continues; inbound source is `removed`).
