...
| Wrong pairings | Map create has no sample-type or analysis picker. A row holds TAT + ordered `process_definition[]`. A route **may have multiple LimsRun analyses**. Map save 409s when TAT, first-step allow-lists, **and** LIMS Run analysis **sets** overlap. When the ask is an **assay**, Route assigns when current type is on the first process’s first ordered step **and** `asked.analysis_id` matches the **named asked-for LimsRun slot** (exactly one assay LimsRun). Eligibility is **not** “any chain that contains Qubit.” **Quantified DNA ask (CEO Confirm 1–6 Rolf):** wear existing Qubit once; Test `(DNA, Qubit)` is the ask; old 1.4 **struck**; **422** on 0 LimsRuns. **Nanodrop may sit** as supporting QC. Named asked-for LimsRun slot Closed (**OQ-WO-8 Closed**): eligibility is `asked.analysis_id` vs that slot, not contains-Qubit. Extract is an **experiment** (equipment execute) on a process. It **may** be a LimsRun later if the equipment is an instrument. After extract on a **sequencing** asked-for (WGS / WES), Qubit / Nanodrop / other QC is a **LimsRun on the DNA**. **Process QC is not an asked-for.** **ELISA is not on DNA** (**Leadership Confirm**): do **not** hang ELISA on the DNA dest after C3. Same Sample **may** have a second tube (Contents) with its **own asked-for + route**; two blood tubes → two process assignments (`container_id`); ELISA route and WGS/extract stay apart. |
...
| Params | Lock: freeze onto `tests.asked_for_params` at the **first LimsRun start** of the asked-for analysis (WO-7). `if test: continue` is **not** a freeze. Freeze skip NULL is **Tobias Pass** on `bf51b19` (classic NULL; first start writes; later start does not overwrite). `{}` on `99b692d3` stays `8cfa2a9` history. **OQ-WO-6 extract CLOSED** for the common path: extract is an experiment (equipment) and does not wear the panel `analysis_id`. When the ask is an assay, asked-for analysis appears **once** on the assay LimsRun. **1.4 / OQ-WO-8 Closed; CEO Confirm 1–6 stands:** Quantified DNA is an assay ask; wear existing Qubit. Named asked-for LimsRun slot Closed (not contains-Qubit). **OQ-WO-7 Closed:** WGS params on the DNA Test from the WO after C3. **AC-P2-OQ-WO-7 Pass** (Tobias, 2026-09-01, `80f054b`). After C3, DNA WGS freezes `{library_kit: TruSeq}` from the blood WO, **not** `{}`. Leftover **`9f86d14`** not recoded. Closeout **1.4 / OQ-WO-8 Closed** (**Full Leadership Confirm**; **CEO Confirm 1–6 stands**; named asked-for LimsRun slot Closed — not contains-Qubit). |
...
2. P2 matches TAT, then keeps the route whose first process / first ordered Experiment/LimsRun accepts current type **and**, when the ask is an **assay**, whose **named asked-for LimsRun slot** has `asked.analysis_id` (supporting QC LimsRuns — Qubit / Nanodrop after extract — are **not** an asked-for). Eligibility is **not** “any chain that contains Qubit.” A WGS map with Qubit as process QC must **not** steal a Quantified DNA ask. **Quantified DNA** does **not** match extract-only zero-LimsRun routes (old 1.4 **struck**; **422** on 0 LimsRuns).
...
Publish refuse is **Tobias-signed Pass** on `8cfa2a9` (carol **422** `test_missing`) and remains history on `b005cfe`. Freeze skip NULL is **Tobias Pass** on `bf51b19` — older “freeze skip stays OPEN” is superseded; `{}` on `99b692d3` stays `8cfa2a9` history. Empty Route **422** / map overlap **409** / 201-not-AND are Pass on `8cfa2a9`. Later-step type-gate **Met** on `9342439` (qPCR-on-blood / still-Blood **422**; not dest-type E2E). **OQ-WO-7 Closed** (WGS params on the DNA Test from the WO after C3). **AC-P2-OQ-WO-7 Pass** (Tobias, 2026-09-01, `80f054b`). After C3, DNA WGS freezes `{library_kit: TruSeq}` from the blood WO, **not** `{}`. Leftover **`9f86d14`** not recoded. Closeout **1.4 / OQ-WO-8 Closed** (**Full Leadership Confirm**; **CEO Confirm 1–6 stands**; named asked-for LimsRun slot Closed — not contains-Qubit). Overall P2 Pass remains unsigned; historical `9c4f9da` / `b005cfe` stamps remain signed history.

Route compares current type with the first process’s first ordered Experiment/LimsRun allow-list. When the ask is an **assay**, the asked-for analysis must occupy the **named asked-for LimsRun slot** (`asked.analysis_id` vs that slot, not “any chain that contains Qubit”); map-save / Route **422** if that analysis appears 0 or 2+ times among LimsRuns. **Quantified DNA ask (CEO Confirm 1–6 Rolf):** zero LimsRuns is **not** legal; **422** on 0 LimsRuns. Named asked-for LimsRun slot Closed (**OQ-WO-8 Closed**): eligibility is `asked.analysis_id` vs that slot, not contains-Qubit. No acceptable row returns **422**; type or missing-analysis refusal uses `route_sample_type`. Two saved rows that both accept this type and this analysis return **409**. Never silently use `first()`. Map save 409s when overlapping TAT, overlapping first-step allow-lists, **and** overlapping LimsRun analyses all hold; extract-first and Qubit-first for the same TAT must save when types or analyses differ. Map save and Route do not AND inbound type across later processes or steps. Map save **422**s when the type emerging from process *x* is not accepted by process *x+1*. Start instantiates only the first process. **No route branching this phase.** Route **blood** for WGS (extraction → sequencing); that asked-for **owns WGS params** — sequencing is the ask; extract is route machinery. **Do not teach blood ELISA as extract → ELISA on DNA.** C3 mints DNA; a **C2 aliquot** of that DNA continues WGS. **WES is a new asked-for on the DNA tube** with **its own params**; that tube is then **aliquoted or used up**. Two asked-fors, two param snapshots, two WOs. A **second receive tube** on the same blood Sample (ELISA vs WGS) is a different pair of asked-fors, not WES-on-DNA. Same-type Later Start follow is the lock on **`1572071`**, **unsigned** until Tobias (C2/C3 numbered on `570bbc0`). Different dest type (C3, also unsigned) mints a derivative sample with `parent_sample_id`; the parent Sample row stays, while the destination sample + container pair replaces the inbound source assignment on the process in the execute transaction. Deiter C2 **Fail** on `02fe95f` is signed history. The dest mint Hold **Pass** on `02fe95f` is Start-extract Blood / **0 DNA** history, not a ban on type-changing execute minting a derivative.
