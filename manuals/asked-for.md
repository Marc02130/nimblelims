...
| Wrong pairings | Map create has no sample-type or analysis picker. A row holds TAT + ordered `process_definition[]`. A route **may have multiple LimsRun analyses**. Map save 409s when TAT, first-step allow-lists, **and** LIMS Run analysis **sets** overlap. Route assigns when current type is on the first process’s first ordered step **and** the asked-for analysis appears on **exactly one** LimsRun (assay step). Extract is an **experiment** (equipment execute) on a process. It **may** be a LimsRun later if the equipment is an instrument. **If the asked-for is Extracted DNA:** they get a **tube of DNA** — no sequencing, **no other LimsRuns**. After extract on an **assay** asked-for (WGS / WES / ELISA), Qubit / Nanodrop / other QC is a **LimsRun on the DNA**. **Process QC is not an asked-for.** |
...
| Params | Lock: freeze onto `tests.asked_for_params` at the **first LimsRun start** of the asked-for analysis (WO-7). `if test: continue` is **not** a freeze. Classic `/tests` must leave `asked_for_params` **NULL**, or we need a **freeze marker**. **OQ-WO-6 extract CLOSED:** extract is an experiment (equipment), never a LimsRun. Asked-for analysis appears **once** on the assay LimsRun. Freeze skip NULL is **Tobias Pass** on `bf51b19` (classic NULL; first start writes; later start does not overwrite). `{}` on `99b692d3` stays `8cfa2a9` history. |
...
2. P2 matches TAT, then keeps **any** route whose first process / first ordered Experiment/LimsRun accepts current type **and** whose chain has the asked-for analysis on **exactly one** LimsRun (the route may have supporting QC LimsRuns too — Qubit / Nanodrop after extract, **not** an asked-for).
