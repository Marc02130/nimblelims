...
| Wrong pairings | Map create has no sample-type or analysis picker. A row holds TAT + ordered `process_definition[]`. A route **may have multiple LimsRun analyses**. Map save 409s when TAT, first-step allow-lists, **and** LIMS Run analysis **sets** overlap. When the ask is an **assay**, Route assigns when current type is on the first process’s first ordered step **and** the asked-for analysis appears on **exactly one** LimsRun (assay step). **Quantified DNA ask (CEO Confirm 1–6 Rolf):** wear existing Qubit once; Test `(DNA, Qubit)` is the ask; old 1.4 **struck**; **422** on 0 LimsRuns. **Nanodrop may sit** as supporting QC. Named-slot pending. **OQ-WO-8 OPEN.** Extract is an **experiment** (equipment execute) on a process. It **may** be a LimsRun later if the equipment is an instrument. After extract on a **sequencing** asked-for (WGS / WES), Qubit / Nanodrop / other QC is a **LimsRun on the DNA**. **Process QC is not an asked-for.** **ELISA is not on DNA** (**Leadership Confirm**): do **not** hang ELISA on the DNA dest after C3. Same Sample **may** have a second tube (Contents) with its **own asked-for + route**; two blood tubes → two process assignments (`container_id`); ELISA route and WGS/extract stay apart. |
...
| Params | Lock: freeze onto `tests.asked_for_params` at the **first LimsRun start** of the asked-for analysis (WO-7). `if test: continue` is **not** a freeze. Freeze skip NULL is **Tobias Pass** on `bf51b19` (classic NULL; first start writes; later start does not overwrite). `{}` on `99b692d3` stays `8cfa2a9` history. **OQ-WO-6 extract CLOSED** for the common path: extract is an experiment (equipment) and does not wear the panel `analysis_id`. When the ask is an assay, asked-for analysis appears **once** on the assay LimsRun. **1.4 / OQ-WO-8 OPEN; CEO Confirm 1–6 Rolf:** Quantified DNA is an assay ask; wear existing Qubit. Named-slot pending. **OQ-WO-7 Closed:** WGS params on the DNA Test from the WO after C3. **AC-P2-OQ-WO-7 Pass** (Tobias, 2026-09-01, `80f054b`). After C3, DNA WGS freezes `{library_kit: TruSeq}` from the blood WO, **not** `{}`. Leftover **`9f86d14`** not recoded. Closeout **1.4 / OQ-WO-8 stays OPEN** (CEO Confirm 1–6 Rolf; named-slot pending). |
