...
| Wrong pairings | Map create has no sample-type or analysis picker. A row holds TAT + ordered `process_definition[]`. A route **may have multiple LimsRun analyses**. Map save 409s when TAT, first-step allow-lists, **and** LIMS Run analysis **sets** overlap. When the ask is an **assay**, Route assigns when current type is on the first process’s first ordered step **and** the asked-for analysis appears on **exactly one** LimsRun (assay step). **Extracted DNA ask:** DNA tube; **zero** assay LimsRuns is legal (**1.4**). **Qubit / Nanodrop may sit** on that ask. Extract is an **experiment** (equipment execute) on a process. It **may** be a LimsRun later if the equipment is an instrument. After extract on a **sequencing** asked-for (WGS / WES), Qubit / Nanodrop / other QC is a **LimsRun on the DNA**. **Process QC is not an asked-for.** **ELISA is not on DNA** (**Leadership Confirm**): do **not** hang ELISA on the DNA dest after C3. Same Sample **may** have a second tube (Contents) with its **own asked-for + route**; two blood tubes → two process assignments (`container_id`); ELISA route and WGS/extract stay apart. |
