...
**Status:** P1 lake shipped. P2 Route / work_orders / WO-7 surfaces are on `feat/work-order-p2`. Signed AC-P2-9..11 history: `9342439`. Product SHA for `0077` assignment slices: `4671ba8` (0077 in that commit). AC-P2-C1/C2 **unsigned**. Deiter / Hans / Heidi / Günter restamp notes (honesty, **not** a merge vote): later-step type-gate **Met** (qPCR-on-blood / still-Blood **422**; not dest-type E2E); AC-P2-11 / handoff Pass is **map-save only**; dest mint Hold; freeze skip **OPEN**; **OQ-WO-6 stays OPEN**; Route stays `test:assign`. Round 2 **Leadership Confirm** (Rolf/Deiter/Hans/Heidi/Günter; R2-1…R2-4). **Overall P2 unsigned.** Prior **signed** at `8cfa2a9`: empty Route 422 / map 409 / 201-not-AND **Pass** (**UI click-save**, not API-only). No map analysis picker; Route matches a LimsRun in the chain; process *x*→*x+1* handoff — **Leadership Confirm** (Rolf/Deiter/Hans/Heidi/Günter). Hold product merge.  
...
5. Experiments → **Work Orders** is the backlog. **First Start instantiates `chain[0]` only** and assigns the receive **container-with-sample**. After aliquot/pool execute, dest continues and inbound source is `removed`. Later Start follows dest, not the parent vessel. Assign without a container → **422** `process_container_required`.
