...
**Status:** P1 lake shipped. P2 Route / work_orders / WO-7 surfaces are on `feat/work-order-p2` @ `8cfa2a9`. Live AC-P2 **per-AC signed; overall P2 unsigned**. New-Test freeze write Pass on `99b692d3`; classic `/tests` skip **OPEN**. Empty Route 422 / map 409 / 201-not-AND **Pass**. Later-step type-gate **unsigned**. Hold product merge.  
...
| Params | Lock: freeze onto `tests.asked_for_params` at the **first LimsRun start** of the asked-for analysis (WO-7). `if test: continue` is **not** a freeze, and neither is skip-on-`{}` — a classic default `{}` and a frozen `{}` are the same JSON. Classic `/tests` must leave `asked_for_params` **NULL**; NULL or default `{}` is **not** frozen, so first start must **write**. Skip is honest only once classic `/tests` leaves NULL or a **freeze marker** lands. Extract LimsRun must **not** share the asked-for `analysis_id` or it attaches/freezes the panel Test at extract start. Do **not** collect params on receive. New-Test write is **Tobias-signed Pass** on `8cfa2a9` (Test `99b692d3`); classic `/tests` skip-on-`{}` is **OPEN**. Do **not** claim freeze closed. Do **not** claim `{}` vs frozen `{}` is distinguishable. |
...
6. WO-7 lock: `if test: continue` is **not** a freeze, and skip-on-`{}` is not one either — classic default `{}` and frozen `{}` are the same JSON. First LimsRun start of the asked-for analysis **writes** `asked_for_params`. Classic `/tests` must leave it **NULL**; NULL or default `{}` is not frozen. Skip is honest only once classic `/tests` leaves NULL or a freeze marker lands. Extract LimsRun must **not** share the asked-for `analysis_id` or it attaches/freezes the panel Test at extract start. If any cohort sample lacks an active Test at publish, **422** refuses the whole run.

Publish refuse is **Tobias-signed Pass** on `8cfa2a9` (carol **422** `test_missing`) and remains history on `b005cfe`. New-Test freeze write is Pass on `99b692d3` only; classic skip **OPEN**. Empty Route **422** / map overlap **409** / 201-not-AND are Pass; later-step type-gate **unsigned**. Overall P2 Pass remains unsigned; historical `9c4f9da` / `b005cfe` stamps remain signed history.
