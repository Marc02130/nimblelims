...
| Params | Lock: freeze onto `tests.asked_for_params` at the **first LimsRun start** of the asked-for analysis (WO-7). `if test: continue` is **not** a freeze. Freeze skip NULL is **Tobias Pass** on `bf51b19` (classic NULL; first start writes; later start does not overwrite). `{}` on `99b692d3` stays `8cfa2a9` history. **OQ-WO-6 extract CLOSED** for the common path: extract is an experiment (equipment) and does not wear the panel `analysis_id`. When the ask is an assay, asked-for analysis appears **once** on the assay LimsRun. **1.4 / OQ-WO-8 pending Leadership Confirm:** Quantified DNA is the asked-for; Qubit is the asked-for LimsRun. **OQ-WO-7 Closed:** WGS params on the DNA Test from the WO after C3. **AC-P2-OQ-WO-7 Pass** (Tobias, 2026-09-01, `80f054b`). After C3, DNA WGS freezes `{library_kit: TruSeq}` from the blood WO, **not** `{}`. Leftover **`9f86d14`** not recoded. Closeout **1.4 / OQ-WO-8** pending Leadership Confirm. |
