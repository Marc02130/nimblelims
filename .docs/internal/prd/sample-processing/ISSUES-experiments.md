...
| E-12 | Template authoring: method often present; **default dest type** weak/missing | Dest type only at runtime | **Coded** `feat/e12-e14-dest-type-transitions`: template **Default dest sample type** (Same as parent. + catalog dests for the method’s mint op). Execute still catalog-enforces. UAT §8 unsigned |
...
| E-14 | S3 transition catalog: read API + seeds; **mutate API + admin UI** lag | Can’t configure without DB | **Coded** `feat/e12-e14-dest-type-transitions`: `GET/POST/PATCH/DELETE /v1/sample-type-transitions`; mutate **config:edit** (S3); Admin **Dest-type transitions**. UAT §9 unsigned |
...
3. **E-14 + E-12** (catalog admin, template dest sample type) — **coded** this branch; UAT §8/§9 unsigned  
