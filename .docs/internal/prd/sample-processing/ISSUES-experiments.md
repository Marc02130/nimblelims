...
**Status:** Synced 2026-09-20 (E-7 Met on `main` PR **132**; E-6 Coded, UAT unsigned)  
...
| ID | Issue | Why it hurts | Suggested next |
|----|-------|--------------|----------------|
| E-6 | Decision #24 (Available for Testing + process membership) vs accessioning still setting **Received** | Can’t start experiments on freshly accessioned samples | **Coded** on `feat/e6-intake-available-for-testing`: `POST /samples/accession` and `/samples/bulk-accession` write **Available for Testing** (same as CORE receive). Process assign still does **not** change Sample.status (Decision #24). UAT unsigned. |
...
4. **E-7** type gate — **Met** on `main` (PR **132**, `e59a045`). **E-6** intake Available for Testing — **Coded**; UAT unsigned  
5. Docs **E-17–E-19**  
