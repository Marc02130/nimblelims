...
15. **Live `1572071` (unsigned until Tobias):** C2 is **same-type dest-follow only**. Same dest type = same sample, additional container; `_follow_destination_in_process` may retarget `container_id` only on that path; emptied-source assign **422**; later Start follows that dest container. Different dest type = new derivative sample in a new container (`parent_sample_id`); only the parent Sample row stays, preserving its original type and parent-type work. For both grains, execute adds the destination sample + destination container pair to `eln_process_samples` and marks the inbound source assignment `removed` in the same transaction. Dest mint Hold is lifted only for type-changing execute. Test identity stays `(sample, analysis)`. In code on this SHA; not QA-clicked. Do not write C2 Pass or teach type-changing dest as parent `container_id` retarget.
