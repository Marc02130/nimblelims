...
12. **Earlier LimsRun `analysis_id` (OQ-WO-6 stays OPEN — Marc lock pending Leadership overwrite, 2026-08-31):** Stays **OPEN** until extract LimsRun **cannot** share asked-for `analysis_id`. Analysis-in-chain does **not** close it. Extract must **not** be the panel assay or it freezes the panel Test on the parent (usually blood). Type gates catch blood-on-Qubit. Supporting QC = other analyses, own Tests — not a second asked-for on the same process; do not fold QC into extract’s asked-for `analysis_id`. Do **not** write Leadership Confirm on this fold.
...
16. **Marc lock pending Leadership overwrite (2026-08-31) — not Confirm:** One asked-for per process instance. Supporting QC = other analyses, own Tests. DNA extract once (C3) may **join many work orders**, each with **one asked-for** — not extract-every-WO, not one DNA = one WO forever. Freeze per Test: first start writes; later start does not overwrite including frozen `{}`; skip-on-`{}` OPEN. **OQ-WO-6 stays OPEN.** Click SHA C2/C3 remains `570bbc0`. `9342439` / `02fe95f` untouched. Tobias C2/C3 unsigned. Hold merge. Not IC50.
...
**L3 / A5 / SC5 / Hans:** Freeze is **per Test** `(sample, analysis)`. First LimsRun start writes `asked_for.params` → `tests.asked_for_params`. Later start does **not** overwrite — including frozen `{}`. NULL = not frozen yet. `{}` after first start = locked empty. `if test: continue` is **not** a freeze. Classic `/tests` default `{}` makes skip-on-`{}` **not** a freeze. Classic `/tests` must leave `asked_for_params` **NULL**, or we need a **freeze marker**. Do **not** close freeze skip. Do **not** teach skip-on-`{}` as shipped. **OQ-WO-6 stays OPEN** until extract LimsRun **cannot** share asked-for `analysis_id`. Extract must not be the panel assay. P1 does **not** write that Test snapshot. Freeze skip stays unsigned. Marc lock pending Leadership overwrite — not Confirm.

A process instance is bound to **one** asked-for row. Supporting QC analyses get their **own** Tests `(sample, analysis)` — not a second asked-for on the same process, and not folded into extract’s asked-for `analysis_id`. Type-changing execute mints the DNA Sample **once**; that DNA sample may **join many work orders**, each with **one asked-for**. Do not teach extract-every-WO or one DNA sample = one WO forever.
...
| Earlier LimsRun shares asked-for `analysis_id` (OQ-WO-6) | Bounce — **OPEN** until extract LimsRun cannot share it. Extract must not be the panel assay. Analysis-in-chain does not close this |
| One process instance carrying two asked-for assays | Bounce — one asked-for per process instance (Marc lock pending overwrite) |
| QC folded into extract asked-for `analysis_id` | Bounce — supporting QC = other analyses, own Tests |
...
| 2 | P2 routing + work_order + ordered `process_definition[]` + LimsRun WO-7 + 0077 assignment. Signed AC-P2-9..11 `9342439`. Deiter click at `4671ba8` / `02fe95f`: C1 **Pass**, C2 **Fail**, dest mint Hold **Pass** (signed history). On live `570bbc0`, Deiter C1/C2/C3 execute is **Lab Ops Met**, not QA Pass; execute joints remain `1572071`. Tobias restamp has no Results yet. Overall P2 **unsigned / not Pass**. **Hold product merge to main.** Freeze skip, OQ-WO-6, Route two-accept 409, and P2-4 `0074` remain open. Marc lock pending overwrite: one asked-for per process; QC own Tests; DNA may join many WOs. Not Confirm. |
