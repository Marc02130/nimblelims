...
15. **Live `1572071` (unsigned until Tobias):** C2 is **same-type dest-follow only**. Same dest type = same sample, additional container; `_follow_destination_in_process` retargets that assignment (or replaces the source pair with the same-sample dest pair); emptied-source assign **422**; later Start follows that dest container. Different dest type = new derivative sample in a new container (`parent_sample_id`); parent assignment stays and the dest is the new sample+container. Dest mint Hold is lifted only for type-changing execute. In code on this SHA; not QA-clicked. Do not write C2 Pass or teach type-changing dest as parent `container_id` retarget.
...
Start: `ELNProcessService.instantiate_from_definition` on **process[0] / `chain[0]` only**. Assignment is a Contents pair: `container_id` NOT NULL (`0077`). Deiter C1 **Pass** and C2 **Fail** on `02fe95f` are signed history. Live C2 on **`1572071`** is same-type only: `_follow_destination_in_process` retargets the same sample’s assignment to its additional container (or replaces the source pair with the same-sample dest pair); emptied-source assign **422**; later Start follows that dest container. Different dest type mints a new derivative Sample with `parent_sample_id` in a new container; parent assignment stays. **PATCH is not a path.** Instantiate stays `experiment:manage`. Route stays `test:assign`.

**Dest mint Hold history (Deiter Pass, distinct from C2):** on `02fe95f`, Start extract remained Blood / 0 DNA. That signed Result is not a ban on type-changing execute. The Hold is lifted only for type-changing execute, which inserts a new derivative Sample with `parent_sample_id`; the parent stays. C2 remains same-type dest-follow only.
...
| Type-changing execute | Mint new derivative Sample + new container with `parent_sample_id`; parent assignment stays. Dest mint Hold lifted only for this path |
