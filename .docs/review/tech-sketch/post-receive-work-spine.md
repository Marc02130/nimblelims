...
| Layer | Where | When |
|-------|--------|------|
| **Catalog** | `analysis_param_defs.analysis_id` | Admin associates keys to an **assay** (`config:edit`). `unit`, `data_type`, `required`, optional `source_list_id` / `allowed_values`. |
| **Order (P1)** | `asked_for.params` jsonb | User fills values for that analysis on the request. Validate keys vs defs. **No Test.** |
...
`asked_for` — request row. FK sample, analysis. `tat_days int`. `params jsonb`. `status` check constraint.
...
4. Validate params vs defs (unknown key / missing `required` → 422). **OQ-AF-6:** `required` is boolean only, set on the analysis. No “required if …” engine.
5. Insert requested with `params` JSON
6. **Do not** write `tests.asked_for_params`. **Do not** call `_create_tests` / `_create_asked_for_tests`
...
**L3 / A5 / SC5:** At LimsRun start, `tests.asked_for_params = asked_for.params` (copy) and freeze. Column ships in the **P2** migration. P1 does not add it. Shape: JSON object matching that analysis’s defs (see working-note §3 snapshots).
