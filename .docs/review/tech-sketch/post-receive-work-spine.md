...
**Status:** P2 `b005cfe` per-AC history signed; publish-refuse Pass; freeze OPEN then; AC-P2-5 chain-AND Pass (history — do not rewrite). Live `8cfa2a9`: first-start freeze and ordered-route 422/409 are **in code, unsigned until QA**. Overall P2 unsigned/not Pass. Hold product merge. Current lock: ordered `process_definition[]`; no create type picker; derive first-step types; map-save 409 only when TAT **and** first-step allow-lists overlap; Route 409 when two saved rows both accept current type; Start = first process only. Not IC50.
...
9. **Freeze:** first LimsRun start wins. `_mint_tests_at_start` must **not** overwrite `asked_for_params` on an existing Test. Guard is **in code on `8cfa2a9`**, **unsigned until QA**. The `b005cfe` Pass does **not** include this half (OPEN then).
...
WO-7 publish @ `b005cfe` is Tobias-signed Pass: missing cohort Test returns 422, status stays complete / unpublished, and no Test is reminted. First-start params freeze is **in code on `8cfa2a9`**, **unsigned until QA**. Overall P2 Pass is unsigned.
...
| Publish without Test (deleted Test or empty plan / 0 data rows) | **422** the whole run (`_require_wo7_tests` / `plan.errors` @ `b005cfe`). Stay unpublished. Zero Results. Publish-refuse Pass (history). First-start freeze in code on `8cfa2a9`, unsigned until QA. |
...
| 2 | P2 ordered routes + work orders. **Hold product merge.** `b005cfe` remains signed history; publish-refuse Pass; freeze OPEN then; AC-P2-5 chain-AND history. Live `8cfa2a9` unsigned. Current lock: ordered process definitions; map-save 409 only on TAT **and** first-step overlap; Route 409 when two saved rows both accept current type (in code, not QA-clicked); Start = first process only. |
