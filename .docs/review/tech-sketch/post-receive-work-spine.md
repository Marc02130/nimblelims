...
| 2 | P2 routing + work_order + ordered `process_definition[]` + LimsRun WO-7 + 0077 assignment. Signed AC-P2-9..11 `9342439`. Deiter click at `4671ba8` / `02fe95f`: C1 **Pass**, C2 **Fail**, dest mint Hold **Pass** (history). Live SHA **`1572071`**: same-type dest-follow in code; AC-P2-C2 **unsigned**. Type-changing execute mints a derivative Sample with `parent_sample_id`; parent stays. Overall P2 **unsigned / not Pass**. **Hold product merge to main.** Still open: Tobias C2 click, Hans freeze, **OQ-WO-6**, P2-4 `0074`. |
