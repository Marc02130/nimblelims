...
UI /asked-for ──▶ asked_for (P1)
         ...
         existing /v1/eln-processes
                 ...
                 ▼ Target Later Start = next process, dest container-with-sample
                    (lock on `1572071`, unsigned until Tobias;
...
Start: `ELNProcessService.instantiate_from_definition` on **process[0] / `chain[0]` only**. Assignment is a Contents pair: `container_id` NOT NULL (`0077`). Deiter C1 **Pass** and C2 **Fail** on `02fe95f` are signed history. Live C2 on **`1572071`** is same-type only: `_follow_destination_in_process` follows the same sample into its additional container; emptied-source assign **422**; later Start follows that dest container. Different dest type mints a new derivative Sample with `parent_sample_id` in a new container. The parent Sample row stays with its original type; its inbound process assignment is `removed`. In either path, execute writes the destination Contents pair to `eln_process_samples` in the same transaction. **PATCH is not a path.** Instantiate stays `experiment:manage`. Route stays `test:assign`.
...
| Emptied-source assign (Contents amount 0) | **422** — lock on `1572071`, **unsigned**; leftover 201 is C2 Fail history |
| Execute does not join dest / remove inbound in the same txn | **C2 Fail history** on `02fe95f`. Live lock on `1572071`: follow in execute txn; **unsigned**. PATCH is not a path |
| Join / release no-op without `entry.process_step_id` | Bounce on `02fe95f`. Live follow does not require `entry.process_step_id` (`1572071`, unsigned) |
| Later Start follows inbound parent / emptied source | Bounce — live lock on `1572071` is dest container, **unsigned** |
...
| Later Start | Target is next process on dest container (lock on `1572071`, **unsigned**). C2 **Fail** on `02fe95f` is signed history |
...
| 2 | P2 routing + work_order + ordered `process_definition[]` + LimsRun WO-7 + 0077 assignment. Signed AC-P2-9..11 `9342439`. Deiter click at `4671ba8` / `02fe95f`: C1 **Pass**, C2 **Fail**, dest mint Hold **Pass** (Start-extract Blood / 0 DNA history, not a live ban). Live SHA **`1572071`**: same-type dest-follow in code; AC-P2-C2 **unsigned**. Type-changing execute mints a derivative Sample with `parent_sample_id`; the parent Sample row stays and the inbound process assignment is `removed`. Overall P2 **unsigned / not Pass**. **Hold product merge to main.** Still open: Tobias C2 click, Hans freeze, **OQ-WO-6**, P2-4 `0074`. |
