...
| 2 | Instrument catalog grain: instance vs model/vendor? | **Decided** | P0 catalog | **Type** (vendor, model) + **instance** (type FK, serial, name). No location until `.docs/internal/ideas/lab-locations.md`. Parsers/runs key **instance**. | 2026-07-19 | Product | Format ~ type; lineage ~ instance |
...
| 6 | Non-reportable run (no analysis) / method-dev path? | **Decided** | All run paths | **No non-reportable runs.** `analysis_id` **required** on every run (create/start/import/publish). Method-dev deferred to `.docs/internal/ideas/orders-and-projects.md` | 2026-07-19 | Product | See **Decision #6**; aligns [run-results #6](run-results.md) |
...
| 7 | Instruments/CRO catalogs multi-tenant scope? | **Decided** | P0 | **Lab-global only.** No org segregation. Multi-tenant **out of scope** until real multi-org users — see `.docs/internal/ideas/multi-tenant.md` | 2026-07-18 | Product | Pre-release; single lab deployment |
...
**Status:** **Decided** · **Date:** 2026-07-19 · **Owner:** Product  
**Blocks:** Nothing for P1 special-case import (path removed). Method-dev UX blocked on `.docs/internal/ideas/orders-and-projects.md`
...
4. **Method development / scratch / work-over-time** is **deferred** until **lab projects** (`.docs/internal/ideas/orders-and-projects.md`). Still uses a real analysis later—not null.  
...
- **No non-reportable runs** (Decision #6); method-dev → `.docs/internal/ideas/orders-and-projects.md`.  
