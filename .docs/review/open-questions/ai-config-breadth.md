...
| **Unpark (blocker only)** | Open **UI schema DDL** foundation — real CREATE TABLE / ADD COLUMN + **table/column registries** ([`ui-schema-ddl` requirements](../requirements/ui-schema-ddl.md), [OQs](ui-schema-ddl.md)). This is goal-1 robust config so goal-2 AI has something honest to configure. |
