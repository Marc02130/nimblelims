...
- **P2:** explicit Route; ordered `process_definition[]`; one process instance per later start with route position; no map sample-type or analysis field; assign when first-step types match **and** asked-for analysis is a LIMS Run in the route; zero acceptable 422; two saved rows that both accept this type and this analysis 409; map-save 409 when TAT **and** first-step types **and** LIMS Run analyses overlap; map-save 422 when process *x* emerging type is not accepted by *x+1*. **Leadership Confirm** of analysis-in-chain + overlap 409 + handoff 422 (OQ-WO-4 / OQ-TAT-1 / OQ-WO-5; Rolf/Deiter/Hans/Heidi/Günter). Dest-type mint remains Hold. UI shows process/step order, first-step types, chain LIMS Run analyses, and emerging types. Merge held until signed UAT Pass.
