...
**OQ-WO-7 Closed.** Older “OPEN / AC unsigned until Tobias” copy in walls above is **superseded** — do not restamp those walls. Closeout **1.4 / OQ-WO-8** is **Closed** by later Full Leadership Confirm (2026-09-02) — not this Result. Overall P2 remains **unsigned**. Leadership did not stamp overall Pass.
...
**Marc lock pending Leadership Confirm.** Send: [2026-09-01-p2-closeout-1-4-quantified-dna](../../discussions/2026-09-01-p2-closeout-1-4-quantified-dna.md). Not coded. Not a UAT Result. Not overall P2 Pass. Not IC50. Does **not** rewrite OQ-WO-7 Closed / AC-P2-OQ-WO-7 Pass / `bf51b19` / ELISA Confirm.

1. **Quantified DNA is the asked-for** (data). Not a tube-only SKU.
2. **Qubit is the asked-for LimsRun.** Wears that `analysis_id` once. Test `(DNA, Qubit)` is the ask.
...
Older “Extracted DNA = DNA tube; zero assay LimsRuns legal” is **pending overwrite**.

## CEO Confirm 1–6 (Rolf) — Quantified DNA — 2026-09-02 (history; 1–6 stands)

**CEO Confirm 1–6 (Rolf)** plus Qubit-reuse punch. Docs-only. No product code. Not a UAT Result. Not overall P2 Pass. Not IC50.

This fold punched PR **117** / `3e1856b` back from Full Leadership Confirm. **OQ-WO-8 stayed OPEN** on that fold until Deiter / Hans / Heidi / Günter stamped. Named asked-for LimsRun slot was a **punch pending Leadership Confirm** on that fold. **1–6 stands.** Do **not** restamp 1–6.

Send: [2026-09-01-p2-closeout-1-4-quantified-dna](../../discussions/2026-09-01-p2-closeout-1-4-quantified-dna.md).

Does **not** rewrite OQ-WO-7 Closed / AC-P2-OQ-WO-7 Pass / `bf51b19` / ELISA Confirm.

1. Assay ask (concentration/data), not tube-only. Do not mint `extracted = true/false`.
2. Qubit is the asked-for LimsRun (exactly one). Test `(DNA, Qubit)` is the ask.
3. Wear existing Qubit `analysis_id`. Do **not** create a second catalog analysis named Quantified DNA.
4. Other QC (Nanodrop) may sit: own `analysis_id`, own Test, not a second asked-for.
5. Extract stays experiment: no `analysis_id`, no boolean Result.
6. Do not code old 1.4 (zero LimsRuns / extract-only). **422 on 0 LimsRuns is right.** WGS/WES/ELISA unchanged: Qubit is process QC. Tube-only DNA is a later SKU.

**Named-slot punch on that fold (pending then):** Route must name the asked-for LimsRun slot, not any-chain-contains-Qubit. **Superseded** by Full Leadership Confirm below.

## Full Leadership Confirm — OQ-WO-8 Closed (named asked-for LimsRun slot) — 2026-09-02

**Full Leadership Confirm** (Rolf / Deiter / Hans / Heidi / Günter). Docs-only. No product code. Not a UAT Result. Not overall P2 Pass. Not IC50.

**OQ-WO-8 Closed.** Named asked-for LimsRun slot is no longer pending. **CEO Confirm 1–6 stands** (PR **118** / `7612ab4`).
...
- Product honesty **as written on 2026-09-03**: the named-slot decision was Closed with code still pending, and `_acceptable_maps` containment through `_asked_for_lims_run_count(chain, analysis_id) == 1` could let WGS+Qubit-as-QC steal a Quantified DNA ask. **Superseded on the product branch.** Named-slot code shipped at **`6a67667`** (Alembic `0079`) on `feat/p2-named-slot`; containment is **not** the current behavior there. It remains the behavior on `main`, where named-slot code has not merged.
...
## Named-slot product and QA Results — 2026-09-03

OQ-WO-8 remains **Closed** history from PR 119; this is a product/UAT punch, not a reopen, recode, or restamp of the OQ.
...
## Merge-gate restamp — named-slot product on `6a67667` — 2026-09-04

Not a reopen of OQ-WO-8 and not a rewrite of the `6244bf6` Result or the Leadership overall P2 Pass. Markdown only. Not IC50.

- `6244bf6` was rebased onto `8887e36`; the product SHA that must land is **`6a67667`** (`6a67667b4d6378877a38e5542077df316a35077f`), Alembic **`0079`**.
- **AC-P2-OQ-WO-8 Pass** (Tobias QA merge-gate restamp, 2026-09-04 20:18 ET) on click SHA **`effd242`** (`effd24215b23e1bf9e73a00467a11bdc02bd6eb2`), with `6a67667` an ancestor. Live IDs (asked-for, map, work-order, HTTP codes) are in [`UAT_Scripts/uat-post-receive-work-spine.md`](../../../UAT_Scripts/uat-post-receive-work-spine.md).
...
- **P2 live (Leadership Confirm 2026-09-01, Rolf/Deiter/Hans/Heidi/Günter; leftover `9f86d14` on `80f054b` is OQ-WO-7 Grok Build):** P2 is on `main`. Product SHA for **AC-P2-OQ-WO-7** is **`80f054b`** (`80f054b274b02bb48f9dcbba5a05378419ea6b90`). Per-AC **Pass** on `bf51b19` — do **not** rewrite those Results. Overall P2 **unsigned / not Pass**. Care about the asked-for only (PR **111** still stands). **ELISA is not on DNA** — do not hang ELISA on the DNA dest after C3. Second blood tube (Contents) may have own asked-for + route; two tubes → two process assignments (`container_id`); ELISA route and WGS/extract stay apart. **OQ-WO-8 / closeout 1.4** — **Closed.** **Full Leadership Confirm** (Rolf / Deiter / Hans / Heidi / Günter, 2026-09-02). **CEO Confirm 1–6 stands:** Quantified DNA is an assay ask; Qubit is the asked-for LimsRun; wear existing Qubit; do **not** mint Quantified DNA as a second analysis. Old 1.4 struck; **422 on 0 LimsRuns is right.** Map / Route **names the asked-for LimsRun slot**. Eligibility is `asked.analysis_id` vs **that slot**, not “any chain that contains Qubit.” A WGS map with Qubit as process QC must **not** steal a Quantified DNA ask (409 / wrong join). Named-slot is no longer pending. Same OQ-WO-7 lookup after C3; **do not recode.** Do **not** code extract-only zero LimsRuns. No product code in this fold. Product code may start after this fold is on `main`. Assay ask = **one** asked-for LimsRun. Extract typically a process (does not wear panel `analysis_id`); do **not** forever-ban extract-as-LimsRun. **OQ-WO-6 still:** asked-for `analysis_id` once on the assay LimsRun, not extract. Freeze skip NULL **Pass** on `bf51b19`. Tobias C1/C2/C3 **Pass** on `bf51b19`; `570bbc0` remains Lab Ops Met history. **OQ-WO-7 Closed.** **AC-P2-OQ-WO-7 Pass** (Tobias, 2026-09-01, `80f054b`): Test **`55f9cad9`** `(DNA, WGS)` has `{library_kit: TruSeq}` from blood WO **`4ea9de0c`**. Leftover **`9f86d14`** is the lookup (WO asked-for same `analysis_id`, else parent lineage, else `{}`). **Was not recoded.** Lookup on `80f054b`: (1) WO asked-for **only if** `asked.analysis_id == run.analysis_id`; (2) else walk `parent_sample_id`; (3) else `{}`; (4) freeze skip write onto NULL, do not overwrite frozen including locked empty `{}`. Pass/Fail/not-a-Fail stay the AC definition in Hans’s brief. Merged `5040f2d` with this OPEN is history. Closeout **1.4 / OQ-WO-8 Closed** separately. **Named-slot merge gate:** the merge click is the **`6a67667`** restamp — **AC-P2-OQ-WO-8 Pass** (Tobias, 2026-09-04, click SHA **`effd242`**). Do **not** merge on the `6244bf6` paper stamp alone; that Pass is history of a rebased-away SHA. Named-slot code is still **not on `main`**, so containment is only `main` behavior, not current `feat/p2-named-slot` behavior. Do **not** invent overall P2 Pass. Not IC50.
