...
| OQ-TAT-1 | TAT overlap matching when two ranges overlap? | **Decided (provisional) — refined** | P2 | **409 on save only when same analysis, overlapping TAT, and overlapping first-step allow-lists.** Extract-first vs Qubit-first for the same TAT is legal. No first-match. | 2026-08-29 | Leadership |
...
| OQ-WO-4 | Type eligibility on analysis vs execute steps? | **Decided — superseded** | P2 L2 | No map type field. Match analysis + TAT, then current type against each candidate’s first process / first Experiment-LimsRun list. Zero acceptable → 422; two saved rows that both accept current type → 409. Later starts gate current type then; empty fails closed. Dest-type Hold remains out. | 2026-08-29 | Marc / Rolf / Leadership |
...
- **P2:** explicit Route; ordered `process_definition[]`; one process instance per later start with route position; no map sample-type field; zero acceptable routes 422; two saved rows that both accept current type 409; map-save 409 only when TAT **and** first-step allow-lists overlap. UI preserves process/step order and derives first-process / first-step types.
