...
| OQ-WO-7 | After C3, does WO-7 look up asked-for by dest cohort `sample_id`? | **Open** (post-merge follow-up). **Does not block merge.** Marc/CEO merge bar **Met** 2026-09-01. | After merge | WO-7 `_mint_tests_at_start` looks up `AskedFor` by cohort `sample_id` + `analysis_id`. After Blood→DNA, assay on DNA dest looking up asked-for by dest `sample_id` → `{}` (wrong cohort). Not scored on seq-1. Fix via `work_order.asked_for_id` (or parent lineage). **Not freeze skip. Not dest-follow.** Freeze skip NULL **Pass** on `bf51b19` stands. Asked-for-only Marc lock (PR 111) stands. | 2026-09-01 | Grok Build |
...
- **P2 live (Marc lock 2026-09-01, pending Leadership overwrite; merge bar Met):** Care about the asked-for only (PR **111** still stands). **ELISA not on DNA**; second blood tube may have own asked-for + route; separate containers = separate process assignments. Extracted DNA ask = DNA tube; **zero** assay LimsRuns legal (**1.4**). Assay ask = **one** asked-for LimsRun. Extract typically a process (does not wear panel `analysis_id`); do **not** forever-ban extract-as-LimsRun. Freeze skip NULL **Pass** on `bf51b19` — older freeze skip OPEN in Confirm walls is superseded. Tobias C1/C2/C3 **Pass** on `bf51b19`; `570bbc0` remains Lab Ops Met history (do not restamp unsigned C2/C3 on that SHA). **OQ-WO-7 / closeout 1.2 dest-cohort lookup** after C3 (asked-for by dest `sample_id` → `{}`) is **OPEN post-merge**, **not** a merge hold. Overall P2 unsigned / not Pass. **Merge bar Met** — product may merge `feat/work-order-p2` to `main`. Not IC50.
