...
| OQ-WO-4 | Type eligibility on analysis vs on execute steps? | **Decided — superseded by Marc/Rolf authoring lock** | P2 L2 | **Not** `analysis_accepted_sample_types`. A map may name intake/current sample type for analysis × type × TAT matching, but map save and Route do **not** compare it with every step. The planner displays the first ordered Experiment or LimsRun step’s allowed types for information only. At each step start, compare the sample’s **current** type with that step’s `eln_process_definition_step_accepted_sample_types`; empty or incompatible set returns `422 route_sample_type`. Later ordered steps may expect a transformed type. Do not read `sample_type_transitions`; dest-type Hold remains unchanged. No analysis-level accepted-type M2M. | 2026-08-29 | Marc / Rolf |
...
- **P2:** TAT overlap decided. **OQ-WO-1 Decided** (explicit Route). OQ-WO-3 Decided (`eln_processes.work_order_id`). **OQ-WO-4 Decided** (type gate at step start; no chain-wide authoring gate). Schema-changes must list that M2M (+ nullable template / `analysis_id` on `lims_run` steps so Qubit can be a LimsRun) before P2 coding. Route UI must preserve process/step order and show first-step allowed types as information only.
