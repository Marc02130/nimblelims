...
| OQ-WO-6 | May extract wear the asked-for `analysis_id`? | **CLOSED (extract punch).** Leadership Confirm 2026-08-31; reinforced by Quantified DNA Confirm 2026-09-02. Round 2 “stays Open” is history. | P2 WO-7 | Extract stays an **experiment** with no `analysis_id` and no boolean Result. Manual or robot does not make extract a LimsRun for Quantified DNA. Exactly one named LimsRun slot is asked-for. For Quantified DNA, that slot wears the existing Qubit analysis; for WGS/WES/ELISA, Qubit remains process QC. Do not create a second Quantified DNA analysis. [Quantified DNA Confirm](../../discussions/2026-09-01-p2-closeout-1-4-quantified-dna.md). | 2026-09-02 | Leadership |
...
| OQ-WO-8 | Quantified DNA asked-for: Qubit as the assay LimsRun, or tube-only zero LimsRuns? | **Closed — Leadership Confirm 1–6** (Rolf/Deiter/Hans/Heidi/Günter). [Living Confirm](../../discussions/2026-09-01-p2-closeout-1-4-quantified-dna.md). | — (Closed; docs-only fold) | Quantified DNA is data. Reuse the existing **Qubit** catalog analysis on exactly one **named asked-for LimsRun slot**; Test `(DNA, Qubit)` is the ask. Do not create a second Quantified DNA analysis. Nanodrop/other QC has its own `analysis_id` and Test. Extract remains experiment, with no `analysis_id` or boolean Result. Zero LimsRuns → **422**. Route containment is insufficient because WGS may contain Qubit as process QC; eligibility must compare the asked-for with the named slot. WGS/WES/ELISA unchanged. Old tube-only / zero-LimsRun 1.4 is struck; tube-only DNA is a later SKU. OQ-WO-7 lookup unchanged. Overall P2 unsigned. | 2026-09-02 | Leadership |
...
## Leadership Confirm — Quantified DNA / OQ-WO-8 Closed — 2026-09-02

**Leadership Confirm 1–6** (Rolf, Deiter, Hans, Heidi, Günter). This living lock supersedes the preceding pending Marc lock and the old closeout 1.4 tube-only guidance. Historical walls and signed UAT remain history. Docs-only. Not overall P2 Pass. Not IC50.

- Quantified DNA is an assay ask. Existing **Qubit** is exactly one asked-for LimsRun and Test `(DNA, Qubit)` is the ask.
- One `analysis_id`: reuse Qubit. Do not mint a Quantified DNA catalog analysis.
- Other QC keeps its own analysis and Test. Extract stays experiment, with no `analysis_id` or boolean Result.
- **422** on zero LimsRuns is right for this SKU. Tube-only DNA is later.
- WGS/WES/ELISA keep Qubit as process QC.
- Map / Route must name the asked-for LimsRun slot. “Any route containing Qubit” is forbidden because it also matches WGS process-QC routes.
- OQ-WO-7 is unchanged: WO asked-for only when `asked.analysis_id == run.analysis_id`, else parent lineage, else `{}`.

## Gate rule

- **P1:** Unblocked (OQ-AF-* decided, including AF-6: no conditional required).  
- **P2 live:** P2 is on `main`. Per-AC **Pass** on `bf51b19`; do not rewrite those Results. **OQ-WO-7 Closed** with AC-P2-OQ-WO-7 Pass on `80f054b`; its lookup remains WO same-analysis, else parent lineage, else `{}` and was not recoded. **OQ-WO-8 Closed** by Leadership Confirm 2026-09-02: existing Qubit is the named asked-for slot for Quantified DNA; no second analysis; extract is experiment; zero LimsRuns → 422; containment-only matching is superseded. Product code is not part of this fold. Overall P2 remains unsigned / not Pass. Not IC50.
