...
| OQ-WO-8 | Quantified DNA asked-for: Qubit as the assay LimsRun, or tube-only zero LimsRuns? | **Marc lock, pending Leadership Confirm.** Send: [2026-09-01-p2-closeout-1-4-quantified-dna](../../discussions/2026-09-01-p2-closeout-1-4-quantified-dna.md). | Closeout 1.4 | **Quantified DNA is the asked-for** (data). **Qubit is the asked-for LimsRun** (analysis_id once; Test `(DNA, Qubit)` is the ask). Other process QC (Nanodrop, …) supporting — other analyses, own Tests, not a second asked-for. Extract stays experiment / equipment; no analysis_id; no boolean `extracted` Result. Do **not** code extract-only zero-LimsRun routes. WGS/WES/ELISA unchanged: Qubit stays process QC on those asks. Older “Extracted DNA = tube; zero LimsRuns legal” is pending overwrite. Not coded. Not overall P2 Pass. | 2026-09-01 | Marc |
