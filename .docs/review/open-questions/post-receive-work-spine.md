...
| OQ-WO-7 | WGS params on the DNA Test from the WO after C3? | **OPEN.** **In-bar** for method complete / overall P2 Pass honesty. **CEO Accept (Rolf)** of Hans’s written UAT brief (what/why) before Tobias. Result **unsigned**. Merged to `main` (`5040f2d`) with this OPEN — that history stands; do **not** pretend merge was held for it. **That click never landed.** **Science 2026-09-01.** Leadership Confirm 2026-09-01: stays OPEN. | **In-bar** — method complete / overall P2 Pass honesty (not a rewrite of `5040f2d`) | After C3, WGS LimsRun starts on DNA; asked-for WGS with params still on blood. Lookup by dest `sample_id` freezes `{}` on Test `(DNA, WGS)`. Freeze from the **work order’s asked-for** (blood asked-for / WO cohort), **not** `{}`. Not Qubit. Not extract=LimsRun. Not dest-follow. Numbered AC: **AC-P2-OQ-WO-7**. **Grok Build codes first**, then Tobias clicks. Land: **Brief → code → UAT with Pass/Fail and not-a-Fail → stamp → merge**. Seq-1 on `bf51b19` did **not** score this. Leftover `9f86d14` is not a UAT click and not on `main`. **Not freeze skip.** Freeze skip NULL **Pass** on `bf51b19` stands. Do **not** invent Pass/Fail. Closeout **1.4** may stay OPEN separately. | 2026-09-01 | Sci CSO |
...
Per-AC on `bf51b19` **Pass**. **Overall P2 stayed unsigned.** We merged (`5040f2d`) with **OQ-WO-7 OPEN** (WGS params on the DNA Test from the WO after C3). **That click never landed.** Clarifying the issue made the product better. Land follow-up as **Brief → code → UAT with Pass/Fail and not-a-Fail → stamp → merge**. Going forward the method requires that path; OQ-WO-7 is **in-bar** (blocks “method complete” / overall P2 Pass honesty). Do **not** pretend `5040f2d` was held for it.
...
3. **OQ-WO-7 stays OPEN** until Tobias stamps Pass/Fail. **In-bar** for method complete / overall P2 Pass honesty. Brief → code → UAT Pass/Fail + not-a-Fail → stamp → merge.
...
- **P2 live (Science 2026-09-01; Leadership Confirm 2026-09-01, Rolf/Deiter/Hans/Heidi/Günter; CEO Accept Rolf 2026-09-01 of Hans AC-P2-OQ-WO-7 brief):** P2 is on `main` (`5040f2d`). Per-AC **Pass** on `bf51b19`. Overall P2 **unsigned / not Pass**. Care about the asked-for only (PR **111** still stands). **ELISA is not on DNA** — do not hang ELISA on the DNA dest after C3. Second blood tube (Contents) may have own asked-for + route; two tubes → two process assignments (`container_id`); ELISA route and WGS/extract stay apart. Extracted DNA ask = DNA tube; **zero assay LimsRuns** legal (**1.4**); **Qubit/Nanodrop allowed** on that ask. Assay ask = **one** asked-for LimsRun. Extract typically a process (does not wear panel `analysis_id`); do **not** forever-ban extract-as-LimsRun. **OQ-WO-6 still:** asked-for `analysis_id` once on the assay LimsRun, not extract. Freeze skip NULL **Pass** on `bf51b19`. Tobias C1/C2/C3 **Pass** on `bf51b19`; `570bbc0` remains Lab Ops Met history. **OQ-WO-7 OPEN and in-bar** — WGS params on the DNA Test from the WO after C3 (Hans brief / **AC-P2-OQ-WO-7**; CEO Accept of the written what/why). Merged with this OPEN. **That click never landed.** **Grok Build codes first**, then Tobias clicks. Result unsigned until Tobias. Land: Brief → code → UAT (Pass/Fail and not-a-Fail) → stamp → merge. Do not invent Pass/Fail. Closeout **1.4** may stay OPEN separately. Not IC50.
