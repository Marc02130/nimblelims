...
| OQ-WO-4 | Type eligibility on analysis vs on execute steps? | **Decided — superseded again by Marc/Rolf map-create lock** | P2 L2 | Routing-map authoring has **no sample-type field**. Match is analysis + TAT. Display allowed types derived from the selected first/only process and its first ordered Experiment/LimsRun step; optional storage/refresh is derived, never admin-authored. Route compares the sample’s current type with that first-step allow-list only; empty or incompatible returns `422 route_sample_type` and mints nothing. Later steps check current type only at start; empty fail-closes there. Do not read `sample_type_transitions`; dest-type Hold remains unchanged. | 2026-08-29 | Marc / Rolf |
...
- **P2:** TAT overlap decided. **OQ-WO-1 Decided** (explicit Route). OQ-WO-3 Decided (`eln_processes.work_order_id`). **OQ-WO-4 Decided** (no map sample-type field; Route gates first-step derived types; later steps gate at start). Route UI must preserve process/step order and show first-process / first-step allowed types as derived information.
