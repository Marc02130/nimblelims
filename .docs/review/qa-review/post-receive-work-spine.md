...
**P2–P5 are not this PR.** Later scenarios inherit intake matching, first-step Route refusal, later step-start gates, WO-7, persist lock, and SOP Apply ≠ dest-type. Do not invent Qubit/blood testdata IDs.
...
| **QA13** | **P2** | Map row = analysis × intake type × TAT → one process definition. Assert informational first-step types, no chain-wide save gate, first-step mismatch at Route, later/empty-list mismatch at step start, and ordered typed steps. Signed `b005cfe` AC-P2-5 Pass is chain-AND history only. Freeze OPEN; overall P2 unsigned. | Hans/Heidi lock. |
...
| ID | Phase | Notes |
|----|-------|--------|
| AF-WO-* | P2 | Intake match; one process definition; first-step Route gate; later step-start gates; freeze OPEN; publish missing-Test 422 |
...
| **Date** | 2026-08-28 |
| **Implement gate** | P2 OQ-WO-1/3 are Decided under the Hans/Heidi lock; product merge remains held because first-start freeze is OPEN and overall P2 Pass unsigned. |
