...
| **QA13** | **P2** | Map row = analysis + TAT + ordered `process_definition[]`; no type picker. Assert UI order, first-process / first-step derived display, zero acceptable → 422, two acceptable → 409, no `first()`, first-process-only Start, later-start advancement and type gates, no later chain-AND. Signed `b005cfe` AC-P2-5 Pass is chain-AND history only — do not teach map-save 422 as the live rule. First-start freeze remains OPEN. Live AC-P2 stays unsigned for overall Pass. | Marc/Rolf superseding lock. |
