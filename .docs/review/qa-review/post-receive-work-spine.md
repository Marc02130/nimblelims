...
| **QA13** | **P2** | Map row = analysis + TAT + ordered list; no type picker. Route snapshots that list, **zero Tests**. First Start instantiates `chain[0]` only — **Tobias-signed Pass** on `8cfa2a9` (ELISA first LimsRun, 1 step, no Qubit/qPCR Tests). Later Start = next pending process, on the sample that exists then. `if test: continue` is not a freeze; classic `/tests` must leave `asked_for_params` NULL and NULL or default `{}` is not frozen; **skip-on-`{}` is not a freeze either** — classic default `{}` and frozen `{}` are the same JSON; do **not** claim they are distinguishable; new-Test write Pass on `99b692d3` only; extract LimsRun must not share asked-for `analysis_id`. Live AC-P2 **per-AC signed; overall unsigned** on `8cfa2a9`. Later-step type-gate **unsigned**. Dest-type Hold out. Hold product merge. | Hans/Heidi/Günter lock. Live Tobias click 2026-08-30. |
...
| ID | Phase | Notes |
|----|-------|--------|
| AF-WO-* | P2 | Ordered `process_definition[]`; no type picker; map-save 409 only on TAT **and** first-step overlap; empty Route 422 Pass on `8cfa2a9`; two-accept 409 unsigned; first process starts first **Pass**; later-step type-gate unsigned; new-Test freeze write Pass on `99b692d3`; classic skip OPEN; publish missing-Test 422 Pass on `8cfa2a9` (`b005cfe` history) |
