...
| **QA13** | **P2** | UAT later: map create has analysis + TAT + one process and no sample-type picker; first-process / first-step types are derived display; Route compares current type with first step only and returns **422 `route_sample_type`** with no WO on mismatch; later steps gate current type only at start; empty fails closed there; TAT overlap **409**; ordered-step UI; WO does not mint Tests; publish without Test **422**; first-start freeze remains OPEN on `b005cfe`. The signed `b005cfe` chain-AND map-save result remains history and is not re-scored. | Marc/Rolf superseding lock; Lab Ops L2–L4. |
...
| ID | Phase | Notes |
|----|-------|--------|
| AF-WO-* | P2 | No map type picker; analysis + TAT match; derived first-process / first-step types; first-step `route_sample_type` gate at Route; later-step gate at start; ordered UI; first-start freeze OPEN on `b005cfe`; TAT overlap 409; publish without Test 422 |
