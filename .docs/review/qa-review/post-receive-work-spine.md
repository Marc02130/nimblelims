...
| **QA13** | **P2** | Map row = analysis + TAT + ordered `process_definition[]`; no type picker. Assert UI order, first-process / first-step derived display, map-save 409 only on overlapping TAT **and** overlapping first-step allow-lists, extract-first vs Qubit-first same TAT legal, zero acceptable → 422, two saved rows that both accept current type → 409, no `first()`, first-process-only Start, later-start advancement and type gates, no later chain-AND. Signed `b005cfe` AC-P2-5 Pass is chain-AND history only. Live freeze / Route 422/409 are **in code on `8cfa2a9`, unsigned until QA**. Live AC-P2 has no Pass/Fail. Hold product merge. | Heidi/Leadership lock. |
