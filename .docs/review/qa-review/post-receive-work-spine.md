...
| **QA13** | **P2** | Map row = analysis + TAT + ordered list; no type picker. Route snapshots that list, **zero Tests**. First Start instantiates `chain[0]` only — if it also mints later processes (Qubit/reporting) or their Tests, punch it; do not teach as shipped. Later Start = next pending process, on the sample that exists then. `if test: continue` is not a freeze; classic `/tests` must leave `asked_for_params` NULL and NULL or default `{}` is not frozen; **skip-on-`{}` is not a freeze either** — classic default `{}` and frozen `{}` are the same JSON, so skip is testable only once classic leaves NULL or a freeze marker lands; extract LimsRun must not share asked-for `analysis_id` or it freezes the panel Test at extract start. Live AC-P2 unsigned on `8cfa2a9`. Do not claim freeze closed. Dest-type Hold out. Hold product merge. | Hans/Heidi/Günter lock. |
...
| ID | Phase | Notes |
|----|-------|--------|
| AF-WO-* | P2 | Ordered `process_definition[]`; no type picker; map-save 409 only on TAT **and** first-step overlap; zero acceptable 422; two-accept 409; first process starts first; later starts advance/gate; freeze / Route 422/409 in code on `8cfa2a9`, unsigned until QA; publish missing-Test 422 (`b005cfe` Pass history) |
