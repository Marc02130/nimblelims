...
| **QA13** | **P2** | UAT later: matching route mints WO + asked-for `routed`; empty map → no WO; map save permits a later ordered step to accept a transformed type and does not chain-AND intake type; first ordered Experiment/LimsRun allowed types are informational; starting a step with an incompatible current type → **422 `route_sample_type`** and does not mark the sample broken; TAT overlap **409**; Route/process UI shows ordered steps, not a bag; WO does not mint Tests; LimsRun start mints Test (WO-7); publish without Test **422**; first-start freeze remains OPEN on `b005cfe`; no OOB claim that dest-type Hold is closed. | Marc/Rolf authoring lock; Lab Ops L2–L4 as superseded. |
...
| ID | Phase | Notes |
|----|-------|--------|
| AF-WO-* | P2 | Map authoring without chain-wide type AND; `route_sample_type` at step start; first-step types informational; ordered-step UI; L3 params snapshot target (first-start freeze OPEN on `b005cfe`); TAT overlap 409; publish without Test 422 |
