...
```
 /asked-for  ──► asked_for (requested)
                      ...
                      │  type gate = analysis_accepted_sample_types  (NOT sample_type_transitions)
                      ▼
...
| **A4** | **P2** | Blocks P2 | **Type eligibility is CONFIG, not `sample_type_transitions`.** SoT: new M2M `analysis_accepted_sample_types (analysis_id, sample_type_id)` → `analyses` + `list_entries`. Empty set = fail closed on map save and on route. Check: **current** sample type ∈ asked-for analysis accepted set. Named error `route_sample_type` **422**. Do not read `sample_type_transitions`. Do not infer “an earlier extract will mint DNA.” Extract → Qubit keyed on blood **refuses**. No OOB blood→Qubit rows. Do not invent Qubit/blood testdata IDs. Schema-changes must list this table before P2 coding. |
...
| **A8** | **P4** | Blocks P4 | **LimsRun steps need `analysis_id`.** ALTER `eln_process_definition_steps`: `experiment_template_id` nullable; ADD `analysis_id` uuid NULL FK `analyses`. CHECK: `eln_experiment` ⇒ template NOT NULL; `lims_run` ⇒ `analysis_id` NOT NULL. Same on instance steps if they snapshot the def. `sop_parse_jobs.process_definition_id` as already listed. Apply never silent-activate (L5). No dest-type E2E in this packet. |
