...
| Type gate (L2 / OQ-WO-4) | new `eln_process_definition_step_accepted_sample_types`; ALTER steps: nullable template + `analysis_id` for `lims_run` | Gate on **steps** (experiment and LimsRun), not analysis. **Qubit is a LimsRun.** Empty accepted set = fail closed. **Do not** read `sample_type_transitions`. |
| Route | `asked_for_service` + new `routing_service` / `work_order_service` | Auto-route vs button = OQ-WO-1 (Open). Empty map → 200 `work_order: null`. Overlap 409 on map save. |
| Start | `ELNProcessService.instantiate_from_definition` (`backend/app/services/eln_process_service.py` ~L212) via `POST /v1/eln-process-definitions/{id}/instantiate` | Existing `experiment:manage`. Snapshot chain walk (L4) is new. |
...
| **D12** | **P2 (not P1)** | OQ-WO-1 / OQ-WO-3 / **OQ-WO-4 Decided.** Type gate table is `eln_process_definition_step_accepted_sample_types` (experiment **and** LimsRun steps; **not** analysis). Qubit = LimsRun. Also: nullable `experiment_template_id` + `analysis_id` on lims_run steps, `btree_gist`, `ensure_test` off publish. No OOB blood→Qubit rows. | Lab Ops L2; Leadership rejected analysis-level accepted types as too narrow. |
