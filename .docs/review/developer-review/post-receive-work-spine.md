...
| Area | File | Work |
|------|------|------|
| Migration | new `007x_routing_work_orders.py` | `routing_map` (`int4range` + `EXCLUDE USING gist` → **`CREATE EXTENSION IF NOT EXISTS btree_gist`**), `work_orders`. Prefer **one** FK: `work_orders.process_id` (OQ-WO-3 still Open). |
| Type gate (L2 / OQ-WO-4) | new `eln_process_definition_step_accepted_sample_types`; ALTER steps: nullable template + `analysis_id` for `lims_run` | At step start, compare current type with that step (Experiment or LimsRun). Empty/incompatible = `route_sample_type` 422. Map save and Route do not chain-AND intake type. **Do not** read `sample_type_transitions`. |
| Route | `asked_for_service` + new `routing_service` / `work_order_service` | Explicit Route per OQ-WO-1. Empty map → 200 `work_order: null`. Overlap 409 on map save. Intake type participates in matching; display first ordered step’s allowed types as information only and expose ordered steps. |
...
| **D12** | **P2 (not P1)** | OQ-WO-1 / OQ-WO-3 / **OQ-WO-4 Decided and superseded by Marc/Rolf authoring lock.** Keep `eln_process_definition_step_accepted_sample_types`, but enforce against current type only when a step starts. Do not AND intake type across steps at map save or Route. First ordered Experiment/LimsRun allowed types are informational; Route UI preserves order. Also: nullable `experiment_template_id` + `analysis_id` on LimsRun steps, `btree_gist`, `ensure_test` off publish. Dest-type Hold remains out. | Authoring lock + Lab Ops step-start integrity. |
