...
| **D12** | **P2 (not P1)** | **Marc/Rolf superseding lock:** remove map sample-type authoring. Match analysis + TAT. Derive allowed types from the selected first/only process and first ordered Experiment/LimsRun step. Route gates that step only; later steps gate current type at start. Keep `eln_process_definition_step_accepted_sample_types`, nullable `experiment_template_id` + `analysis_id`, `btree_gist`, and ensure-test off publish. | Map-create lock + Lab Ops integrity. |
