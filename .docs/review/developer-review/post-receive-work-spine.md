...
| Type gate (L2) | `eln_process_definition_steps` **needs a new column** | Today `experiment_template_id` is NOT NULL; no `analysis_id`; no accepted sample type. Analyses have no accepted-type field. **Do not** read `sample_type_transitions`. |
...
| **D12** | **P2 (not P1)** | Do **not** start P2 until OQ-WO-1 and OQ-WO-3 are **Decided**, L2 has a **named column** for accepted sample type on the LimsRun definition step and/or analysis, `btree_gist` is in the migration, and `ensure_test` is moved off publish. No OOB blood→Qubit rows. | Lab Ops P2 CLOSED; type gate is unimplementable on current `eln_process_definition_steps`. |
