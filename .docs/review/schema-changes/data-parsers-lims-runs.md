...
**Location:** Do **not** FK instruments → client **`locations`**. That table should become **`addresses`** (rename idea). Lab **buildings/rooms** → `.docs/internal/ideas/lab-locations.md`. No instrument location this cycle.
...
| **`instruments.room_id` / location** | Lab buildings/rooms + rename `locations`→`addresses` — `.docs/internal/ideas/lab-locations.md` |
...
**Multi-tenant / org segregation:** out of scope — `.docs/internal/ideas/multi-tenant.md`. Do not add tenant keys “for later” in this feature.
...
| **Multi-tenant / org segregation** | **Out of scope** — `.docs/internal/ideas/multi-tenant.md` |
