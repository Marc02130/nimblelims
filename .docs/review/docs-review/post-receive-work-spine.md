...
| **DOC-H8** | Hans: the fold still taught “skip only when a snapshot **already exists** (including frozen `{}`)” as if `{}` proved a first-start snapshot. A classic default `{}` and a frozen `{}` are the **same JSON**, so skip-on-`{}` cannot be a freeze. Classic `/tests` must leave `asked_for_params` **NULL**. | Strike skip-on-`{}` everywhere (HOWTO, live UAT expect, README, `manuals/asked-for.md`, `manuals/api-endpoints.md`, `manuals/lims-runs.md`, requirements RQ-WO-11 / AC-P2-8 / bounce list, tech sketch room lock 9 + failure modes, QA13). Skip is honest only once classic `/tests` leaves NULL or a **freeze marker** lands; until then first LimsRun start must **write** and skip only on a provable LimsRun-start snapshot. Live AC-P2 stays unsigned until QA clicks; freeze not closed on `8cfa2a9`. |
| **DOC-H9** | Tobias clicked `8cfa2a9` (compose down). Live stamp was still “unsigned — Tobias has not clicked.” | Fold per-AC Results: receive stay-on-form Pass; asked-for 0 WO Pass; empty Route 422 Pass; alice first process only Pass; new-Test freeze write Pass on `99b692d3` (not classic skip); carol publish 422 Pass; map UI / overlap 409 / 201-not-AND Pass; later-step type-gate **unsigned**. Do **not** write overall P2 Pass. Do **not** rewrite P1 or `b005cfe` (chain-AND stays history, not carried). Hans classic-NULL punch stays on the page. |
...
| Field | Value |
|-------|--------|
| **Verdict** | **Accept with conditions** (DOC-H1–H9 land in this docs-only PR) |
...
```
DOCUMENTARIAN REVIEW: Accept with conditions (DOC-H1–H9 honesty fold)
...
| **Consistency** | Current P2 SoT: analysis + TAT + ordered `process_definition[]`; explicit Route; zero acceptable 422; two saved rows that both accept current type 409; map-save 409 only on TAT **and** first-step allow-list overlap. Signed `b005cfe` chain-AND behavior remains history only. |
...
Already normative (restated so implementers do not drop them): empty routing map mints nothing; map-save 409 only on overlapping TAT **and** overlapping first-step allow-lists; client cannot write asked-for; no `results.unit_id`; no asked-for columns on `samples`; no projects→orders rename.
...
| Phase | When | Docs |
|-------|------|------|
| **P2** | Heidi/Leadership ordered-route lock | Docs must teach analysis + TAT + ordered `process_definition[]`, no type picker, derived first-process / first-step display, map-save 409 only on overlapping TAT **and** overlapping first-step allow-lists, extract-first vs Qubit-first for the same TAT legal, Route zero→422 / two-accept→409, first-process-only Start, later starts/gates, and no chain-AND. Preserve `b005cfe` signed history; freeze OPEN. |
