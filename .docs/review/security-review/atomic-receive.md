...
| **S-AR-3** | High | **Locked (sketch §4b)** | **One path, one txn:** one `POST /api/samples/receive`. Sample + first Container + Contents (+ optional tests) in a **single DB transaction**. First-vessel create shares that txn (not a follow-up call). Bounce a second receive API. |
