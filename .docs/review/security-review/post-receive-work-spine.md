...
| **Tampering** | Params allowlist (S4). Routing overlap 409; empty map mints nothing (S5). WO chain snapshot at mint (S8). Results two-writer 409 (S9). SOP Apply does not eval model output (S10). |
...
| **S5** | High | **P2** | `routing_map` mutate (POST/PATCH/DELETE) = **`config:edit` only**. Not `test:assign`, not `experiment:manage` alone, not Client. FORCE RLS; write policy admin / `config:edit`. Empty map mints **nothing**. TAT overlap → **409** (gist exclude). Auto-route or explicit Route may mint a WO from a **matching** row; it must **not** mutate the map. |
| **S6** | High | **P2** | `work_orders` FORCE RLS + USING/WITH CHECK via sample → project. `POST …/route` and `POST …/start` load asked-for / WO under RLS; hidden → **403**. Start instantiates the first definition via **existing process AuthZ** (`experiment:manage` / `ELNProcessService.create`). **No new start permission. No client expand.** Completing process *N* starts *N+1* from the **WO snapshot** under the same process AuthZ — no second routing hop that can retarget another client’s sample. |
...
| **S8** | Med | **P2** | Snapshot `process_definition_ids` at mint so later map edits cannot retarget in-flight WOs. Client: read WO rows only for own-project samples; **no start**; Client does **not** need routing-map read (deny Client map write; do not leak lab-wide process packs as a client list). Audit: map save, route/mint, WO start, LimsRun Test mint (actor + ids). |
