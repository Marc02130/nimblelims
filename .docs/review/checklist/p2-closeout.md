...
**Now:** P2 is on `main`. Product SHA for **AC-P2-OQ-WO-7** is **`80f054b`** (`80f054b274b02bb48f9dcbba5a05378419ea6b90`). Tobias signed **per-AC Pass** on `bf51b19` (do **not** rewrite those Results). **Overall P2 stayed unsigned / not Pass.** Merge `5040f2d` landed with **OQ-WO-7 OPEN** — that history stands; do **not** pretend it was held for OQ-WO-7. Leftover **`9f86d14`** on **`80f054b`** **is** the OQ-WO-7 Grok Build work. **Do not recode.** Pytest for Hans **AC-P2-OQ-WO-7** landed in that same commit. Remaining work is **Tobias**, not a new design. Tobias clicks **AC-P2-OQ-WO-7** on `80f054b`. **OQ-WO-7 stays OPEN / AC unsigned** until Tobias stamps. Do **not** invent Tobias Pass. **CEO Accept (Rolf)** of Hans’s written what/why stands; “Grok Build codes first” is **done**. Freeze skip NULL is **Tobias Pass** on `bf51b19`. Asked-for-only Marc lock from PR **111** still stands. **Leadership Confirm 2026-09-01** (ELISA not on DNA; second tube; extract LimsRun later). Closeout **1.4** may stay OPEN separately. Do **not** fold dest-follow, extract-as-LimsRun, freeze skip, cardinality, Route 409, or closeout 1.4 as this work. Stack **down**. Not IC50.
...
| OQ-WO-7 WGS params on DNA Test from WO after C3 | **OPEN / AC unsigned** until Tobias stamps. Product SHA **`80f054b`**. Leftover **`9f86d14` is** this Grok Build work — **do not recode**. Remaining work is **Tobias**. **CEO Accept (Rolf)** of Hans brief **AC-P2-OQ-WO-7**. Do **not** invent Tobias Pass. Merged `5040f2d` with OPEN — that history stands |
...
2. **OQ-WO-7 / closeout 1.2 — OPEN until Tobias stamps. In-bar.** After C3 (Blood→DNA) WGS start on the DNA dest must freeze **WGS params from the work order’s asked-for** onto the **DNA Test** (`{library_kit: TruSeq}`), **not** `{}`, **not** Qubit params. Seq-1 on `bf51b19` did **not** score this. Merge `5040f2d` did **not** include it — do **not** rewrite that merge as a hold. Leftover **`9f86d14`** on **`80f054b`** (`80f054b274b02bb48f9dcbba5a05378419ea6b90`) **is** this Grok Build work (pytest in that same commit). **Do not recode.** **Do not** treat leftover as a UAT click. Numbered AC: **AC-P2-OQ-WO-7**. **CEO Accept (Rolf)** of Hans’s written what/why. Remaining work is **Tobias**, not a new design. Tobias clicks on `80f054b`. Result unsigned until Tobias. Lookup already on that SHA: (1) WO asked-for **only if** `asked.analysis_id == run.analysis_id`; (2) else walk `parent_sample_id` for a routed asked-for of that analysis; (3) else `{}`; (4) freeze skip: write onto NULL; do not overwrite an already-frozen payload including locked empty `{}`. Pass: Test `(DNA, WGS)` has `{library_kit: TruSeq}` from the blood WO asked-for. Fail: `{}` / missing kit; WGS Test on blood; assay hung on extract. Not a Fail: no dest at Route/Start; `source_amount_null`; Qubit empty supporting freeze; 1.4; ELISA on a second blood tube. Do **not** fold dest-follow, extract-as-LimsRun, freeze skip, cardinality, Route 409, closeout 1.4 as this work. It is **not** freeze skip and **not** dest-follow.
...
| **1.2 / OQ-WO-7** | WGS params on the DNA Test from the WO after C3 | **OPEN / AC unsigned.** Code **is** on `main` via leftover **`9f86d14`** on **`80f054b`**. **CEO Accept (Rolf)** of Hans brief **AC-P2-OQ-WO-7**. Do **not** invent Tobias Pass. Seq-1 Pass without scoring dest-cohort params | **Do not recode.** Tobias clicks **AC-P2-OQ-WO-7** on `80f054b`. Freeze `{library_kit: TruSeq}` from WO asked-for, not `{}`, not Qubit |
...
| AC-P2-OQ-WO-7 WGS params from WO after C3 | **Unsigned** — CEO Accept (Rolf) of Hans brief; leftover **`9f86d14`** on **`80f054b` is** the Grok Build work; **do not recode**; Tobias has not clicked |
...
| OQ-WO-7 dest-cohort params (1.2) | **OPEN / AC unsigned.** Code on `main` via **`9f86d14`** on **`80f054b`**. **CEO Accept (Rolf)** of **AC-P2-OQ-WO-7**. Do **not** invent Tobias Pass | Tobias clicks **AC-P2-OQ-WO-7** on `80f054b`. WGS params `{library_kit: TruSeq}` from the blood WO asked-for after C3, not `{}`. **Do not recode** |
...
| Science 2026-09-01 | **Folded** | Per-AC Pass. Overall unsigned. Merged with OQ-WO-7 OPEN. Leftover `9f86d14` is this work on `80f054b`. Tobias has not stamped. Not IC50 |
| CEO Accept (Rolf) of Hans AC-P2-OQ-WO-7 brief | **Folded** 2026-09-01 | Written what/why Accept before Tobias. Grok Build (`9f86d14`) is on `80f054b`. Remaining work is Tobias. Result unsigned. In-bar. Not IC50 |
