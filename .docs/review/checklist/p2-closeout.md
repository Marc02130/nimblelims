...
| Cardinality 1 (map-save/Route 422 on 0 or 2+ asked-for LimsRuns) | Coded + Tobias **Pass** `bf51b19` for assay asks. **Leadership Confirm 1.4:** Quantified DNA is an assay ask; existing Qubit appears once in the named asked-for slot; 0 remains **422** |
...
| OQ-WO-6 extract punch | **CLOSED** for the common path — extract is an **experiment** and does not wear an `analysis_id`. **Leadership Confirm 1.4:** existing Qubit is the named asked-for LimsRun for Quantified DNA; Qubit remains process QC for WGS / WES / ELISA |
...
| Closeout 1.4 Quantified DNA | **Leadership Confirm** (2026-09-02; Rolf/Deiter/Hans/Heidi/Günter). Existing Qubit is exactly one named asked-for LimsRun slot; no second catalog analysis; extract is experiment; zero LimsRuns → **422**; containment-only matching is superseded. [Living Confirm](../../discussions/2026-09-01-p2-closeout-1-4-quantified-dna.md) |
...
3. **Closeout 1.4 — Quantified DNA (Leadership Confirm):** Quantified DNA is data, not a tube-only product. Reuse the existing **Qubit** catalog analysis for exactly one named asked-for LimsRun slot; Test `(DNA, Qubit)` is the ask. Do not create a Quantified DNA analysis. Other QC has other analyses and Tests. Extract has no `analysis_id` or boolean Result. **422** on 0 LimsRuns is right. WGS/WES/ELISA keep Qubit as process QC. Route must match the named asked-for slot, not any chain containing Qubit. Old tube-only / zero-LimsRun 1.4 is struck; tube-only DNA is a later SKU.
4. **Overall P2 Pass** — unsigned. Leadership did not stamp overall Pass. Closeout **1.4** waits Leadership Confirm. Do not write overall Pass from this fold.
...
| **1.4 Quantified DNA** | Tube-only zero-LimsRun vs assay ask | **Leadership Confirm.** Existing Qubit = exactly one named asked-for LimsRun slot; other QC supporting | **Closed for docs.** Product code later: name the slot; do not use containment-only matching; no second analysis; 0 LimsRuns → 422 |
...
| Quantified DNA (1.4) | **Leadership Confirm.** Existing Qubit is the named asked-for LimsRun; old zero-LimsRun 1.4 struck | Docs fold only. Product coding remains gated behind this fold |
...
| Overall P2 Pass | **Unsigned** | Do not invent from this fold. Leadership did not stamp overall Pass |
| Merge to `main` | **Landed** `5040f2d` | With OQ-WO-7 OPEN (history) |
