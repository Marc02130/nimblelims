...
| AC-P2-OQ-WO-8 named slot + picker | **Pass** Tobias, `6244bf6` (`feat/p2-named-slot`, `0079`). Named slot is `routing_map.asked_for_step_id`; 0→422, 1→mint, 2+→409 `route_pick_required` then pick mints; no containment steal. **Restamped Pass** on click **`effd242`** / product **`6a67667`** (2026-09-04) for the merge gate |
...
| Merge to `main` | **Landed** `5040f2d` with OQ-WO-7 OPEN (history). Named-slot code **not on `main`**; product SHA to land is **`6a67667`** (`0079`) |
| Named-slot merge gate | **Met.** AC-P2-OQ-WO-8 **Pass** (Tobias QA merge-gate restamp, 2026-09-04, click **`effd242`**, product **`6a67667`**). Not the `6244bf6` paper stamp alone. API-only. Distinct from the UI dogfood Pass |
| Named-slot on `main` | **Landed** merge **`6625490`** (PR **122**), product **`6a67667`** / Alembic **`0079`**. Living tip at UI click: **`be65d1e`** |
| Tobias UI dogfood (alice-tech picker) | **Pass** Tobias, 2026-09-04 21:25 ET, `be65d1e`. Ask-1 mint Qubit-A-named (Map B no steal); Ask-2 redo UI refuse / stayed requested; Ask-3/4 Select a route then pick C-pack. Ops: Active maps Delete-only (no Active toggle) — known, not a Fail. Artifacts `/workspace/uat-oq-wo-8-ui-dogfood-main/`. Not the `effd242` API restamp |
...
6. **Merge** — **Landed** `5040f2d` with OQ-WO-7 OPEN (history). Named-slot **code is not on `main`**. Product SHA after rebase onto `8887e36` is **`6a67667`**. `6244bf6` AC-P2-OQ-WO-8 Pass stays signed history of that SHA and is **not** merge authority on its own. The merge gate is met by the **restamp**: **AC-P2-OQ-WO-8 Pass** (Tobias QA merge-gate restamp, 2026-09-04 20:18 ET, click SHA **`effd242`**, product **`6a67667`**, Alembic `0079`) — one named Qubit slot minted, map B (WGS named / Qubit QC) alone returned **422** and did not steal, zero maps **422**, 2+ **409** `route_pick_required` then mint only on the pick ([brief](../../discussions/2026-09-04-p2-named-slot-uat-restamp.md), IDs in the UAT). Merging `feat/p2-named-slot` to `main` remains a Leadership/Marc action. Freeze skip NULL **Pass** on `bf51b19` is not a hold.
7. **Honesty punch after PR 122 — named-slot on `main`; Tobias UI dogfood Pass.** Product landed on `main` at merge **`6625490`** (PR **122**), product **`6a67667`** / `0079`. **Tobias UI dogfood Pass** 2026-09-04 21:25 ET on `main` tip **`be65d1e`** (alice-tech picker). Ask-1 minted Qubit-A-named; Map B did not steal. Ask-2 redo: UI refuse toast, stayed requested, wo null. Ask-3/4: Select a route, Map B not in picker, pick C-pack → WO. Ops: admin map grid has no Active toggle (Delete only) — **known, not a Fail of named-slot**. The `effd242` API restamp remains history; API-only is not this Pass. Do **not** rewrite `6244bf6` or Leadership overall. Artifacts `/workspace/uat-oq-wo-8-ui-dogfood-main/`. Not IC50.
...
**Do not code in this docs fold:** any product `.py` / `.ts` / Alembic; route branching; dest auto-join second WO; copy WGS params onto WES; second asked-for for Qubit; analysis picker on `/receive`; P3–P5. The named-slot and 2+ picker product is on `main` (merge `6625490` / PR 122, product `6a67667`). This PR only records the Tobias UI dogfood Pass on `be65d1e`. Older “409 only / no picker” copy is superseded. Do not rewrite `6244bf6`, `effd242`, or Leadership overall.
...
| Named-slot on `main` | **Landed** `6625490` (PR 122) | Product `6a67667` / `0079`. Do not rewrite `6244bf6` / Leadership overall |
| Tobias UI dogfood | **Pass** `be65d1e` 21:25 ET | Alice-tech picker. Distinct from `effd242` API restamp |
