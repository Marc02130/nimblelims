...
**Now:** Tobias signed **per-AC Pass**. **Overall P2 is unsigned / not Pass.** Hold merge to `main`. Stack **down**. Not IC50.
...
| Cardinality 1 (map-save/Route 422 on 0 or 2+ asked-for LimsRuns) | Coded + Tobias **Pass** `bf51b19` |
| Supporting QC same-route, own Test | Tobias **Pass** `bf51b19` |
| Freeze skip NULL | Coded `0078` + Tobias **Pass** `bf51b19` (classic NULL; first start `{cell_line: A549}`; later start left it). `{}` on `99b692d3` stays `8cfa2a9` history |
| Route two-accept 409 | Tobias **Pass** `bf51b19` |
| Seq-1 sequential asked-fors (two WOs; WES on DNA tube) | Tobias **Pass** `bf51b19`. Dest-cohort params (**1.2**) **not scored** |
| OQ-WO-6 extract punch | **CLOSED** — extract is an **experiment** (equipment); **never** a LimsRun. After extract, Qubit/Nanodrop QC LimsRun quantifies DNA. **Process QC is not an asked-for.** Exactly one asked-for LimsRun |
