...
| Cardinality 1 (map-save/Route 422 on 0 or 2+ asked-for LimsRuns) | Coded + Tobias **Pass** `bf51b19` for **assay** asked-fors. **1.4 Quantified DNA** is an assay ask (Qubit once) — pending Leadership Confirm |
...
| Closeout 1.4 Quantified DNA | **Marc lock, pending Leadership Confirm.** Qubit is the asked-for LimsRun; other QC supporting. Do **not** code zero-LimsRun extract-only. Send: [2026-09-01-p2-closeout-1-4-quantified-dna](../../discussions/2026-09-01-p2-closeout-1-4-quantified-dna.md) |
...
3. **Closeout 1.4 — Quantified DNA (Marc lock, pending Leadership Confirm):** The SKU is **data**, not a tube-only product. **Quantified DNA is the asked-for.** **Qubit is the asked-for LimsRun** (wears that `analysis_id` once; Test `(DNA, Qubit)` is the ask). **Other process QC** (Nanodrop, …) may sit — other analyses, own Tests, **not** a second asked-for. Extract stays experiment / equipment; **no** analysis_id; **no** boolean `extracted = True/False` Result. Do **not** code extract-only zero-LimsRun routes. 422 on 0 LimsRuns is **right** for this SKU. WGS/WES/ELISA unchanged: Qubit stays process QC on those asks. Send: [2026-09-01-p2-closeout-1-4-quantified-dna](../../discussions/2026-09-01-p2-closeout-1-4-quantified-dna.md). Older “Extracted DNA = tube; zero LimsRuns legal” copy is **pending overwrite**.
...
| **1.4 Quantified DNA** | Tube-only zero-LimsRun vs assay ask | **Marc lock, pending Leadership Confirm.** Quantified DNA = asked-for; Qubit = asked-for LimsRun; other QC supporting | **Do not code** extract-only / zero LimsRuns / boolean Test. If Confirm: cardinality 1 on Qubit (same as ELISA) |
...
| Quantified DNA (1.4) | **Marc lock, pending Leadership Confirm.** Qubit is the asked-for LimsRun | Leadership Confirm of the send. Do **not** code zero-LimsRun extract-only |
...
| Overall P2 Pass | **Unsigned** | Do not invent from this fold. 1.4 pending Leadership Confirm. Leadership did not stamp overall Pass |
| Merge to `main` | **Landed** `5040f2d` | With OQ-WO-7 OPEN (history) |
