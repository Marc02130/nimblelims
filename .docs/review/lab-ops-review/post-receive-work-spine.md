...
**Qubit-on-blood must refuse when the Qubit step starts and the sample’s current type is still blood.** Named error, not a broken sample. Map authoring may still name blood as the intake type and include a later Qubit DNA step; it does not AND blood across the chain. The sketch must not read `sample_type_transitions` as proof that an earlier step minted DNA. Until extract-hold dest type actually writes DNA + `parent_sample_id` + process membership, the Qubit step-start gate still refuses the unchanged parent.
...
| **L2** | **P2** | **Marc/Rolf authoring lock:** map save and Route do not AND the intake/current type across all process steps. The map may name that type for analysis × type × TAT matching; later ordered steps may accept a transformed type. Show the first ordered Experiment or LimsRun step’s allowed types for information only. At each step start, compare the sample’s current type with that step’s accepted types; empty or incompatible returns `route_sample_type` (422) and does not mark the sample broken. **Qubit is a LimsRun.** `sample_type_transitions` is not proof execute minted a daughter. Until extract-hold dest type writes DNA + `parent_sample_id` + `eln_process_samples`, starting Qubit on the unchanged blood parent refuses. No invented Qubit/blood testdata IDs. | Preserves real extraction workflows without authoring an impossible all-steps intake-type AND, while retaining the bench integrity gate where work starts. |
...
| **P2** | Historical gate was **CLOSED** until L2–L4 entered the sketch. Marc/Rolf now supersede L2 authoring semantics: no chain-wide map-save type AND; gate current type at step start. L4 means ordered steps in one process definition. |
