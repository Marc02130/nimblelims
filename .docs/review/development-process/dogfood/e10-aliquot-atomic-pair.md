...
**Live state:** formal **§7 Result: Fail** (Tobias QA, 2026-09-14 21:31 ET, `e5a8fdd`) and **Rolf Confirm: Hold merge** (`feat/e10-aliquot-atomic-pair` → `main`). Packet 7.1–7.8 Pass; overall Fail = Deiter **double-Add** (API has no single-pair uniqueness). Do **not** invent overall §7 Pass. Product owns the API uniqueness fix before restamp; no fix is claimed here.  
