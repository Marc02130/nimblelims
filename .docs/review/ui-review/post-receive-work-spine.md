...
| **U11** | **P2** | Binds L2–L4 UX | Routing-map authoring has analysis, TAT, and one ordered process definition — **no sample-type picker**. Display allowed types derived from the selected first/only process and its first ordered Experiment/LimsRun step; refresh on process/step change. Route type refuse (`route_sample_type`): “The first step does not accept this sample’s current type. No work order created. The sample is not broken.” Do not inspect later steps at Route. Later step-start refuse uses the same current-type wording. Completing step **N** surfaces the next ordered step. First-start freeze remains OPEN on `b005cfe`. |
