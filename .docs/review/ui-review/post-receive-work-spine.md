...
asked-for requested
   ...
   Work orders surface
     ...
     complete N → start N+1 from WO snapshot
...
| **U11** | **P2** | Binds L2–L4 UX | Completing process **N** surfaces start of **N+1** from the WO snapshot — no “go find a definition.” Type refuse (`route_sample_type`): “This process does not accept {sample type}. No work order created.” No OOB blood→Qubit row in the routing UI. At LimsRun start, asked-for params show as **frozen** on the Test; tech does not re-type them. |
