...
**P1 may implement** if **U1–U8** land. P2 U9–U11 are bound by the Hans/Heidi map/type lock; product merge remains held on the OPEN freeze / unsigned overall Pass.
...
```
asked-for requested
   explicit Route → match analysis × intake type × TAT
                  → no map: no_route | first step rejects: 422 | match: work_order
   Work orders surface
     Start → one process definition with ordered typed steps
...
| **U10** | **P2** | Explicit Route | Match analysis × intake type × TAT. No map shows `no_route`. If the mapped first step rejects intake, show 422 `route_sample_type`. |
| **U11** | **P2** | Binds L2–L4 UX | Map authoring has analysis, intake sample type, TAT, and one process definition. Display first-step allowed types as information. Route refuses first-step mismatch; later steps show current-type refusal at start. Steps remain ordered. Freeze remains OPEN. |
...
| **OQ-WO-3** | **Decided:** P2 work order snapshots one process definition. |
