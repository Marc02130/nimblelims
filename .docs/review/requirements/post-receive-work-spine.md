...
| AC-P2-C2 | **Fail** on `02fe95f` (Deiter history). Live **`1572071` unsigned**: same dest type → same sample, new container; dest on process; emptied source **422**; Later Start follows dest container. Not Blood→DNA. |
| AC-P2-C3 | **Unsigned** on `1572071`: dest type DNA → new Sample + container (`parent_sample_id`); parent stays Blood; dest pair is the only active process assignment. Same click as extract-hold 1.7. |
