...
**Date:** 2026-08-28  
**Status:** Spec **Accept with conditions** on P2 (`feat/work-order-p2`). **P1 shipped** on `main` (PR 81; UAT Pass). **Hold product merge to main.** Live SHA `9342439` — **Tobias-signed AC-P2-9..11 Pass**. Restamp notes honesty, **not** a merge vote: later-step type-gate **Met**; dest mint **Hold**; freeze skip **OPEN**; **OQ-WO-6 OPEN** (earlier LimsRun must not share asked-for `analysis_id`; extract is not a special assay); Route stays `test:assign`; **overall P2 unsigned**. A route **may have multiple analyses**; asked-for matches any chain that **contains** that analysis. Round 2 send: [2026-08-30-p2-route-lock](../../discussions/2026-08-30-p2-route-lock.md). `8cfa2a9` / `b005cfe` signed history. Not IC50.
