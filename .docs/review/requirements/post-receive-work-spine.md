...
**Date:** 2026-08-28  
**Status:** Spec **Accept with conditions** on P2 (`feat/work-order-p2`). **P1 shipped** on `main` (PR 81; UAT Pass). **Hold product merge to main.** Signed AC-P2-9..11 history: `9342439`. Deiter clicked the Contents grain on product `4671ba8` / assignment commit `02fe95f` (`0077`): AC-P2-C1 **Pass**, AC-P2-C2 **Fail**, dest mint Hold **Pass**. Docs Confirm `84d2810` is not a new execute and not the click SHA. Assignment is a Contents pair (`container_id` NOT NULL) — the tube in hand; **Contents grain Leadership Confirm** (Rolf/Deiter/Hans/Heidi/Günter). C2 records that execute mints dest, does not join dest or remove source; emptied-source assign is a **201 mix-up**; **PATCH is not a path**. Do not teach dest-follows as shipped. Dest mint Hold Pass is distinct: Start extract remains Blood with **0 DNA** and does not close `_execute_transfer`. Freeze skip **OPEN**. **OQ-WO-6 stays OPEN**. Route stays `test:assign`. Round 2 **Leadership Confirm** (Rolf/Deiter/Hans/Heidi/Günter; R2-1…R2-4). **Overall P2 unsigned / not Pass.** Send: [2026-08-30-p2-route-lock](../../discussions/2026-08-30-p2-route-lock.md). `8cfa2a9` / `b005cfe` signed history. Not IC50.
...
8. **P2 ordered route (Leadership / Heidi / Günter; AC-P2-9..11 Pass on `9342439`; prior signed on `8cfa2a9`):** Route snapshots the ordered list, **zero Tests**. First Start = process[0] / `chain[0]` only — **Tobias-signed Pass** (`8cfa2a9`). Empty Route **422** Pass. Map overlap **409** / blood+qPCR **201** (not AND) Pass (`8cfa2a9`). Later-step type-gate **Met** on `9342439` (qPCR-on-blood / still-Blood **422**; not dest-type E2E). Deiter clicked `0077` at `4671ba8` / `02fe95f`: C1 **Pass**, C2 **Fail**, dest mint Hold **Pass**. C2 means do not teach later Start dest-follows as shipped.
...
15. **0077 assignment / dest follow (`02fe95f` — Heidi / Mathilda / Rolf; Contents grain **Leadership Confirm** 2026-08-30, Rolf/Deiter/Hans/Heidi/Günter):** Deiter C1 **Pass** at product `4671ba8` / assignment commit `02fe95f`: assignment is the **tube in hand**; no vessel and ambiguous multi-vessel assign return **422**, receive-tube assign returns **201**. Deiter C2 **Fail**: execute mints dest, does **not** join dest or remove source; emptied-source assign is a **201 mix-up**; **PATCH is not a path**. Do not teach equivalent-aliquot dest-follows as shipped. Dest mint Hold **Pass** is distinct: Start extract remains Blood with **0 DNA**; `_execute_transfer` dest mint is not closed. OQ-WO-6 and freeze skip stay OPEN. Route stays `test:assign`. Hold product merge. Send: [2026-08-30-p2-route-lock](../../discussions/2026-08-30-p2-route-lock.md), “Contents grain / `0077`”.
...
| **P2** | Routing + work_order | Test ordering / processing | Architecture / UI / Spec Accept with conditions @ `02fe95f`. Deiter C1 **Pass**, C2 **Fail**, dest mint Hold **Pass** on `4671ba8` / `02fe95f`. Punch dest mint open. OQ-WO-6 and freeze skip stay OPEN. `9342439` AC-P2-9..11 Pass stays signed history. Overall P2 **unsigned / not Pass**. |
...
| **RQ-WO-12** | **0077 (Contents grain — Leadership Confirm; Rolf/Deiter/Hans/Heidi/Günter):** Assignment is the **tube in hand**, a Contents pair. Deiter C1 **Pass** (`4671ba8` / `02fe95f`): assign **422** if no vessel or 2+ without a pick; receive-tube assign **201**. Deiter C2 **Fail**: execute mints dest, does not join dest or remove source; emptied-source assign **201 mix-up**; **PATCH is not a path**. The target equivalent aliquot remains same sample, new container, but do not teach dest-follows as shipped. Dest mint Hold **Pass**: Start extract remains Blood with **0 DNA**; this does not close `_execute_transfer`. Test stays `(sample, analysis)`; the container is which vessel was measured and takes the conc write-through. |
