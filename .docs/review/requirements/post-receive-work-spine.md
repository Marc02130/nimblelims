...
16. **Leadership Confirm — dest-type split; live `1572071` C2 remains unsigned until Tobias:** same dest type = same sample, additional container; `_follow_destination_in_process` may retarget `container_id` only on that path. Emptied-source assign is **422** and later Start follows that dest container. Different dest type = new derivative Sample in a new container (`parent_sample_id`); only the parent Sample row stays, preserving its original type, Tests, and parent-type work. Type-changing execute mints and joins the destination pair and marks the inbound source assignment `removed` in the execute transaction; only the destination pair remains on this chain. Route / Start / map-save mint zero daughters. Dest mint Hold is lifted only for type-changing execute. C2 and extract-hold UAT 1.7 remain two clicks. In code on this SHA; not QA-clicked. Do **not** write C2 Pass or teach type-changing dest as parent `container_id` retarget. Send: [2026-08-30-p2-route-lock](../../discussions/2026-08-30-p2-route-lock.md), “Leadership Confirm — dest-type split”.
