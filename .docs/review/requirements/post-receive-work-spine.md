...
**Leadership Confirm (2026-09-02; Rolf / Deiter / Hans / Heidi / Günter) — closeout 1.4 / OQ-WO-8 Closed:** Quantified DNA is an assay ask. Reuse the existing **Qubit** analysis on exactly one **named asked-for LimsRun slot**; Test `(DNA, Qubit)` is the ask. Do not create a Quantified DNA catalog analysis. Other QC has its own `analysis_id` and Test. Extract stays experiment with no `analysis_id` or boolean Result. Zero LimsRuns → **422**. WGS/WES/ELISA keep Qubit as process QC. Route containment alone is insufficient because it also matches WGS maps containing Qubit as QC. OQ-WO-7 lookup unchanged. No product code in this fold. [Living Confirm](../../discussions/2026-09-01-p2-closeout-1-4-quantified-dna.md). Overall P2 unsigned. Not IC50.
...
| **RQ-WO-2** | Routing map keys: **TAT day range + ordered `process_definition[]` + named asked-for LimsRun slot**. No free-form analysis or sample-type picker; the asked-for slot must identify one LimsRun already present in the ordered route. A route may contain multiple LimsRun analyses. Extract is an experiment and cannot be the slot. Route snapshots the ordered list and slot, **zero Tests**. First Start instantiates process[0] / `chain[0]` only. |
| **RQ-WO-3** | Mutate routing map = **`config:edit` only**. Empty map yields zero acceptable routes: **422**, no mint. |
| **RQ-WO-4** | Map save **409**s only when overlapping TAT, overlapping first-step allow-lists, and overlapping **asked-for-slot analyses** all hold. Supporting analysis-set overlap does not make two maps equivalent. Route **409**s when two saved rows both accept this type and their named slots match the asked-for analysis. No silent `first()`. |
...
| **RQ-WO-6** | Tech hits **Route**; asked-for save does not mint work. Eligible maps = TAT + first-step type + `asked.analysis_id == named_slot.analysis_id`. A route containing the analysis only as supporting QC is **not** eligible. Missing/invalid named slot or zero LimsRuns → **422**. Exactly one acceptable row snapshots the ordered list and named slot, mints one work order and zero Tests, and sets asked-for `routed`; two acceptable rows → **409**. One process instance remains bound to one asked-for row. |
...
| **RQ-WO-9** | Parser is chosen at import. Extract is an experiment with no `analysis_id` or Result. For Quantified DNA, the named asked-for slot uses the existing **Qubit** catalog analysis once; do not create a Quantified DNA analysis. Nanodrop/other QC uses another `analysis_id` and Test. For WGS/WES/ELISA, Qubit remains supporting process QC and cannot make that map eligible for a Qubit asked-for unless Qubit is the map’s named asked-for slot. |
...
26. Teaching extract as a LimsRun, giving extract an `analysis_id`/boolean Result, allowing zero LimsRuns for Quantified DNA, creating a second Quantified DNA catalog analysis, or matching Route by any chain containment instead of the named asked-for LimsRun slot.
