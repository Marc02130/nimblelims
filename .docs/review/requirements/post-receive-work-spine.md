...
**Date:** 2026-08-28  
**Status:** Spec **Accept with conditions** on P2 (`feat/work-order-p2`). **P1 shipped** on `main` (PR 81; UAT Pass). **Hold product merge to main.** **Hold merge until QA clicks C1/C2.** Signed AC-P2-9..11 history: `9342439`. Product SHA for `0077` assignment slices: `4671ba8` (0077 in that commit). AC-P2-C1/C2 **unsigned**. Assignment is a Contents pair (`container_id` NOT NULL). Later Start follows dest vessels (`_continuing_assignments`), not inbound parent. Dest mint **Hold**: Start extract does not mint a DNA daughter; P2 follow is a same-sample dest **container**. Freeze skip **OPEN**. **OQ-WO-6 stays OPEN**. Route stays `test:assign`. Round 2 **Leadership Confirm** (Rolf/Deiter/Hans/Heidi/Günter; R2-1…R2-4). **Overall P2 unsigned.** Send: [2026-08-30-p2-route-lock](../../discussions/2026-08-30-p2-route-lock.md). `8cfa2a9` / `b005cfe` signed history. Not IC50.
...
15. **0077 assignment / dest follow (`02fe95f` — Heidi / Mathilda / Rolf):** `eln_process_samples.container_id` required. Assign **422** if no vessel or ambiguous multi-vessel (lab-readable). Omit `container_id` is 422 unless exactly one vessel. Aliquot/pool execute: dest container-with-sample stays on the process; inbound source `removed`. Later Start follows non-`removed` dest assignments (`_continuing_assignments`), not bare `wo.sample_id` when dests exist. **Punch dest mint:** `_execute_transfer` still inserts a new Sample with `dest_sample_type` (can be DNA) and `_join_minted_destination` puts that row on the process. Parent `sample_type` is not rewritten. Hold: **no new DNA row, no type rewrite.** P2 follow is a **same-sample dest container**. OQ-WO-6 and freeze skip stay OPEN. Route stays `test:assign`. Hold merge until QA clicks C1/C2.
...
| **RQ-WO-12** | **0077:** Assignment is a Contents pair. `eln_process_samples.container_id` **NOT NULL**. Assign **422** if no vessel or 2+ without a pick (lab-readable). Omit `container_id` is 422 unless exactly one vessel. Aliquot/pool execute keeps dest container-with-sample on the process and `removed`s inbound source. Dest mint **Hold**: P2 extract / Start does **not** mint a DNA daughter; P2 follow is a **same-sample dest container**. AC-P2-C1/C2 **unsigned** (`4671ba8`). |
...
| AC-P2-C1 | Assign: `container_id` required. 0 vessels or 2+ without a pick → **422**, lab-readable |
| AC-P2-C2 | Later Start follows dest containers on the process, not the inbound parent. No dest mint (no new DNA row / no dest-type rewrite) |
