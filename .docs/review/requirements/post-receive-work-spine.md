...
16. **Live `1572071` (unsigned until Tobias):** dest-follow in the execute txn (`_follow_destination_in_process`; retarget `container_id` or remove source then insert dest; does not require `entry.process_step_id`); emptied-source assign **422**; equivalent aliquot = same sample, new container; later Start follows dest container. In code on this SHA; not QA-clicked. Do **not** write C2 Pass. Do **not** teach dest-follow as shipped. Dest mint Hold stays Hold. Send: [2026-08-30-p2-route-lock](../../discussions/2026-08-30-p2-route-lock.md), “Honesty fold — dest-follow on `1572071`”.
...
| **RQ-WO-12** | **0077 C2 Fail (`02fe95f`, Deiter click; Leadership Confirm — not unsigned) is signed history.** Assignment is the **tube in hand**. C1 **Pass**: **422** if no vessel or 2+ without a pick; receive-tube **201**. C2 **Fail**: dest not on the process; execute never wrote the same-sample dest container onto `eln_process_samples`; join/release no-op unless `entry.process_step_id`; emptied-source assign **201** is leftover amount-0 Contents / leftover process-join; later Start rode the emptied parent; **PATCH is not a path**. Dest mint Hold **Pass** is a different punch (still Blood, **0 DNA**). Live lock on **`1572071`** (expect, not Result): dest-follow in the execute txn (`_follow_destination_in_process`); emptied-source assign **422**; equivalent aliquot = same sample, new container. **Unsigned** until Tobias. Do not write C2 Pass. |
...
| AC-P2-C2 | **Fail** on `02fe95f` (Deiter signed history). Live on **`1572071`**: **unsigned** until Tobias. **Expect (not Result Pass):** equivalent aliquot = same sample, new container; dest-follow in the **execute txn**; emptied-source assign **422**; later Start follows dest **container**; PATCH is not a path. Dest mint **Hold** is **not** this AC. Do not write Pass. |
