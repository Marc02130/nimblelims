...
| **RQ-WO-11** | **L3 / SC5 / A5:** Asked-for `params` are **order capture**. At **LimsRun start** for the asked-for analysis, insert Test if missing; copy `asked_for.params` → `tests.asked_for_params` (jsonb) and **freeze**. Skip later overwrite only when a snapshot **already exists** (including frozen `{}`). A classic `/tests` row with NULL or default `{}` is **not** a freeze — first start must **write**. Extract LimsRun must not share the asked-for `analysis_id`. Empty defs freeze as `{}`. Not receive, not publish, not result columns. P1 does not write the Test snapshot. Do not teach skip-on-existing-Test as shipped. Live AC-P2 unsigned. |
