...
8. **P2 ordered route (Leadership / Heidi / Günter; AC-P2-9..11 Pass on `9342439`; prior signed on `8cfa2a9`):** Route snapshots the ordered list, **zero Tests**. First Start = process[0] / `chain[0]` only — **Tobias-signed Pass** (`8cfa2a9`). Later Start = next process, on the sample that exists then. Empty Route **422** Pass. Map overlap **409** / blood+qPCR **201** (not AND) Pass (`8cfa2a9`). Later-step type-gate **Met** on `9342439` (qPCR-on-blood / still-Blood **422**; not dest-type E2E). Dest-type Hold out.
