...
**Full Leadership Confirm #2 (2026-09-03, Rolf / Deiter / Hans / Heidi / Günter):** Keep the Marc locks for **no route branching** and **asked-for only** (PR 111). WGS asked-for on blood owns WGS params; C3 DNA still serves WGS; a C2 aliquot continues WGS; WES is a new asked-for on the DNA tube. Tobias Seq-1 Pass of two WOs is not this Confirm. The older “Extracted DNA = DNA tube; zero assay LimsRuns legal” clause is **struck**. OQ-WO-8 Closed history from PR 119 stands; this Confirm does not recode or restamp OQ-WO-7 and is not overall P2 Pass.
...
## Full Leadership Confirm #2 and named-slot product brief — 2026-09-03

**Full Leadership Confirm** (Rolf / Deiter / Hans / Heidi / Günter). This is a product brief, not a UAT Result and not overall P2 Pass.

1. Keep **no route branching**: WGS on blood owns WGS params; C3 DNA and its C2 aliquot continue WGS; WES is a new asked-for on the DNA tube. Seq-1 Pass of two WOs is not this Confirm.
2. Keep **asked-for only** from PR 111.
3. Strike the remaining zero-LimsRun Extracted DNA clause. Quantified DNA wears existing Qubit; no second Quantified DNA analysis.
4. Named-slot eligibility comes first: `asked.analysis_id` must equal the persisted author-named LimsRun slot’s `analysis_id`. Containment anywhere in the chain is not eligibility.
5. After that filter: 0 maps → 422; 1 map → Route mints; 2+ maps → tech chooses a route, then posts `routing_map_id` and mints. No silent `first()`.
...
### Merge-gate restamp — product `6a67667` — 2026-09-04

`6244bf6` was rebased onto `8887e36`. The named-slot product SHA that must land is **`6a67667`** (`6a67667b4d6378877a38e5542077df316a35077f`), Alembic **`0079`**. **AC-P2-OQ-WO-8 Pass** (Tobias QA merge-gate restamp, 2026-09-04 20:18 ET, click SHA **`effd242`**): one named Qubit slot minted; map B (WGS named, Qubit as process QC) alone returned **422** and did not steal; zero acceptable maps returned **422**; 2+ returned **409** `route_pick_required` and minted only on the posted pick. Map A and map B shared first-step DNA type and overlapping TAT, so the proof is the slot.
...
| **RQ-WO-2** | Routing map keys: **TAT day range + ordered `process_definition[]` + one author-named asked-for LimsRun slot**. No sample-type picker. Persist the selected `eln_process_definition_steps` LimsRun reference on `routing_map` (preferred), or persist the author-named ask as `routing_map.analysis_id`; do not derive it from `analyses[0]`. A route may contain multiple LimsRun analyses. Map-save **422** if the named slot’s analysis appears 0 or 2+ times among route LimsRuns. Extract (process) and Qubit (supporting LimsRun) may sit in a WGS chain without making Qubit the asked-for slot. Route snapshots the ordered list, **zero Tests**. First Start instantiates process[0] / `chain[0]` only. Historical execute Results remain unchanged. |
| **RQ-WO-3** | Mutate routing map = **`config:edit` only**. Empty map yields zero acceptable routes: **422**, no mint. |
| **RQ-WO-4** | Map save overlap still considers overlapping TAT, overlapping first-step allow-lists, and named asked-for analyses. Two extract routes, same TAT and inbound types, ELISA vs NGS, both save. At Route, filter by TAT + current type + `asked.analysis_id == named_slot.analysis_id`: 0 acceptable maps → **422**; 1 → mint; 2+ → return candidates for **manual assignment**. The tech picks one route and posts its `routing_map_id`; only then mint. No silent `first()`. This supersedes the older “409 only / no picker” product teaching. |
...
| **RQ-WO-6** | Tech hits **Route**; asked-for save does not mint work. Eligibility is TAT + first-step type + `asked.analysis_id` matching the map’s **named asked-for LimsRun slot**, not containment anywhere in the chain. A WGS map whose Qubit LimsRun is process QC must not accept a Quantified DNA ask. Exactly one acceptable map snapshots the ordered list, mints one work order, zero Tests, and sets asked-for `routed`. Two or more acceptable maps require manual route assignment and mint only after the tech posts the chosen `routing_map_id`. P1 never writes `routed`. A process instance remains bound to one asked-for row. |
...
| AC-P2-3 | Two or more named-slot-matching rows show a route picker; posting the selected `routing_map_id` mints exactly one work order. No silent `first()` and no mint before selection. |
