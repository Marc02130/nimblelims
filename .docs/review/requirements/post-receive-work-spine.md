...
**Date:** 2026-08-28  
**Status:** Spec **Accept with conditions** on P2 @ `02fe95f` (tip `4671ba8` is extra internal docs). **P1 shipped** on `main` (PR 81; UAT Pass). **Hold merge until QA clicks C1/C2.** Assignment is a Contents pair (`container_id` NOT NULL). Later Start follows dest vessels (`_continuing_assignments`), not inbound parent. **Punch dest mint:** `_execute_transfer` still inserts a new Sample with `dest_sample_type` (can be DNA). Hold: no new DNA row, no type rewrite. P2 follow is a same-sample dest **container**. OQ-WO-6 and freeze skip stay OPEN. Route stays `test:assign`. Live SHA `9342439` remains Tobias-signed AC-P2-9..11 Pass (not a merge vote). **Overall P2 unsigned.** Not IC50.
...
8. **P2 ordered route (Leadership / Heidi / Günter; AC-P2-9..11 Pass on `9342439`; prior signed on `8cfa2a9`):** Route snapshots the ordered list, **zero Tests**. First Start = process[0] / `chain[0]` only — **Tobias-signed Pass** (`8cfa2a9`). Later Start = next process, on dest vessels that exist then. Empty Route **422** Pass. Map overlap **409** / blood+qPCR **201** (not AND) Pass (`8cfa2a9`). Later-step type-gate **Met** on `9342439` (qPCR-on-blood / still-Blood **422**; not dest-type E2E). Dest-type Hold out.
...
| **RQ-WO-7** | First Start = process[0] / `chain[0]` only (`experiment:manage`). Later Start = next process, on dest vessels still assigned (`_continuing_assignments`); not inbound parent / bare `wo.sample_id` when dests exist. Bounce **first Start minting later processes or their Tests**. Route is `test:assign`. **P2-4:** Route must **read** the mapped def/steps catalog-visible. Do not require `experiment:manage` on Route. **`0074` is not catalog-visible.** Mutate stays `config:edit`. |
| **RQ-WO-12** | **0077:** Assignment is a Contents pair. `eln_process_samples.container_id` **NOT NULL**. Assign **422** if no vessel or 2+ without a pick (lab-readable). Omit `container_id` is 422 unless exactly one vessel. Aliquot/pool execute keeps dest container-with-sample on the process and `removed`s inbound source. **Punch dest mint:** `_execute_transfer` must **not** insert a new Sample / DNA row / dest-type rewrite. P2 follow is a **same-sample dest container**. |
...
| AC-P2-7 | First Start = process[0] only, no Tests for later processes. Later Start = next process, on dest vessels that exist then |
