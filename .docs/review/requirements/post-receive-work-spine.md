...
| AC-P2-OQ-WO-8 | Quantified DNA matches the map’s named Qubit slot; WGS+Qubit-as-QC does not match; 2+ acceptable named-slot maps show a picker and mint only after the tech posts one selected `routing_map_id`. Result is unsigned until Tobias runs it on the named-slot product SHA. |
