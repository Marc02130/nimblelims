...
8. **P2 ordered route (Leadership / Heidi / Günter; per-AC signed on `8cfa2a9`):** Route snapshots the ordered list, **zero Tests**. First Start = process[0] / `chain[0]` only — **Tobias-signed Pass**. Later Start = next process, on the sample that exists then. Empty Route **422** Pass. Map overlap **409** / blood+qPCR **201** (not AND) Pass. Later-step type-gate **unsigned**. Dest-type Hold out.
...
14. **After `8cfa2a9` (CEO Confirm; pending Deiter/Hans/Heidi/Günter overwrite-or-confirm):** no map analysis picker. Route matches a LIMS Run **in the chain**. Map 409 = TAT ∩ first-step types ∩ LIMS Run analyses. Map 422 when process *x* emerging type is not accepted by *x+1*. Dest-type mint Hold. Live SHA `9342439`. Send: [2026-08-30-p2-route-lock](../../discussions/2026-08-30-p2-route-lock.md). Not full Leadership Accept.
