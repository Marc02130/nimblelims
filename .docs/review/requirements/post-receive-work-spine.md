...
**Date:** 2026-08-28  
**Status:** Spec **Accept with conditions** on P2 (`feat/work-order-p2`). **P1 shipped** on `main` (PR 81; UAT Pass). **Hold product merge to main.** Signed AC-P2-9..11 history: `9342439`. Product SHA for `0077` assignment slices: `4671ba8` (0077 in that commit). AC-P2-C1/C2 **unsigned**. Restamp notes honesty, **not** a merge vote: later-step type-gate **Met**; dest mint **Hold**; freeze skip **OPEN**; **OQ-WO-6 stays OPEN** (Leadership Confirm R2-3: earlier LimsRun must not share asked-for `analysis_id`; do not teach extract-as-special); Route stays `test:assign`; **overall P2 unsigned**. Round 2 **Leadership Confirm** (Rolf/Deiter/Hans/Heidi/Günter; R2-1…R2-4). A route **may have multiple analyses**; asked-for matches any chain that **contains** that analysis. Parser at import. Send: [2026-08-30-p2-route-lock](../../discussions/2026-08-30-p2-route-lock.md). `8cfa2a9` / `b005cfe` signed history. Not IC50.
...
8. **P2 ordered route (Leadership / Heidi / Günter; AC-P2-9..11 Pass on `9342439`; prior signed on `8cfa2a9`):** Route snapshots the ordered list, **zero Tests**. First Start = process[0] / `chain[0]` only — **Tobias-signed Pass** (`8cfa2a9`). Later Start = next process on the continuing **dest** container-with-sample after aliquot/pool execute (inbound source `removed`); not the parent vessel. Empty Route **422** Pass. Map overlap **409** / blood+qPCR **201** (not AND) Pass (`8cfa2a9`). Later-step type-gate **Met** on `9342439` (qPCR-on-blood / still-Blood **422**; not dest-type E2E). Dest-type Hold out. `0077` at `4671ba8`; AC-P2-C1/C2 **unsigned**.
...
| **P2** | Routing + work_order | Test ordering / processing | Architecture / UI / Spec Accept with conditions @ `8cfa2a9`. Live SHA `9342439` — **Tobias-signed AC-P2-9..11 Pass**; Leadership restamp notes (Deiter/Hans/Heidi/Günter, **not** a merge vote): later-step **Met**; AC-P2-11 map-save only; dest mint Hold. Overall P2 **unsigned**. **Hold product merge to main.** First Start = chain[0] only **Pass**. Still open: Hans freeze (`{}` ambiguous), extract `analysis_id`, P2-4 `0074`. |
...
| **RQ-WO-7** | First Start = process[0] / `chain[0]` only (`experiment:manage`). Later Start = next process on dest container-with-sample after execute; bounce later Start following the parent/source tube. Bounce **first Start minting later processes or their Tests**. Route is `test:assign`. **P2-4:** Route must **read** the mapped def/steps catalog-visible. Do not require `experiment:manage` on Route. **`0074` is not catalog-visible.** Mutate stays `config:edit`. |
...
- Extract-hold dest type / dest **mint** (own stem: aliquot/pool OOB entry submit). P2 must not mint daughters on Route or process Start. P4 must not pretend mint is done.
