...
| AC-P2-C2 | **Fail** on `02fe95f` (Deiter history). **Unsigned** (numbered on `570bbc0`; execute `1572071`): same dest type → same sample, new container; dest on process; inbound assignment `removed`; Later Start follows dest container. Leftover inbound volume is whatever was not transferred — emptying is not required. Amount 0 on that tube is an edge (**422**), not the AC. Not Blood→DNA. |
