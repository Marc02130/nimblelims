...
16. **Live `1572071` (unsigned until Tobias):** AC-P2-C2 is **same-type dest-follow only**. Same dest type = same sample, additional container; `_follow_destination_in_process` retarget is that path; emptied-source assign **422**; later Start follows that dest container. Different dest type = new derivative sample in a new container (`parent_sample_id`); parent assignment stays and the dest is the new sample+container. Dest mint Hold is lifted only for type-changing execute. In code on this SHA; not QA-clicked. Do **not** write C2 Pass or teach type-changing dest as parent `container_id` retarget. Send: [2026-08-30-p2-route-lock](../../discussions/2026-08-30-p2-route-lock.md), “Honesty fold — dest-follow on `1572071`”.
...
| AC-P2-C2 | **Fail** on `02fe95f` (Deiter signed history). Live on **`1572071`**: **unsigned** until Tobias; same-type dest-follow only. **Expect (not Result Pass):** same dest type = same sample, additional container; `_follow_destination_in_process` retargets that same-sample assignment; emptied-source assign **422**; later Start follows that dest container. Different dest type mints a derivative sample with `parent_sample_id`; parent stays and is not retargeted. PATCH is not a path. Do not write Pass. |
