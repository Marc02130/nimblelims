...
**Date:** 2026-08-28  
**Status:** Spec **Accept with conditions** on P2 (`feat/work-order-p2`). **P1 shipped** on `main` (PR 81; UAT Pass). **Hold product merge to main.** **Hold merge until QA clicks C1/C2.** Signed AC-P2-9..11 history: `9342439`. Product SHA for `0077` assignment slices: `4671ba8` (0077 in that commit). AC-P2-C1/C2 **unsigned**. Assignment is a Contents pair (`container_id` NOT NULL) — the tube in hand; **Contents grain Leadership Confirm** (Rolf/Deiter/Hans/Heidi/Günter). Later Start follows dest vessels (`_continuing_assignments`) by **container**, not inbound parent and not dest type. Dest mint **Hold**: `_execute_transfer` still mints a Sample with `dest_sample_type`; Start extract does not mint a DNA daughter; P2 follow is an **equivalent aliquot** — same sample, new **container**. Freeze skip **OPEN**. **OQ-WO-6 stays OPEN**. Route stays `test:assign`. Round 2 **Leadership Confirm** (Rolf/Deiter/Hans/Heidi/Günter; R2-1…R2-4). **Overall P2 unsigned.** Send: [2026-08-30-p2-route-lock](../../discussions/2026-08-30-p2-route-lock.md). `8cfa2a9` / `b005cfe` signed history. Not IC50.
...
8. **P2 ordered route (Leadership / Heidi / Günter; AC-P2-9..11 Pass on `9342439`; prior signed on `8cfa2a9`):** Route snapshots the ordered list, **zero Tests**. First Start = process[0] / `chain[0]` only — **Tobias-signed Pass** (`8cfa2a9`). Later Start = next process on dest container-with-sample after aliquot/pool execute (inbound source `removed`); not the parent vessel. Empty Route **422** Pass. Map overlap **409** / blood+qPCR **201** (not AND) Pass (`8cfa2a9`). Later-step type-gate **Met** on `9342439` (qPCR-on-blood / still-Blood **422**; not dest-type E2E). Dest-type Hold out. `0077` at `4671ba8`; AC-P2-C1/C2 **unsigned**.
...
13. **Earlier LimsRun `analysis_id` (OQ-WO-6 stays OPEN — Leadership Confirm R2-3; Rolf/Deiter/Hans/Heidi/Günter):** Earlier LimsRun must **not** share asked-for `analysis_id`. Do not teach extract-as-special-assay. Type gates catch blood-on-Qubit. Blood→DNA is a derivative (dest mint Hold). WO-7 mints Test at every LimsRun start for `(cohort sample, run.analysis_id)`. Any **earlier** LimsRun that reuses the asked-for panel analysis freezes that Test on the parent. Earlier LimsRun = own analysis or experiment-only.
14. **After `8cfa2a9` (Round 1 Leadership Confirm — Rolf/Deiter/Hans/Heidi/Günter; Round 2 Leadership Confirm R2-1…R2-4):** no map analysis picker. A route **may have multiple LimsRun analyses**. Asked-for assay → any route that **contains** that analysis. Map 409 = TAT ∩ first-step types ∩ LIMS Run analysis **sets**. Map 422 when process *x* emerging type is not accepted by *x+1* (**map-save only**; dest mint Hold). Live SHA `9342439` — **Tobias-signed AC-P2-9..11 Pass**; restamp notes honesty, **not** a merge vote. Freeze skip OPEN; OQ-WO-6 stays OPEN; parser at import; Route stays `test:assign`; overall P2 unsigned. Send: [2026-08-30-p2-route-lock](../../discussions/2026-08-30-p2-route-lock.md).
15. **0077 assignment / dest follow (`02fe95f` — Heidi / Mathilda / Rolf; Contents grain **Leadership Confirm** 2026-08-30, Rolf/Deiter/Hans/Heidi/Günter):** Assignment is the **tube in hand** — `eln_process_samples.container_id` required. Assign **422** if no vessel or ambiguous multi-vessel (lab-readable); **no silent pick**. Omit `container_id` is 422 unless exactly one vessel. **Equivalent aliquot** = same sample, new container: dest container-with-sample stays on the process; the emptied inbound source is `removed`. Later Start follows non-`removed` dest assignments (`_continuing_assignments`) by **container**, not by dest **type**, and not bare `wo.sample_id` when dests exist. **Punch dest mint:** `_execute_transfer` still inserts a new Sample with `dest_sample_type` (can be DNA) and `_join_minted_destination` puts that row on the process — that is **dest mint at execute**, a write of identity, and it is **not** the equivalent aliquot AC-P2-C2 teaches. Parent `sample_type` is not rewritten. Hold: **no new DNA row, no type rewrite.** Coding stays **Grok Build** — do not claim equivalent-aliquot code closed while `_execute_transfer` mints a Sample. OQ-WO-6 and freeze skip stay OPEN. Route stays `test:assign`. Hold merge until QA clicks C1/C2. Send: [2026-08-30-p2-route-lock](../../discussions/2026-08-30-p2-route-lock.md), “Contents grain / `0077`”.
...
| **P2** | Routing + work_order | Test ordering / processing | Architecture / UI / Spec Accept with conditions @ `02fe95f`. **Hold merge until QA clicks C1/C2.** Punch dest mint open. OQ-WO-6 and freeze skip stay OPEN. `9342439` AC-P2-9..11 Pass stays signed history. Overall P2 **unsigned**. |
...
| **RQ-WO-5** | Type gate is on process-definition steps for both `eln_experiment` and `lims_run`. Route compares current type with process[0]’s **first** Exp/LimsRun allow-list. Zero acceptable → **422** `route_sample_type`. Later processes/steps gate current type only when started; empty fails closed then. **Dest mint is not routing/process:** aliquot/pool OOB entry **submit** creates sample + container (`extract-hold-dest-type`). P2 only **reads** declared dest for map-save handoff 422. |
...
| **RQ-WO-7** | First Start = process[0] / `chain[0]` only (`experiment:manage`). Later Start = next process on dest container-with-sample after execute (`_continuing_assignments`); bounce later Start following the parent/source tube or bare `wo.sample_id` when dests exist. Bounce **first Start minting later processes or their Tests**. Route is `test:assign`. **P2-4:** Route must **read** the mapped def/steps catalog-visible. Do not require `experiment:manage` on Route. **`0074` is not catalog-visible.** Mutate stays `config:edit`. |
| **RQ-WO-12** | **0077 (Contents grain — Leadership Confirm; Rolf/Deiter/Hans/Heidi/Günter):** Assignment is the **tube in hand**, a Contents pair. `eln_process_samples.container_id` **NOT NULL**. Assign **422** if no vessel or 2+ without a pick (lab-readable); **no silent pick**, never `first()`. Omit `container_id` is 422 unless exactly one vessel. **Equivalent aliquot** (same sample, new container) keeps the dest container-with-sample on the process and `removed`s the emptied inbound source; later Start follows the dest **container**, not a dest **type**. Dest mint **Hold**: `_execute_transfer` still inserts a new Sample with `dest_sample_type` — that is a write of identity, not the equivalent aliquot; P2 extract / Start does **not** mint a DNA daughter and does not rewrite `sample_type`. Test stays `(sample, analysis)`; the container is which vessel was measured and takes the conc write-through. AC-P2-C1/C2 **unsigned** (`4671ba8` / `02fe95f`). |
...
- Extract-hold dest type / dest **mint** (own stem: aliquot/pool OOB entry submit). P2 must not mint daughters on Route, process Start, **or aliquot/pool execute**. P4 must not pretend mint is done.
...
27. Dest mint at execute (new DNA Sample / dest-type rewrite / `_join_minted_destination` of a new row). P2 follow is a same-sample dest **container**.
28. Assign that omits `container_id` when vessel count is 0 or 2+. Later Start that follows inbound parent / `wo.sample_id` when dest assignments exist.
...
| AC-P2-7 | First Start = process[0] only, no Tests for later processes. Later Start = next process on dest container-with-sample after execute |
...
| AC-P2-C1 | Assign is the tube in hand: `container_id` required. 0 vessels or 2+ without a pick → **422**, lab-readable, no silent pick. Receive tube assign → **201** with `container_id` |
| AC-P2-C2 | **Equivalent aliquot** = same sample, new container. Dest container-with-sample stays on the process; emptied inbound source comes off. Later Start follows the dest **container**, not the inbound parent and not a dest **type**. No new sample identity, no `sample_type` rewrite. A new Sample minted with `dest_sample_type` (`_execute_transfer`) is dest mint **Hold** — **not** this AC |
