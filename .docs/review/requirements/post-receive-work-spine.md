...
**Date:** 2026-08-28  
**Status:** Spec **C2 Fail** on `02fe95f` is signed history (Deiter C1 **Pass**, dest mint Hold **Pass**). Leadership Confirmed that click — C1/C2 are **not** unsigned. **P1 shipped** on `main` (PR 81; UAT Pass). **Hold product merge.** Docs Confirm `84d2810` is not a new execute and not the click SHA. Live product SHA **`1572071`**: AC-P2-C2 covers **same-type dest-follow only** and remains **unsigned** until Tobias. Same dest type = **same sample, additional container**; `_follow_destination_in_process` follows that path. Different dest type = **new derivative sample** in a new container (`parent_sample_id`); the parent **Sample row** stays with its original type and parent-type work. For both grains, execute puts the destination sample + destination container pair on `eln_process_samples` and marks the inbound source assignment `removed` in the same transaction. Dest mint Hold is lifted only for type-changing execute. Do **not** write C2 Pass, rewrite the parent `sample_type`, or retarget the parent Sample onto the destination tube. **PATCH is not a path.** Deiter’s dest mint Hold **Pass** is Start-extract Blood / **0 DNA** history. OQ-WO-6 and freeze skip stay OPEN. Signed AC-P2-9..11: `9342439`. **Overall P2 unsigned / not Pass.** Not IC50.
...
16. **Live `1572071` (unsigned until Tobias):** AC-P2-C2 is **same-type dest-follow only**. Same dest type = same sample, additional container; `_follow_destination_in_process` may retarget `container_id` only on that path; emptied-source assign **422**; later Start follows that dest container. Different dest type = new derivative sample in a new container (`parent_sample_id`); only the parent Sample row stays, preserving its original type and parent-type work. For both grains, the destination sample + destination container pair lands on `eln_process_samples` and the inbound source assignment becomes `removed` in the execute transaction. Dest mint Hold is lifted only for type-changing execute. In code on this SHA; not QA-clicked. Do **not** write C2 Pass or teach type-changing dest as parent `container_id` retarget. Send: [2026-08-30-p2-route-lock](../../discussions/2026-08-30-p2-route-lock.md), “Honesty fold — dest-follow on `1572071`”.
...
| **P2** | Routing + work_order | Test ordering / processing | Spec **C2 Fail** on `02fe95f` is signed history. Deiter C1 **Pass**, dest mint Hold **Pass**. Live SHA **`1572071`**: dest-follow in execute txn, AC-P2-C2 **unsigned**. OQ-WO-6 and freeze skip stay OPEN. `9342439` AC-P2-9..11 Pass stays signed history. Overall P2 **unsigned**. |
...
- Extract-hold dest type / dest **mint** as a P2 C2 result (own stem: aliquot/pool OOB entry submit). Route and process Start do not mint daughters; only type-changing execute may mint the derivative. P4 must not present that OOB execute as UAT-verified.
...
| AC-P2-7 | First Start = process[0] only, no Tests for later processes. Later Start following dest container after execute is the lock on **`1572071`**, **unsigned** until Tobias |
...
| AC-P2-C2 | **Fail** on `02fe95f` (Deiter signed history). Live on **`1572071`**: **unsigned** until Tobias; same-type dest-follow only. **Expect (not Result Pass):** same dest type = same sample, additional container; `_follow_destination_in_process` follows that same-sample assignment; emptied-source assign **422**; later Start follows that dest container. Different dest type mints a derivative sample with `parent_sample_id`; the parent Sample row stays with its original type and is not retargeted. Both grains put the destination sample + container pair on the process and mark the inbound source assignment `removed` in the execute transaction. PATCH is not a path. Do not write Pass. |
...
Script: `UAT_Scripts/uat-post-receive-work-spine.md` (P1 cases). **P1 Pass**; merged PR 81. Live product SHA **`1572071`**: AC-P2-C2 **unsigned**. Do not use retired `uat-sample-accessioning.md`.
