...
10. **Freeze (in code on `8cfa2a9`, unsigned until QA):** first LimsRun start wins. Do not overwrite `asked_for_params` on an existing Test (including `{}`). Not shipped Pass.
