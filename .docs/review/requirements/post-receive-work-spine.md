...
16. **Leadership Confirm — dest-type split; live `570bbc0` C2 and C3 remain unsigned until Tobias (`1572071` is the execute-txn implementation SHA):** same dest type = same sample, additional container; `_follow_destination_in_process` may retarget `container_id` only on that path. Emptied-source assign is **422** and later Start follows that dest container. Different dest type = new derivative Sample in a new container (`parent_sample_id`); only the parent Sample row stays, preserving its original type, Tests, and parent-type work. Type-changing execute mints and joins the destination pair and marks the inbound source assignment `removed` in the execute transaction; only the destination pair remains on this chain. Route / Start / map-save mint zero daughters. Dest mint Hold is lifted only for type-changing execute. C2 (same dest type) and C3 / extract-hold UAT 1.7 (different dest type) remain two clicks. In code since `1572071`; not QA-clicked on `570bbc0`. Do **not** write C2 or C3 Pass, and do not teach type-changing dest as parent `container_id` retarget. Send: [2026-08-30-p2-route-lock](../../discussions/2026-08-30-p2-route-lock.md), “Leadership Confirm — dest-type split”.
...
| **P2** | Routing + work_order | Test ordering / processing | Spec **C2 Fail** on `02fe95f` is signed history. Deiter C1 **Pass**, dest mint Hold **Pass**. Live SHA **`570bbc0`**: dest-follow in execute txn (implementation `1572071`), AC-P2-C2 and AC-P2-C3 **unsigned**. OQ-WO-6 and freeze skip stay OPEN. `9342439` AC-P2-9..11 Pass stays signed history. Overall P2 **unsigned**. |
...
28. Assign that omits `container_id` when vessel count is 0 or 2+. Emptied-source assign **201** (Contents amount 0) — leftover of `02fe95f`; live lock on `570bbc0` is **422**. Later Start that follows inbound parent / `wo.sample_id` when dests exist. PATCH of `eln_process_samples` as dest-follow. Join/release that no-ops without `entry.process_step_id` on `02fe95f`.
...
| AC-P2-7 | First Start = process[0] only, no Tests for later processes. Later Start following dest container after execute is the lock on **`570bbc0`**, **unsigned** until Tobias |
...
| AC-P2-C2 | **Fail** on `02fe95f` (Deiter history). Live **`570bbc0` unsigned**: same dest type → same sample, new container; dest on process; emptied source **422**; Later Start follows dest container. Not Blood→DNA. |
| AC-P2-C3 | **Unsigned** on `570bbc0`: dest type DNA → new Sample + container (`parent_sample_id`); parent stays Blood; dest pair is the only active process assignment. Same click as extract-hold 1.7. |
...
Script: `UAT_Scripts/uat-post-receive-work-spine.md` (P1 cases). **P1 Pass**; merged PR 81. Live product SHA **`570bbc0`**: AC-P2-C2 and AC-P2-C3 **unsigned**. Do not use retired `uat-sample-accessioning.md`.
