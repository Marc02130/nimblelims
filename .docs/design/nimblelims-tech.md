...
- Version 2.2: Added Experiment Management (Chunk 1): experiment_templates, experiments, experiment_details, experiment_sample_executions tables; API under /v1/experiments and /v1/experiment-templates (CRUD, link_sample_to_experiment, add_experiment_detail_step, link_experiments, get_experiment_lineage, get_sample_experiments, list with mine=true); permission experiment:manage; workflow actions (create_experiment, create_experiment_from_template, link_sample_to_experiment, add_experiment_detail_step, link_experiments, update_experiment_status). Frontend: dedicated Experiments accordion in sidebar (All Experiments, Experiment Templates); routes /experiments, /experiments/:id, /experiments/templates; ExperimentsManagement list/detail, My Experiments filter (?mine=true), bidirectional sample↔experiment links, lineage view; back button for experiment detail. See .docs/design/experiment-planning.md and .docs/manuals/navigation.md (February 2025).
- Version 2.3: Experiment Templates management UI and SOP parse pipeline: page `ExperimentTemplatesManagement` at /experiments/templates (experiment:manage); multipart POST /v1/sop-parse, GET job, POST apply (Claude extraction via ANTHROPIC_API_KEY); mandatory review sign-off and activation guard in UI; API docs in .docs/manuals/api-endpoints.md. Sidebar Templates sub-item uses experiment:manage (same as All Experiments). See .docs/design/experiment-planning.md Chunk 2 (April 2026).
...
- **Frontend**: React app with Axios for API calls; React Router for navigation; Material-UI or similar for components. Sidebar navigation: unified accordions (Core, Sample Mgmt, **Experiments**, Lab Mgmt, Admin); Experiments section has sub-items All Experiments and Experiment Templates (both require experiment:manage).
...
**This file (`.docs/design/nimblelims-tech.md`) modified:** Version history 2.2 added for Experiment Management (Chunk 1) and Experiments accordion; Frontend architecture bullet updated to mention Experiments sidebar section. No other sections changed.