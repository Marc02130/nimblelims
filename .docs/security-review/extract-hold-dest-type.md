...
**S1 Met.** **S3** (`config:edit` on catalog mutate) is an implement condition — fold into sketch with Wilhelmina; gate **OPEN** for implement with S3 + Lab Ops L2 + seed.
...
| **S3** | High | **Implement condition** | `sample_type_transitions` mutate is **`config:edit` only**. Not Client, not `experiment:manage` alone. Execute refuse stays integrity. Fold into sketch. |
