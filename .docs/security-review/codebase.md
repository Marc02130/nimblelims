...
**Scope:** Feature packet (STRIDE) over HEAD `6a21c947`, plus Tobias live UAT of `security/high-s1-s6` @ `d97e756`. DEEP CSO: skipped.
...
Tobias live UAT + pytest (2026-08-21) on `security/high-s1-s6` @ `d97e756`: **S2, S3, S4, S6 Met.** **S1 Met with residual.** **S5 refuse Met.** Residual **Sec9:** labtech execute 500 on `INSERT containers` (fail-closed, not 403). Admin execute works.
...
| RLS / `lims_app` | Isolation smoke held. |
...
|----|----------|--------|-----------|
| S1 | High | **Met with residual** | `lims_app` not Superuser; Client B 404 on Client A sample. |
