...
# ID configuration Admin Page
##1. Prompt for Name Templates Admin Page
"Generate a React functional component named NameTemplatesAdmin for an admin page at path '/admin/name-templates'. It should manage name templates with CRUD operations. Include:
A list view using a DataTable component showing columns: entity_type, template, is_active, created_at, updated_at.
Filters: A dropdown for entity_type with options like 'sample', 'project', 'batch', 'analysis', 'container'.
A modal form for create/edit with fields: entity_type (select from predefined types), template (text input with examples like 'SAMPLE-{YYYY}-{SEQ}'), description (optional textarea).
Validation: Check that only one template is active per entity_type; show a warning toast if activating a new one would deactivate others.
Actions: Buttons for Activate/Deactivate, Delete (with confirmation dialog, only if not active).
Use Axios for API calls to endpoints like GET/POST/PATCH/DELETE '/admin/name-templates'.
Implement authentication checks to restrict to admin roles.
Use React hooks like useState, useEffect for state management and data fetching.
Include error handling with toasts, e.g., for API errors.
Add a preview section in the form that generates example names based on the template (e.g., 'SAMPLE-2024-001').
Export the component as default."

## 2. Prompt for Custom Attributes Config Admin Page
"Generate a React functional component named CustomAttributesAdmin for an admin page at path '/admin/custom-attributes'. It should manage custom attribute configurations with CRUD operations. Include:
A list view using a DataTable component showing columns: entity_type, attr_name, data_type (e.g., string, number, boolean), validation_rules (display as JSON), is_required, is_active.
Filters: A dropdown for entity_type with options like 'samples', 'tests', etc.
A modal form for create/edit with fields: entity_type (select), attr_name (text input), data_type (select: string, number, boolean, etc.), validation_rules (JSON editor component), is_required (checkbox), is_active (checkbox).
Actions: Buttons for Activate/Deactivate, Delete (with confirmation).
Use Axios for API calls to CRUD endpoints '/admin/custom-attributes-config'.
Implement admin-only permissions with auth checks.
Use React hooks for state and data handling.
Error handling with toasts.
Export the component as default."

## 3. Prompt for Lists and List Entries Admin Page
"Generate a React functional component named ListsAdmin for an admin page at path '/admin/lists'. It should manage lists and their entries in a two-level view with CRUD operations. Include:
First level: A list or table of 'lists' (e.g., sample_status, project_status) with columns: name, description, created_at.
For each list: A sub-view (e.g., expandable or separate tab/modal) to manage list_entries with add/edit/delete, showing columns: value, description, is_active.
Allow creating new lists (though most are predefined).
Use Axios for API calls to CRUD '/admin/lists' and '/admin/list-entries'.
Admin permissions checks.
Use hooks like useState, useEffect.
Error handling with toasts.
Two-level navigation: Clicking a list opens its entries editor.
Export the component as default."
