...
- Version 2.3: Experiment Templates management UI and SOP parse pipeline: page `ExperimentTemplatesManagement` at /experiments/templates (experiment:manage); multipart POST /v1/sop-parse, GET job, POST apply (Claude extraction via ANTHROPIC_API_KEY); mandatory review sign-off and activation guard in UI; API docs in .docs/api_endpoints.md. Sidebar Templates sub-item uses experiment:manage (same as All Experiments). See .docs/experiment-planning.md Chunk 2 (April 2026).
