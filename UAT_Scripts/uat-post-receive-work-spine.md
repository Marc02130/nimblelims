...
**Leadership Confirm (2026-08-31):** extract is never a LimsRun (experiment / equipment execute on a process); exactly one asked-for LimsRun is the assay step. **Rolf/Marc:** After extract, Qubit / Nanodrop / other QC instruments are **supporting LimsRuns** that quantify DNA (own Tests, own params freeze). **Process QC is not an asked-for.** **No route branching:** WGS asked-for on blood owns WGS params (extract → seq); C3 DNA then C2 aliquot into WGS; WES is a new asked-for on the DNA tube, which is then aliquoted or used up (own params). **OQ-WO-6 extract CLOSED.** Not Pass.
