...
| 3.1 | Navigate to Receive (`/receive`) | Page loads (user has `sample:create` permission). `/accessioning` redirects here. |
...
| **Sidebar Visibility** | - Admin section hidden from users without `config:edit` permission<br>- Lab Technician sees: Dashboard, Receive, Samples, Tests, Containers, Batches, Results, Help |
